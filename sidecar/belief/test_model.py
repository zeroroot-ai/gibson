...
These exercise the deterministic parts (evidence mapping, novel-node detection,
artifact validation) and the full exact-inference path, which now runs on numpy
alone (`infer.py`) and so has nothing left to skip on. Agreement with the pgmpy
implementation it replaced is asserted separately in `test_parity.py`, which
does skip when pgmpy is absent — the runtime image no longer ships it
(gibson#1436).
