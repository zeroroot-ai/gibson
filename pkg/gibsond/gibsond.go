...
// vaultJWTSourceCloser is the minimum surface run needs from a JWT source: it
// must satisfy jwtsource.JWTSource (so daemon.WithVaultJWTSource accepts it)
// AND expose Close() so the shutdown path can release the underlying SPIRE
...
// dumpFGAModelJSON is the source for `gibson dump-fga-model`. A package var so
// tests can substitute a failing implementation and exercise the error path
// without a broken embedded model (which cannot occur in practice).
var dumpFGAModelJSON = authz.ModelJSON

// Run is the gibson daemon entry point. It loads config, wires the SPIRE JWT
// source, constructs the daemon, and runs it until ctx is cancelled, returning
// a process exit code. Callers (OSS cmd/gibson or a hosted build) handle signal
// wiring and os.Exit. Register any closed providers (e.g. the Stripe-backed
// Entitlements provider) BEFORE calling Run.
func Run(ctx context.Context, args []string, stdout, stderr io.Writer) int {
	return run(ctx, args, stdout, stderr, defaultDaemonFactory, defaultVaultJWTSourceFactory)
}
...
func run(
	...
	// dump-fga-model is a positional subcommand, not a flag: it prints the
	// daemon's embedded OpenFGA model as JSON and exits. The install seeds
	// OpenFGA from the daemon image itself (`gibson dump-fga-model`), so the
	// seeded model is the enforced model by construction — no separately
	// vendored copy to drift. Handled before flag parsing and config load
	// because it takes no flags and must run with nothing configured.
	if len(args) > 0 && args[0] == "dump-fga-model" {
		out, err := dumpFGAModelJSON()
		if err != nil {
			_, _ = fmt.Fprintf(stderr, "gibson: dump-fga-model: %v\n", err)
			return 1
		}
	...
	// is owned by run and closed alongside the daemon. The defer
