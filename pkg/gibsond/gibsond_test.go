...
func Test_run(t *testing.T) {
	...
	tests := []struct {
		...
		{
			// The install seeds OpenFGA from `gibson dump-fga-model`, so this
			// path must emit the embedded model as JSON and exit before any
			// flag parsing or config load.
			name: "dump-fga-model prints the model JSON and exits 0",
			args: func(*testing.T) []string { return []string{"dump-fga-model"} },
			factory: func(*testing.T) daemonFactory {
				return fakeFactory(&fakeDaemon{})
			},
			jwtSourceFactory: func(*testing.T) vaultJWTSourceFactory {
...
// Test_run_dumpFGAModelError covers the dump-fga-model error path: when the
// model source fails, run prints to stderr and exits 1. The package var
// dumpFGAModelJSON is the seam that makes this reachable (the real embedded
// model cannot fail).
func Test_run_dumpFGAModelError(t *testing.T) {
	orig := dumpFGAModelJSON
	dumpFGAModelJSON = func() (string, error) { return "", errors.New("boom") }
	defer func() { dumpFGAModelJSON = orig }()

	var stdout, stderr bytes.Buffer
	code := run(context.Background(), []string{"dump-fga-model"}, &stdout, &stderr,
		fakeFactory(&fakeDaemon{}), passthroughJWTSourceFactory(&fakeJWTSource{}))
	if code != 1 {
		t.Fatalf("want exit 1 on model source error, got %d", code)
	}
	if !strings.Contains(stderr.String(), "dump-fga-model") {
		t.Errorf("stderr should name dump-fga-model; got %q", stderr.String())
	}
}
