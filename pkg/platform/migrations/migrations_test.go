...
func TestEmbed_PlatformHasExpectedFiles(t *testing.T) {
	...
	if upCount != 19 {
		t.Errorf("platform: expected 19 up.sql files, got %d", upCount)
	}
	if downCount != 19 {
		t.Errorf("platform: expected 19 down.sql files, got %d", downCount)
	}
}
...
func TestPlatformMaxVersion(t *testing.T) {
	...
	if v != 19 {
		t.Errorf("PlatformMaxVersion: got %d, want 19", v)
	}
}
