...
func (p *configProvider) Limits(ctx context.Context, tenantID string) (Limits, error) {
	...
	if p.db == nil {
		// No backing store at all. Unlimited is the OSS posture; in a
		// deployment that declares entitlements REQUIRED it is a
		// misconfiguration that must never resolve to "everything allowed".
		if Required() {
			return Limits{}, fmt.Errorf("entitlements: no platform database configured for %s: %w", tenantID, ErrEntitlementsRequired)
		}
		return Limits{}, nil
	}
...
// read performs the SELECT against tenant_quotas.
//
// A missing row means the tenant has no admin-set quota configuration. That
// resolves two different ways depending on the deployment posture:
//
...
//     tenant nobody ever provisioned limits for, which is exactly the posture
//     [BlockedProvider] exists to prevent. Return [ErrEntitlementsRequired] so
//     the enforcement layer denies instead of failing open.
func (p *configProvider) read(ctx context.Context, tenantID string) (Limits, error) {
	...
	if errors.Is(err, sql.ErrNoRows) {
		if Required() {
			return Limits{}, fmt.Errorf("entitlements: no tenant_quotas row for %s: %w", tenantID, ErrEntitlementsRequired)
		}
		return Limits{}, nil
	}
	if err != nil {
