...
func newOpenAIEmbedder(cfg Config) (Embedder, error) {
	...
	// cfg.BaseURL is tenant-supplied here just as it is for the
	// openai-compatible backend below, so it gets the same SSRF check. (The
	// connect-time guard on cfg.httpClient() is the load-bearing control; this
	// fails fast with a clear message.)
	if err := validateEndpoint(baseURL, cfg.AllowPrivateEndpoint); err != nil {
		return nil, err
	}
