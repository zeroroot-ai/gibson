...
// TestNewFromProvider_SSRFGuard_NativeOpenAIBaseURL covers the KindOpenAI
// backend, which honours a tenant base_url but used to skip validateEndpoint
// entirely — the same gap the LLM providers had.
func TestNewFromProvider_SSRFGuard_NativeOpenAIBaseURL(t *testing.T) {
	_, err := NewFromProvider(Config{
		Kind:    KindOpenAI,
