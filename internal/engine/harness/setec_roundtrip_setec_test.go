...
func TestSetecRoundTrip_UntrustedToolExecutesViaSandbox(t *testing.T) {
	...
	// the enabled tuple.
	h := newSandboxedHarness(t, exec, nil, &recordingAuthorizer{allow: true})

	ctx, cancel := context.WithTimeout(callerCtx(t, "svc-agent", "e2e"), 5*time.Minute)
	defer cancel()
...
func TestSetecRoundTrip_UntrustedDeniedWhenNotSandboxed(t *testing.T) {
	...
	req := &componentpb.CallToolRequest{ToolName: deniedTool, InputJson: env.inputJSON}
	var resp componentpb.CallToolResponse
	err = h.CallToolProto(ctx, deniedTool, req, &resp)
