...
type CallbackConfig struct {
	...
	// ServiceOptions are applied to the HarnessCallbackService this manager
	// builds. The daemon uses it to wire dependencies the harness package
	// cannot construct itself — the session-sandbox registry DevboxExec routes
	// through (gibson#1183) needs a setec client, and setec lives behind a
	// build tag in internal/server/daemon.
	//
	// Without this passthrough the option existed and nothing called it, which
	// the deadcode guard correctly reported as a feature shipped dead.
	ServiceOptions []CallbackServiceOption
...
func NewCallbackManager(cfg CallbackConfig, logger *slog.Logger) *CallbackManager {
	...
	server := NewCallbackServerWithRegistry(logger, port, registry, cfg.ServiceOptions...)
...
func (m *CallbackManager) SetWorldViewSource(source WorldViewSource) {
	if m.server != nil && m.server.service != nil {
		m.server.service.mu.Lock()
		defer m.server.service.mu.Unlock()
		m.server.service.worldViewSource = source
		m.logger.Debug("set world-view source on callback service")
	}
}
...
// the callback service so components can checkpoint session-local context via
// Put/Get/DeleteSessionContext (gibson#1184). When not set, those RPCs return
// Unavailable. Should be called before Start().
func (m *CallbackManager) SetSessionContextStore(store SessionContextStore) {
	if m.server != nil {
		m.server.SetSessionContextStore(store)
		m.logger.Debug("set session-context store on callback service")
	}
}
