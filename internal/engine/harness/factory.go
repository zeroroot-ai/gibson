...
func agentDispatchModeOrDefault(seam func(name string) (string, bool)) func(name string) (string, bool) {
	...
	return func(name string) (string, bool) {
		entry, ok := componentcatalog.LookupAgent(name)
		if !ok {
			return "", false
		}
		return entry.DispatchMode, true
	}
}
...
// resolveGraphRAG calls the querier provider, tolerating an unset one.
//
// Resolved per harness rather than once at factory construction: the daemon
...
// minterFrom reads the daemon's minter at harness creation; nil when the
// daemon has none (tests, minimal daemons).
func minterFrom(get func() *capabilitygrant.Minter) *capabilitygrant.Minter {
	if get == nil {
		return nil
	}
	return get()
