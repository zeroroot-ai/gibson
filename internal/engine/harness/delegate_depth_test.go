...
func TestDelegateToAgent_CallerChainPropagation(t *testing.T) {
	...
	_, err := harnessA.DelegateToAgent(callerCtx(t, "user-a", "acme"), "agent_b", task)
...
func TestDelegateToAgent_DepthBelowCap(t *testing.T) {
	...
	task := agent.NewTask("test", "test task", nil)
	_, err := harnessNearCap.DelegateToAgent(callerCtx(t, "user-x", "acme"), "agent_x", task)
...
func TestDelegateToAgent_DELEGATEDTORelationship(t *testing.T) {
	...
	task := agent.NewTask("test", "test task", nil)
	_, err := harnessParent.DelegateToAgent(callerCtx(t, "user-parent", "acme"), "child_agent", task)
	require.NoError(t, err)
...
func TestDelegateToAgent_ExistingDelegationTestsUnbroken(t *testing.T) {
	...
	_, err := h.DelegateToAgent(callerCtx(t, "user-recon", "acme"), "recon_agent", task)
	require.NoError(t, err, "delegation to recon_agent should succeed")

	_, err = h.DelegateToAgent(callerCtx(t, "user-recon", "acme"), "exploit_agent", task)
