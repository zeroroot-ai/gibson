...
func TestAgentDispatchModeOrDefault(t *testing.T) {
	...
	if got := agentDispatchModeOrDefault(stub); got == nil {
		t.Fatal("injected seam must be returned")
	} else if mode, ok := got("anything"); mode != "sandboxed" || !ok {
		t.Errorf("injected seam result = (%q,%v), want (sandboxed,true)", mode, ok)
	}

	// Nil seam falls back to the catalog lookup.
	def := agentDispatchModeOrDefault(nil)
	if mode, ok := def("zerocool"); mode != "sandboxed" || !ok {
		t.Errorf("default(zerocool) = (%q,%v), want (sandboxed,true) — the shipped agent manifest", mode, ok)
	}
	if mode, ok := def("does-not-exist"); ok || mode != "" {
		t.Errorf("default(unknown) = (%q,%v), want (\"\",false)", mode, ok)
	}
}
