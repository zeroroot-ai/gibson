...
// service. They have drifted — 30 methods here against 27 there — with nothing
// comparing them, which is how the knowledge reads ended up on this side and
// not that one.
//
// MEMBERSHIP IS NOT CONVERGED, on purpose. Some divergence is a real boundary:
// Metrics and MissionExecutionContext are daemon-internal and do not belong on a
// public component-dev surface. Some is rot. Nobody can tell which by
// inspection, and deciding every row inside one mechanical refactor guarantees
// none of them get reviewed. Shared names make each difference a small,
// separable question instead.
//
// A group with no members on this side is simply absent: there is no WorldReader
// (no WorldView), no Planner and no MissionManager here.

// LLMCaller is the model-facing surface.
...
// ToolCaller is dispatching tools and inspecting what is available.
...
// WorldEmitter is emitting into the tenant World. Read-side counterpart WorldReader
// does not exist on this interface — the daemon-side harness has no WorldView.
...
type KnowledgeReader interface {
	...
	//
	// One method with a scope, replacing GetPreviousRunFindings and
	// GetAllRunFindings. The scope is data, and modelling it as data is what lets
	// a caller pass it through instead of branching before the call. It also
	// matches the wire, which has always carried a single RPC with a scope field.
	//
	// Example:
	//   filter := NewFindingFilter().WithSeverity(agent.SeverityCritical)
	//   prev, err := harness.GetRunFindings(ctx, harnesspb.RunScopePrevious, *filter)
	GetRunFindings(ctx context.Context, scope harnesspb.RunScope, filter FindingFilter) ([]agent.Finding, error)

	// ── Graph reads ─────────────────────────────────────────────────────────
	//
	// These had no daemon-side harness surface at all, which is why a dispatched
	// agent could not read the tenant graph: ComponentService could serve them,
	// but nothing on this path could reach them.
	...
	// ApplicationFindings returns the open Findings of one Application with,
	// per Finding, whether the code it affects is inside an Image a Deployment
	// of that Application runs, and whether that Deployment exposes a Host.
	// It is a traversal rather than a search, which is why QueryNodes could not
	// answer it (gibson#1669). The answer is JSON-encoded
	// []component.ApplicationFinding.
	ApplicationFindings(ctx context.Context, application string, statuses []string, limit int) ([]byte, error)
}

// WorkspaceAccess is access to cloned repositories with editing and Git operations.
