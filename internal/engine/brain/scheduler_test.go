...
// nodeOf strips the mission scope from a work id, so a test can keep naming the
// node it means ("a") while the World uses the scoped id ("m1/a").
func nodeOf(workID string) string {
	if i := strings.LastIndex(workID, "/"); i >= 0 {
		return workID[i+1:]
	}
	return workID
}

// workByNode finds a work item by its node name within the World.
func workByNode(work []WorkSnapshot, node string) WorkSnapshot {
	for _, wi := range work {
		if nodeOf(wi.ID) == node {
			return wi
		}
	}
	return WorkSnapshot{}
...
func fakeDispatcher(fails map[string]bool) System {
	return func(w *World) []Event {
		...
		for _, wi := range w.WorkSnapshot() {
			...
			if fails[nodeOf(wi.ID)] {
				out = append(out, WorkCompleted{ID: wi.ID, Err: "boom"})
			} else {
				out = append(out, WorkCompleted{ID: wi.ID, Result: "ok:" + nodeOf(wi.ID)})
			}
		}
		return out
	}
...
func dispatchOrder(e *Engine) []string {
	...
	for _, ev := range e.Timeline.Events() {
		if d, ok := ev.(WorkDispatched); ok {
			// Report node names: the assertions below are about ordering
			// between named steps, not about id formatting.
			order = append(order, nodeOf(d.ID))
		}
	}
	return order
}
