...
type Mission struct {
	...
	// terminal transition, not a standing property of a completed mission: an
	// agent that marks a Finding `fixed` an hour after a scan finished must wait
	// for the NEXT scan to verify it, and without this marker the old completed
	// scan — which never saw that Finding — would verify it on the very next
	// tick. Set only by ScanReconciled.
	Reconciled bool
}
