...
func TestCrashResume_RetryPolicyReDispatches(t *testing.T) {
	...
	Reduce(w, WorkDispatched{ID: WorkID("m1", "scan"), MissionID: "m1", ItemKind: "tool", Target: "nmap"})
...
func TestCrashResume_NoRetryPolicyStaysFailed(t *testing.T) {
	...
	Reduce(w, WorkDispatched{ID: WorkID("m1", "exploit"), MissionID: "m1", ItemKind: "tool", Target: "exploit-rce"})
...
func TestCrashResume_RetryBoundedByMaxRetries(t *testing.T) {
	...
	e.Submit(WorkCompleted{ID: WorkID("m1", "scan"), Err: "interrupted: daemon restarted mid-flight"})
