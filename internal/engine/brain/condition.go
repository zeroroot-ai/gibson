...
func ConditionSystem(w *World) []Event {
	...
	//
	// Keyed by node name, not work id: a CUE expression says `nodes.scan`, which
	// is the name the mission author wrote. Work ids are mission-scoped
	// (see WorkID), so keying by them would make every expression miss.
	nodeResults := map[string]any{}
	for _, wi := range work {
		if wi.State == WorkDone {
			nodeResults[nodeName(wi.ID)] = wi.Result
		}
	}
	...
	for _, wi := range work {
		...
		// The CUE branches name nodes; the World keys work by mission-scoped id.
		scoped := make([]string, 0, len(skip))
		for _, n := range skip {
			scoped = append(scoped, WorkID(wi.MissionID, n))
		}
		out = append(out, ConditionResolved{ID: wi.ID, Result: result, Skip: scoped})
	}
	return out
}
