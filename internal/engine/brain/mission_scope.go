		...
		case MissionProjected:
			if e.ID == missionID {
				for _, n := range e.Nodes {
					// Same scoping the reducer applies; a bare node id would
					// claim another mission's identically-named node.
					owned[WorkID(e.ID, n.ID)] = true
				}
			}
		case WorkDispatched:
...
func eventInMission(ev Event, missionID string, owned map[string]bool) bool {
	...
	case FindingRaised:
		...
		return e.MissionID != "" && e.MissionID == missionID
	case EntityObserved:
		// Same mission-evidence rule as hosts and findings: a lifecycle entity
		// is in the slice of the mission whose work observed it. Empty
		// MissionID = tenant-ambient.
		return e.MissionID != "" && e.MissionID == missionID
	case FindingStatusChanged:
		return e.MissionID != "" && e.MissionID == missionID
	case ScanReconciled:
		// The reconciliation verdict belongs to the scan that reached it.
		return e.MissionID == missionID
