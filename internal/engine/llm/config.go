...
const (
	...
	// each. ProviderBedrock above is the first of the three.

	// ProviderVertex is Anthropic's models on Google Vertex.
	ProviderVertex ProviderType = "vertex"
	// ProviderFoundry is Anthropic's models on Microsoft Foundry.
...
type ProviderConfig struct {
	Type         ProviderType           `mapstructure:"type" yaml:"type" validate:"required,oneof=anthropic openai google ollama bedrock cloudflare cohere huggingface llamafile mistral vertex foundry custom voyage openai-compatible tei"`
	...
	// AllowPrivateEndpoint disables the SSRF egress guard on BaseURL. The
	// daemon sets it from security.allow_private_llm_endpoints for operators
	// running a local or in-cluster model server (the dev / air-gap path).
	//
	// The zero value is the secure one: the guard is ON. A construction site
	// that forgets to set this fails closed, never open. It deliberately has no
	// mapstructure/yaml tag — it is an operator-level decision sourced from
	// daemon config, never from a tenant-supplied provider record.
	AllowPrivateEndpoint bool `mapstructure:"-" yaml:"-"`
}
