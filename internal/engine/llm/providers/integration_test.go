	...
	tests := []struct {
		...
		},
		{
			...
			cfg: llm.ProviderConfig{
				...
				// Loopback is blocked by the SSRF egress guard unless the
				// operator opted in via security.allow_private_llm_endpoints —
				// which is what running a local Ollama means.
		...
		{
			name: "ollama provider without the private-endpoint opt-in is refused",
			cfg: llm.ProviderConfig{
				Type:         "ollama",
				DefaultModel: "llama2",
				BaseURL:      "http://localhost:11434",
