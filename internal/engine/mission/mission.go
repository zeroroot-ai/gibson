...
type Mission struct {
	...
	// AdditionalTargetIDs holds extra targets beyond the primary TargetID
	// that this mission is authorized to test. Introduced by gibson#1358
	// (mission origination): the scope rule "a component-originated child
	// mission's target UUID set must be a subset of the originating
	// mission's targets" needs the parent side of that comparison to be a
	// genuine SET, not TargetID alone. TargetSet() is the single place that
	// combines this field with TargetID — read that, not this field
	// directly, so a nil/empty AdditionalTargetIDs (every mission created
	// before this field existed, and every caller that still only sets
	// TargetID) still reports the correct one-element set.
	//
	// Deliberately NOT indexed by GetByTarget/cbMissionByTargetKey — that
	// index only tracks the primary TargetID, matching every existing
	// lookup-by-target caller. A mission is discoverable "by target" via
	// its primary target only; AdditionalTargetIDs is scope bookkeeping,
	// not a query dimension, until something needs it to be.
	AdditionalTargetIDs []types.ID `json:"additional_target_ids,omitempty"`
...
// TargetSet returns the deduplicated set of every target this mission is
// authorized to touch: the primary TargetID (when non-zero) followed by
// AdditionalTargetIDs, each first occurrence kept and later duplicates
// dropped. Order is stable — TargetID first, then AdditionalTargetIDs in
// stored order — so callers that need a deterministic representation (the
// gibson#1358 scope subset check, logging, tests) do not depend on map
// iteration order.
//
// A mission with only TargetID set (every mission that predates this
// method, and every caller that never populates AdditionalTargetIDs)
// returns exactly the one-element set built from TargetID — this method is
// the sole read path for "this mission's targets" specifically so that
// backward-compat case never needs special-casing at call sites.
func (m *Mission) TargetSet() []types.ID {
	if m == nil {
		return nil
	}
	seen := make(map[types.ID]struct{}, 1+len(m.AdditionalTargetIDs))
	out := make([]types.ID, 0, 1+len(m.AdditionalTargetIDs))
	...
	add(m.TargetID)
	for _, id := range m.AdditionalTargetIDs {
		add(id)
	}
	return out
}
