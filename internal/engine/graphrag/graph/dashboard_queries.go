...
// written by the graph projector without a tenant_id property (the per-tenant
// database is the isolation boundary), so such a predicate matches zero rows and
// silently empties every read. The old shared-instance predicate was removed
