...
// TestIntegration_FindSimilarFindingsExcludesTheSource covers the similarity
// read: given a vector hit list that contains the source finding itself, the
// caller must get back the neighbours only.
func TestIntegration_FindSimilarFindingsExcludesTheSource(t *testing.T) {
	...
	// Seed the provider directly — the read is what is under test.
	...
	// finding1 is the source and must be excluded; finding2 is the neighbour.
	require.Len(t, similar, 1, "the source finding must not come back as its own neighbour")
	assert.Equal(t, finding2.ID, similar[0].ID)
	assert.Equal(t, "injection", similar[0].Category)
}
...
func TestIntegration_FindingCorrelationAcrossMissions(t *testing.T) {
	...
	// The neighbour was recorded against a different mission than the source:
	// correlation must not be scoped to the source's mission.
	require.Len(t, similar, 1, "the cross-mission neighbour must be returned")
	assert.Equal(t, finding2.ID, similar[0].ID)
	assert.Equal(t, "xss", similar[0].Category)
	assert.NotEqual(t, finding1.MissionID, similar[0].MissionID, "the match must be cross-mission")
}
...
func TestIntegration_ObservabilitySpans(t *testing.T) {
	...
	// Perform a read that should create spans
	missionID := types.NewID()
	...
	assert.GreaterOrEqual(t, len(spans), 2, "expected at least 2 query spans (vector search + traversal)")
...
func TestIntegration_GraphRAGStoreEndToEnd(t *testing.T) {
	...
	findings := []GraphNode{
		*NewFindingNode(types.NewID(), "SQLi - Login", "SQL injection in login form", missionID).ToGraphNode(),
		*NewFindingNode(types.NewID(), "SQLi - Search", "SQL injection in search functionality", missionID).ToGraphNode(),
		*NewFindingNode(types.NewID(), "XSS - Comments", "Cross-site scripting in comments", missionID).ToGraphNode(),
	}
	for _, node := range findings {
		content := node.GetStringProperty("description")
		embedder.SetEmbedding(content, generateMockEmbedding(content, 1536))
	}

	// Test 1: Query with hybrid search
	...
	// Setup mock results
	provider.SetVectorResults([]VectorResult{
		{NodeID: findings[0].ID, Similarity: 0.95},
		{NodeID: findings[1].ID, Similarity: 0.92},
	})

	provider.SetQueriedNodes([]GraphNode{findings[0], findings[1]})
...
func TestIntegration_ErrorHandlingAndGracefulDegradation(t *testing.T) {
	...
	// Test 2: Embedder failure - the read that embeds its query text must
	// surface the failure rather than searching on a zero vector.
	embedder.SetEmbedError(NewEmbeddingError("embedding service down", nil, false))

	_, err := store.FindSimilarAttacks(ctx, "phishing", 3)
	require.Error(t, err, "should fail when embedder is down")
	...
	embedder.SetEmbedding("phishing", generateMockEmbedding("phishing", 1536))
