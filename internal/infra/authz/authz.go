...
type CheckRequest struct {
	...
	// Context supplies values for the parameters of a CONDITION attached to
	// the tuple being evaluated. OpenFGA merges this with whatever context the
	// stored tuple carries; a condition parameter present in neither makes the
	// whole Check fail with a validation error rather than a denial.
	//
	// The session-revocation gate depends on this: the stored tuple carries
	// revoked_at and the caller supplies token_issued_at per request
	// (gibson#627). Dropping it turns every gated RPC into "FGA unavailable".
...
func (c *fgaClient) doCheck(ctx context.Context, req CheckRequest) (CheckResponse, error) {
	...
	if len(req.Context) > 0 {
		ctxParams := make(map[string]interface{}, len(req.Context))
		for k, v := range req.Context {
			ctxParams[k] = v
		}
		body.Context = &ctxParams
	}
