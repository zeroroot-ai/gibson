...
// TestCheck_ForwardsConditionContext asserts the condition context reaches the
// FGA wire request.
//
// Conditioned tuples are not a softer check: if a condition parameter is absent
// from BOTH the stored tuple and the request context, OpenFGA rejects the whole
// Check as a validation error rather than denying it. ext-authz surfaces that
// as "FGA unavailable" and 503s the caller, so a dropped context takes down
// every session-gated RPC rather than failing one of them (gibson#1191).
func TestCheck_ForwardsConditionContext(t *testing.T) {
	t.Parallel()
	...
	resp, err := c.Check(context.Background(), CheckRequest{
		...
		Object:   "tenant:acme",
		Context:  map[string]any{"token_issued_at": "2026-08-05T12:00:00Z"},
	})
	if err != nil {
		t.Fatalf("Check: %v", err)
	}
	if !resp.Allowed {
		t.Fatalf("want allowed=true, got false")
	}
	...
	if err := json.Unmarshal(got, &payload); err != nil {
		t.Fatalf("unmarshal request body: %v (body=%s)", err, got)
	}
	if payload.Context["token_issued_at"] != "2026-08-05T12:00:00Z" {
		t.Fatalf("condition context missing from wire request: body=%s", got)
	}
}

// A Check with no condition context must not send an empty context object —
// omitting the field and sending {} are different requests to OpenFGA.
func TestCheck_NoConditionContextOmitsField(t *testing.T) {
	t.Parallel()
	...
	if _, err := c.Check(context.Background(), CheckRequest{
		User:     "user:alice",
		Relation: "member",
		Object:   "tenant:acme",
	}); err != nil {
	...
	if bytes.Contains(got, []byte(`"context"`)) {
		t.Fatalf("unexpected context field on an unconditioned Check: body=%s", got)
	}
}
