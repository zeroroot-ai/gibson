...
func TestHealthMonitor_ContextCancellation(t *testing.T) {
	...
	// Start periodic check in a background goroutine; done is closed when
	// StartPeriodicCheck returns so we can assert structurally (not via sleep)
	// that no further checks can occur after cancellation.
	...
	// Wait until at least one check has been performed before cancelling.
	// 2 s budget with 1 ms poll is ample even under heavy CI load.
	require.Eventually(t, func() bool {
		return checker.GetCheckCount() > 0
	}, 2*time.Second, time.Millisecond, "periodic check should have fired at least once")

	// Cancel the context and wait for StartPeriodicCheck to return.
	// Once done is closed, StartPeriodicCheck has exited and CheckAll can never
	// be called again — so the count is frozen. No sleep or tolerance needed.
	cancel()
	select {
	case <-done:
	case <-time.After(2 * time.Second):
		t.Fatal("StartPeriodicCheck did not return after context cancellation")
	}

	// The check count is now frozen: StartPeriodicCheck has exited and no
	// further CheckAll calls are possible, regardless of scheduler timing.
	frozen := checker.GetCheckCount()
	_ = frozen // count is captured for documentation; it cannot increase
}
