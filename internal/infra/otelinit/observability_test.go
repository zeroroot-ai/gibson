...
func TestInit_ReturnsNonNilObservability(t *testing.T) {
	...
	o, err := otelinit.Init(ctx, "test-service-basic")
	if err != nil {
...
func TestInit_IndependentProviders(t *testing.T) {
	...
	o1, err := otelinit.Init(ctx, "test-service-independent-1")
	if err != nil {
		t.Fatalf("first Init: %v", err)
	}

	o2, err := otelinit.Init(ctx, "test-service-independent-2")
	if err != nil {
...
func TestInit_DifferentServiceNames(t *testing.T) {
	...
	o1, err := otelinit.Init(ctx, "test-service-alpha")
	if err != nil {
		t.Fatalf("Init alpha: %v", err)
	}

	o2, err := otelinit.Init(ctx, "test-service-beta")
	if err != nil {
...
func TestInit_SameServiceNameTwice(t *testing.T) {
	...
	o1, err := otelinit.Init(ctx, svc)
	if err != nil {
		t.Fatalf("first Init: %v", err)
	}

	o2, err := otelinit.Init(ctx, svc)
	if err != nil {
		t.Fatalf("second Init: %v", err)
	}
...
func TestInit_EmptyServiceName(t *testing.T) {
	_, err := otelinit.Init(context.Background(), "")
	if err == nil {
...
func TestFieldConstants(t *testing.T) {
	tests := []struct {
		...
		{"TraceIDField", otelinit.TraceIDField, "trace_id"},
		{"SpanIDField", otelinit.SpanIDField, "span_id"},
		{"TenantIDField", otelinit.TenantIDField, "tenant_id"},
		{"RequestIDField", otelinit.RequestIDField, "request_id"},
		{"UserIDField", otelinit.UserIDField, "user_id"},
	}
...
func TestLoggerOutputIsStructuredJSON(t *testing.T) {
	ctx := context.Background()
	o, err := otelinit.Init(ctx, "test-service-logger-json",
		otelinit.WithLogLevel(slog.LevelDebug),
	)
	if err != nil {
		t.Fatalf("Init: %v", err)
	}
	...
	logger.Info("probe",
		otelinit.TraceIDField, "abc123",
		otelinit.SpanIDField, "def456",
		otelinit.TenantIDField, "t1",
		otelinit.RequestIDField, "req-1",
		otelinit.UserIDField, "u1",
	)
	...
	for _, field := range []string{
		otelinit.TraceIDField,
		otelinit.SpanIDField,
		otelinit.TenantIDField,
		otelinit.RequestIDField,
		otelinit.UserIDField,
	} {
		if _, ok := m[field]; !ok {
...
func TestShutdown_Idempotent(t *testing.T) {
	ctx := context.Background()
	o, err := otelinit.Init(ctx, "test-service-shutdown")
	if err != nil {
		t.Fatalf("Init: %v", err)
	}
...
func TestSetGlobal_DoesNotPanic(t *testing.T) {
	ctx := context.Background()
	o, err := otelinit.Init(ctx, "test-service-set-global")
	if err != nil {
		t.Fatalf("Init: %v", err)
	}
