...
func TestNoPanicOnMissingKeys(t *testing.T) {
	...
	// string keys — must not panic. The returned values are deliberately
	// discarded: this test asserts only that a lookup against an empty
	// context does not panic, which is the whole contract being pinned.
	contextkeys.GetTenantID(ctx)         //nolint:errcheck // asserting no panic, not the value
	contextkeys.GetActorID(ctx)          //nolint:errcheck // asserting no panic, not the value
	contextkeys.GetAPIKeyID(ctx)         //nolint:errcheck // asserting no panic, not the value
	contextkeys.GetParentAgentRunID(ctx) //nolint:errcheck // asserting no panic, not the value
