...
1. **Config struct.** Add `Tenant<X>Config` to `internal/infra/config/config.go`
   with admin host, port, database, username, password, ssl mode.
2. **Loader env-interpolation.** Extend the env block in
   `internal/infra/config/loader.go` so `${...}` resolves at startup. Admin
...
(`internal/platform/secrets/service.go`) reads `infra/<store>/{username,password}`
