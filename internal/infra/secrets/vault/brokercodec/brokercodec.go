...
// candidateAuthMethod validates the auth method a wire candidate asks for.
//
// A candidate may select token or AppRole auth — the two the dashboard
// offers, and the two whose credentials the caller supplies in the same
// request. Every other method, JWT included, is operator-side: those rows are
// written by the tenant-operator's provisioning saga, which does not come
// through this codec, and they authenticate with credentials the platform
// mints rather than credentials the caller typed. Accepting an
// operator-side method from a wire candidate would let a caller-supplied
// config decide what the daemon presents, and to whom.
//
// The field is a free-form string on the wire, so this is an allowlist, not a
// denylist: an unrecognised value is rejected rather than passed through to
// the provider.
func candidateAuthMethod(raw string) (vault.AuthMethod, error) {
	switch m := vault.AuthMethod(raw); m {
	case vault.AuthMethodToken, vault.AuthMethodAppRole:
		return m, nil
	case vault.AuthMethodJWT, vault.AuthMethodAWSIAM:
		// Named explicitly so adding a method to vault.AuthMethod forces a
		// decision here rather than defaulting into the reject arm silently.
		return "", fmt.Errorf("brokercodec: auth method %q is operator-side and is not selectable for a broker-config candidate; use %q or %q",
			raw, vault.AuthMethodToken, vault.AuthMethodAppRole)
	case "":
		return "", errors.New("brokercodec: auth method is required")
	default:
		return "", fmt.Errorf("brokercodec: auth method %q is not a known Vault auth method; use %q or %q",
			raw, vault.AuthMethodToken, vault.AuthMethodAppRole)
	}
}
...
func fieldsFromCandidate(c *tenantv1.CandidateConfig, tenant auth.TenantID) (Fields, error) {
	...
	method, err := candidateAuthMethod(c.GetAuthMethod())
	if err != nil {
		return Fields{}, err
	}
	return Fields{
