...
// TestEncodeCandidateAuthMethodAllowlist pins which auth methods a wire
// candidate may select.
//
// The wire field is a free-form string that reached vault.AuthConfig.Method
// unchecked, so a candidate could name an operator-side method — one whose
// credential the platform mints rather than the caller supplying it — and the
// resulting config decided what the daemon would present, and to which
// address. token and AppRole are the two the caller supplies in the same
// request, and the only two accepted.
func TestEncodeCandidateAuthMethodAllowlist(t *testing.T) {
	tenant := mustTenant(t, "acme")
	...
	for _, method := range accepted {
		t.Run("accepts_"+method, func(t *testing.T) {
			...
			if err != nil {
				t.Fatalf("EncodeCandidate(%q): unexpected error: %v", method, err)
			}
			cfg, err := Decode(blob)
			if err != nil {
				t.Fatalf("Decode: %v", err)
			}
			if string(cfg.Auth.Method) != method {
				t.Errorf("auth method: got %q, want %q", cfg.Auth.Method, method)
			}
		})
	}
	...
	for _, method := range rejected {
		t.Run("rejects_"+method, func(t *testing.T) {
			if _, _, err := EncodeCandidate(&tenantv1.CandidateConfig{
				...
				t.Fatalf("EncodeCandidate(%q): expected rejection, got none", method)
			}
		})
	}
}

// TestEncodeStillAcceptsOperatorAuthMethods is the control for the test above.
// The allowlist belongs to the wire-candidate path only: the tenant-operator's
// provisioning saga encodes JWT/role auth through Encode, and must keep
// working.
func TestEncodeStillAcceptsOperatorAuthMethods(t *testing.T) {
	_, blob, err := Encode(Fields{
		Hosted:          true,
		Address:         "http://openbao.gibson.svc:8200",
		NamespaceOrPath: "tenant-acme",
		Auth:            vault.AuthConfig{Method: vault.AuthMethodJWT, Role: "gibson-daemon"},
		Tenant:          mustTenant(t, "acme"),
	})
	if err != nil {
		t.Fatalf("Encode: %v", err)
	...
	if cfg.Auth.Method != vault.AuthMethodJWT {
		t.Errorf("operator-side JWT auth must survive Encode: got %q", cfg.Auth.Method)
	}
}
