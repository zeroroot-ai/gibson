...
type Config struct {
	...
	// guardEgress installs the connect-time egress guard on the client this
	// Config builds. It is unexported on purpose: NewGuarded is the only way
	// to set it, so no persisted or caller-supplied JSON blob can clear a
	// guard the daemon asked for. Copying a Config (by value, or through
	// NewWithRefresher's internal token-config copy) carries it along.
	guardEgress bool
}
...
// NewGuarded constructs a Provider exactly like New, with one difference:
// every connection the client opens is vetted at connect time, and refused
// when the resolved address is loopback, link-local (which includes the cloud
// metadata service), private, CGNAT, multicast or reserved.
//
// Use it wherever cfg.Address did not come from the operator. The daemon's
// broker-config probe path is the case that matters: there the address
// arrives in an RPC from a tenant admin, and the probe both dials it and
// presents credentials to whatever answers. New stays unguarded because the
// platform's own broker row legitimately points at an in-cluster address.
//
// The guard cannot be turned off through cfg: the field it sets is
// unexported, so a config blob cannot clear it.
func NewGuarded(ctx context.Context, cfg Config) (*Provider, error) {
	cfg.guardEgress = true
	return New(ctx, cfg)
}
...
func (p *Provider) Health(ctx context.Context) error {
	client := p.client
	// sys/health is a ROOT/global endpoint. In namespace mode the client carries
	// X-Vault-Namespace=<tenant-ns> on every request, and OpenBao rejects
	// sys/health inside a namespace with 400 "operation unavailable in
	// namespaces". Clone the client and drop the namespace header so the health
	// check targets the root. (The clone shares config; sys/health needs no
	// token, so the token is irrelevant here.)
	if p.cfg.Namespace != "" {
		clone, cerr := p.client.CloneWithHeaders()
		if cerr != nil {
			return fmt.Errorf("%w: vault client clone for health: %w", secrets.ErrUnavailable, cerr)
		}
...
func mapVaultError(err error, name string) error {
	...
	//
	// The bare "404" substring check this replaced was unsound: msg is the
	// fully wrapped error, which embeds the request URL — including the
	// server's port. A bare "404" collides with any port whose digits
	// happen to contain that substring (e.g. 127.0.0.1:44049), silently
	// misclassifying an unrelated status (403, in the flake that surfaced
	// this — gibson#1386) as not-found. "Code: 404" anchors to the literal
	// prefix ResponseError.Error() always emits before the status code
	// (see (*api.ResponseError).Error()'s "Code: %d. %s:" format), the same
	// pattern already used safely for 403 below.
	msg := err.Error()
	if strings.Contains(msg, "secret not found") || strings.Contains(msg, "Code: 404") {
		if name != "" {
