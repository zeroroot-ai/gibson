...
type SecurityConfig struct {
	...
	// tenant-supplied broker-config candidate. Off by default (the secure
	// default) — a candidate whose address resolves to a private,
	// link-local or loopback destination is refused at dial time.
	//
	// It is a separate knob rather than a reuse of the LLM one because the
	// two decisions are unrelated: an operator running a local model server
	// has not thereby agreed to let a tenant admin aim the broker probe at
	// the cluster's internal network.
	//
	// The platform's own broker row is unaffected either way. It is written
	// directly to the platform database by the tenant-operator's
	// provisioning saga, never through the probe path, so the in-cluster
	// Vault address every deployment uses keeps working with this off. Turn
	// it on only for a self-hosted install whose tenants legitimately point
	// BYO Vault at an in-cluster address.
