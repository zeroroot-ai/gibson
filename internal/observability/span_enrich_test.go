...
// TestEnrichSpan_LangfuseUserID asserts the Langfuse-specific magic key is set
// alongside the flat user_id, so Langfuse populates the filterable trace.userId.
func TestEnrichSpan_LangfuseUserID(t *testing.T) {
	ctx := auth.ContextWithActingUser(context.Background(), "user-alice")
	got := captureAttrs(t, ctx, "TestSpan")

	if v := got[AttrLangfuseUserID].AsString(); v != "user-alice" {
		t.Errorf("langfuse.user.id = %q, want user-alice", v)
	}
	// Flat attribute is preserved.
	if v := got[AttrUserID].AsString(); v != "user-alice" {
		t.Errorf("user_id = %q, want user-alice", v)
	}
}

// TestEnrichSpan_LangfuseUserID_UnknownFallback asserts the magic key carries
// the unknown sentinel when no identity is resolvable (consistent with flat).
func TestEnrichSpan_LangfuseUserID_UnknownFallback(t *testing.T) {
	got := captureAttrs(t, context.Background(), "TestSpan")
	if v := got[AttrLangfuseUserID].AsString(); v != unknownUserSentinel {
		t.Errorf("langfuse.user.id = %q, want %q", v, unknownUserSentinel)
	}
}

// TestEnrichSpan_LangfuseTraceTags asserts mission + agent context produce the
// namespaced Langfuse trace tags that the dashboard's ?tags= filter queries.
func TestEnrichSpan_LangfuseTraceTags(t *testing.T) {
	ctx := context.Background()
	ctx = context.WithValue(ctx, contextkeys.MissionID, "mission-1")
	ctx = context.WithValue(ctx, contextkeys.AgentName, "nmap-agent")
	...
	tags := got[AttrLangfuseTraceTags].AsStringSlice()
	assertContains(t, tags, "mission:mission-1")
	assertContains(t, tags, "agent:nmap-agent")
}

// TestEnrichSpan_LangfuseTraceTags_MissionOnly asserts only the mission tag is
// emitted when no agent is in context (the root mission span case).
func TestEnrichSpan_LangfuseTraceTags_MissionOnly(t *testing.T) {
	ctx := context.WithValue(context.Background(), contextkeys.MissionID, "mission-9")
	got := captureAttrs(t, ctx, "TestSpan")

	tags := got[AttrLangfuseTraceTags].AsStringSlice()
	if len(tags) != 1 || tags[0] != "mission:mission-9" {
		t.Errorf("tags = %v, want [mission:mission-9]", tags)
	}
}

// TestEnrichSpan_LangfuseTraceTags_AbsentWithoutMissionOrAgent asserts the tag
// attribute is omitted entirely when neither mission nor agent is present, so
// infrastructure spans (health probes, authz checks) carry no trace tags.
func TestEnrichSpan_LangfuseTraceTags_AbsentWithoutMissionOrAgent(t *testing.T) {
	got := captureAttrs(t, context.Background(), "TestSpan")
	if _, ok := got[AttrLangfuseTraceTags]; ok {
		t.Error("langfuse.trace.tags should be absent when no mission/agent in ctx")
	}
}
...
func assertContains(t *testing.T, haystack []string, needle string) {
	...
	t.Errorf("tags %v does not contain %q", haystack, needle)
}
