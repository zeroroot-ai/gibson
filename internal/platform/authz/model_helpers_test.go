...
// loadModelFromDSL reads internal/platform/authz/model.fga (the authoritative schema),
...
func loadModelFromDSL(t *testing.T, ctx context.Context, baseURL, storeID string) string {
	...
	// NOTE: the payload struct must include Conditions so that models using
	// OpenFGA CEL conditions (e.g. token_not_revoked introduced in gibson#627)
	// are loaded correctly. Omitting Conditions causes OpenFGA to reject the
	// model with "condition X is undefined for relation Y" at write time.
	var payload struct {
		SchemaVersion   string                      `json:"schema_version"`
		TypeDefinitions []fgasdk.TypeDefinition     `json:"type_definitions"`
		Conditions      map[string]fgasdk.Condition `json:"conditions,omitempty"`
	}
	...
	req := fgaclient.ClientWriteAuthorizationModelRequest{
		SchemaVersion:   payload.SchemaVersion,
		TypeDefinitions: payload.TypeDefinitions,
	}
	if len(payload.Conditions) > 0 {
		req.Conditions = &payload.Conditions
	}
