...
var allowedUnauthenticated = map[string]bool{
	...
	//
	// What authorizes them instead is a capability chain the daemon issues and
	// controls end to end:
	//
	//   - RequestEmailVerification takes no capability. It is the entry point,
	//     so it is guarded by daemon-side rate limits (per address, per IP,
	//     global) that fail CLOSED, and it creates nothing but one expiring row
	//     and one email. Its response is empty and identical for every accepted
	//     request, so it cannot be used to enumerate registered addresses.
