...
// migration set, not this repo — see
// pkg/platform/migrations/postgres/platform/022_audit_log_hash_chain.up.sql),
// and the former in-package RunAuditMigrations helper was removed; this
// ephemeral test owns its container DB, so it defines the table itself. The
// column set and nullability mirror that migration, verbatim: id/created_at
// are server-defaulted, decision is NULLable (empty Decision is stored as
// NULL — see writer.go), metadata is BYTEA because the Writer passes a
// []byte parameter via lib/pq (which would be rejected by a JSONB column
