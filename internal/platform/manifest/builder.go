...
func (b *manifestBuilder) Build(ctx context.Context, subject ManifestSubject) (*manifestpb.CapabilityManifest, error) {
	...
	if b.deps.Audit != nil {
		issuance := Issuance{
			...
			Subject:                      subjectFGA,
			TenantID:                     tenantID,
			ImpersonatedAgentPrincipalID: subject.ImpersonatedAgentPrincipalID,
		}
		if err := b.deps.Audit.RecordIssuance(ctx, m, issuance, nil); err != nil {
	...
	b.deps.Logger.Info("manifest: issued",
		...
		"subject", subjectFGA,
		"actor", actor.FGARef(),
		"impersonated_agent_principal_id", subject.ImpersonatedAgentPrincipalID,
		"version", version,
...
func validateSubject(s ManifestSubject) error {
	...
	// Impersonation must name its actor. Falling back to "actor == subject"
	// here would record the impersonated principal as having requested its
	// own manifest — worse than no attribution, because it reads as a clean
	// self-issuance.
	if s.ImpersonatedAgentPrincipalID != "" && s.Actor.FGARef() == "" {
		return errors.New("manifest: impersonated issuance requires an Actor — refusing to issue an unattributable manifest")
	}
	return nil
}
...
func projectComponents(infos []component.ComponentInfo, perms map[string][]string) (agents, tools, plugins []*manifestpb.ComponentCapability, scope []capabilitygrant.ComponentRef) {
	...
	for _, info := range infos {
		...
		cc := &manifestpb.ComponentCapability{
			...
			ComponentRef:  authz.ComponentObject(info.Kind, info.Name),
