...
func TestIntegration_FullPipeline(t *testing.T) {
	...
	stubAuth := &stubAuthorizer{
		listObjects: map[string][]string{
			"user:alice|can_execute|component":              {"component:tool/nmap"},
			"user:alice|can_read|component":                 {"component:plugin/gitlab"},
			"user:alice|can_configure|component":            nil,
			"agent_principal:A|cannot_invoke|component":     {"component:plugin/gitlab"},
...
func TestIntegration_AgentPrincipalCrossDeny(t *testing.T) {
	...
	stubAuth := &stubAuthorizer{
		listObjects: map[string][]string{
			"user:owner|can_execute|component":              {"component:plugin/gitlab"},
			"user:owner|can_read|component":                 nil,
			"user:owner|can_configure|component":            nil,
			"agent_principal:A|can_execute|component":       {"component:plugin/gitlab"},
	...
	if r.SourceComponentRef != "agent_principal:A" || r.TargetComponentRef != "component:plugin/gitlab" {
		t.Fatalf("rule = %+v", r)
	}
}
