...
func benchBuilder(b *testing.B, componentCount, denyRuleCount int) {
	...
	for i := 0; i < denyRuleCount && i < componentCount; i++ {
		listObjects["agent_principal:A|cannot_invoke|component"] = append(listObjects["agent_principal:A|cannot_invoke|component"], "component:tool/c"+strconv.Itoa(i))
	}
...
func TestBuilderBuild_P95Budget_50c_100r(t *testing.T) {
	...
	if p95 > 100*time.Millisecond {
		t.Fatalf("p95 sample %v exceeded 100ms budget", p95)
	}
}
