...
type CreateHumanUserRequest struct {
	...
	// email.
	//
	// Setting this true is only honest when the CALLER has already established
	// control of the address. The signup path sets it because the user is
	// created strictly after redeeming an emailed token, so the claim it makes
	// to the IdP is one the daemon can actually back. Setting it true on an
	// unproven address asserts a fact nobody checked and marks the account
	// sign-in-ready on the strength of a form submission.
	EmailVerified bool
}

// SetHumanPasswordRequest sets a known password on an EXISTING human user,
// activating an account that was created without a usable credential — e.g. the
// founding-owner user the TenantMember invitation flow creates in an initial
// state on a headless self-hosted install, which CreateHumanUser then cannot
// create afresh. OrgID scopes the call; UserID and Password are required.
...
// existing email is an error (ErrAlreadyExists), not a result variant. The
// field used to mean "we resumed and reset that account's password", which is
// the behaviour that was removed.
type CreateHumanUserResult struct {
	// UserID is the IdP-assigned id of the newly created human user.
	UserID string
}
