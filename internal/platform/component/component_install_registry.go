...
// descriptorSetForDB binds a component's proto descriptor set for the INSERT.
//
...
// genuinely has none: a descriptor set describes a compiled tool's protos, not a
// remote worker that speaks JSON over the work queue.
//
// The DEFAULT never applied, because the INSERT names the column, and a nil
// []byte binds as NULL. So every such registration violated the constraint and
// no component_install row was ever written, while the Redis registry recorded
// the instance happily — two stores disagreeing permanently, behind a
// "non-fatal" warning (gibson#1205).
func descriptorSetForDB(in []byte) []byte {
	if in == nil {
		return []byte{}
	}
	return in
