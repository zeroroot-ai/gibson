...
//	Owner binding:  gibson:work:owner:{work_id}          (work_context.go)
//	Result key:     result:{tenant}:{work_id}            (5-minute TTL)
//	Notify channel: result_notify:{tenant}:{work_id}     (pub/sub)
//
// Result keys and notify channels are namespaced by the tenant that enqueued
// the work item. Enqueue records that tenant in the owner binding, and both
// DeliverResult and WaitForResult resolve the namespace from that binding
// rather than from their own arguments, so a work id from one tenant can never
// name another tenant's result slot.
type WorkQueue interface {
	...
	// DeliverResult stores the result under the owning tenant's result key with
	// a 5-minute TTL and publishes a notification on that tenant's notify
	// channel so that any caller blocked in WaitForResult can unblock
	// immediately. It returns an error wrapping ErrWorkOwnerUnknown when the
	// work id has no owner binding.
	//
	// DeliverResult routes; it does not authorise. Callers that accept a work
	// id from a remote component must check ownership themselves before
	// delivering — see ComponentServiceServer.resolveWorkOwner.
	DeliverResult(ctx context.Context, workID string, result WorkResult) error

	// WaitForResult subscribes to the owning tenant's notify channel and returns
	// the result as soon as it arrives, or reads it directly from that tenant's
	// result key if it is already present. Returns an error wrapping
	// context.DeadlineExceeded when timeout elapses without a result, or one
	// wrapping ErrWorkOwnerUnknown when the work id has no owner binding.
...
//
//	result:{tenant}:{work_id}
//
// tenant is the work item's owning tenant, resolved from the owner binding —
// never a value supplied by the component returning the result.
func resultKey(tenant, workID string) string {
	return fmt.Sprintf("result:%s:%s", tenant, workID)
}

// notifyChannel returns the pub/sub channel used to signal result delivery.
//
//	result_notify:{tenant}:{work_id}
func notifyChannel(tenant, workID string) string {
	return fmt.Sprintf("result_notify:%s:%s", tenant, workID)
}
...
func (q *redisWorkQueue) Enqueue(ctx context.Context, tenant, kind, name string, item WorkItem) (string, error) {
	...
	// Bind the work id to this tenant before the item becomes visible on the
	// stream. Everything downstream that is handed a work id by a component
	// resolves the owner from this binding, so it has to exist before the id
	// can leave the daemon. A failure here is fatal to the enqueue: work whose
	// result could not be routed or authorised must not be dispatched.
	if err := bindWorkOwner(ctx, q.client, item.WorkID, tenant); err != nil {
		return "", fmt.Errorf("work queue enqueue: %w", err)
	}
...
// DeliverResult stores the result JSON under the owning tenant's result key
// with a 5-minute TTL, then publishes a signal on that tenant's notify channel.
//
// The namespace comes from the work id's owner binding, so a result can only
// ever land in the slot belonging to the tenant that enqueued the work.
func (q *redisWorkQueue) DeliverResult(ctx context.Context, workID string, result WorkResult) error {
	...
	owner, err := lookupWorkOwner(ctx, q.client, workID)
	if err != nil {
		return fmt.Errorf("work queue deliver result: work %s: %w", workID, err)
	}
	...
	if err := q.client.Set(ctx, resultKey(owner, workID), data, resultTTL).Err(); err != nil {
	...
	if err := q.client.Publish(ctx, notifyChannel(owner, workID), workID).Err(); err != nil {
...
// WaitForResult subscribes to the owning tenant's notify channel and returns as
// soon as a result arrives, or returns context.DeadlineExceeded if timeout
// elapses.
//
// It also checks the result key before blocking on pub/sub to handle the race
// where DeliverResult completes before the subscription is established.
//
// The owner binding is written by Enqueue, and every caller waits on a work item
// it has just enqueued, so the lookup resolves. A work id with no binding is not
// a work item this queue dispatched; waiting on it would block until the timeout
// for a result that can never arrive, so it fails immediately instead.
func (q *redisWorkQueue) WaitForResult(ctx context.Context, workID string, timeout time.Duration) (*WorkResult, error) {
	...
	owner, err := lookupWorkOwner(ctx, q.client, workID)
	if err != nil {
		return nil, fmt.Errorf("work queue wait for result: work %s: %w", workID, err)
	}
	...
	pubsub := q.client.Subscribe(timeoutCtx, notifyChannel(owner, workID))
	defer pubsub.Close()
	...
	if result, err := q.fetchResult(timeoutCtx, owner, workID); err == nil {
		return result, nil
	}
	...
	for {
		...
		case _, ok := <-msgChan:
			...
			result, err := q.fetchResult(timeoutCtx, owner, workID)
			if err != nil {
...
// fetchResult reads and deserializes a WorkResult from the owning tenant's
// result key. Returns an error when the key does not exist yet.
func (q *redisWorkQueue) fetchResult(ctx context.Context, owner, workID string) (*WorkResult, error) {
	data, err := q.client.Get(ctx, resultKey(owner, workID)).Bytes()
	if err != nil {
		if err == redis.Nil {
