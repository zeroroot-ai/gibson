...
func (s *ComponentServiceServer) CompleteWithTools(
	...
	if err != nil {
		...
		return nil, completionStatus(err)
	}
...
// completionStatus surfaces the LLM seam's own gRPC code to the caller.
//
// The adapter already answers with a precise code — Unavailable for a tenant
// with no usable provider, Unimplemented for a provider that cannot enforce a
// JSON schema, InvalidArgument for a malformed request. Re-wrapping all of
// them as Internal told the component "the LLM broke" for what are really
// configuration answers it can act on.
func completionStatus(err error) error {
