...
// enqueueOwnedWork puts a work item on tenant's stream so the queue records the
// work-id-to-owning-tenant binding that DeliverResult and WaitForResult resolve
// their result namespace from. Result-delivery tests go through it rather than
// inventing a bare work id, because a work id with no owner is precisely what
// the queue is meant to refuse.
func enqueueOwnedWork(t *testing.T, q WorkQueue, tenant, workID string) {
	t.Helper()

	_, err := q.Enqueue(context.Background(), tenant, "tool", "owned-work", WorkItem{
		WorkID:   workID,
		WorkType: "execute",
	})
	require.NoError(t, err, "Enqueue must succeed to bind the work id to its tenant")
}
...
func TestWorkQueue_DeliverAndWait(t *testing.T) {
	...
	enqueueOwnedWork(t, q, "acme", workID)
...
func TestWorkQueue_WaitForResult_Timeout(t *testing.T) {
	...
	enqueueOwnedWork(t, q, "acme", "work-never-delivered")
