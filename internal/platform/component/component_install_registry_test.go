...
func TestPluginRegistry_Upsert(t *testing.T) {
	...
	rows := tr.store.list("tenant-abc", "lookup")
	if len(rows) != 1 {
...
func TestPluginRegistry_HeartbeatExpiry_ExcludedFromList(t *testing.T) {
	...
	installs, err := tr.listInstalls(ctx, auth.MustNewTenantID("tenant-abc"), "lookup")
	if err != nil {
...
func TestPluginRegistry_RoundRobin(t *testing.T) {
	...
	installs, err := tr.listInstalls(ctx, tenant, "lookup")
...
func TestPluginRegistry_Status_IncludesUnreachable(t *testing.T) {
	...
	inst1 := &ComponentInstall{TenantID: tenant, Name: "lookup", Version: "1.0", HostID: "h1"}
	inst2 := &ComponentInstall{TenantID: tenant, Name: "lookup", Version: "1.0", HostID: "h2"}
	...
	rows := tr.store.list(tenant.String(), "lookup")
