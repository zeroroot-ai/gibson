...
func resolveMissionContext(
	...
	return resolver.ResolveMissionForWork(ctx, workID, tenant)
}
...
// LLM slot overrides. caller is the authenticated tenant of the requesting
// component; the work id itself arrives from the component and is NOT trusted —
// a work context recorded for another tenant resolves to nothing (gibson#1250,
// same rule as SubmitResult/SubmitFinding after gibson#1248).
//
// Return semantics (all three outcomes are non-error):
//   - ("", nil, nil)   — no work context exists, or the work context does not
//     belong to the caller; caller uses tenant defaults.
...
func (r *MissionContextResolver) ResolveMissionForWork(
	...
	// ------------------------------------------------------------------
	// Step 2: verify the work context belongs to the caller. The work id
	// arrives from the remote component, so the tenant recorded at
	// RegisterWorkContext time is the owner of record and the caller must
	// match it (or be the shared-component _system tenant). A mismatch —
	// including a missing owner or an unauthenticated caller — resolves to
	// no mission rather than leaking another tenant's mission id.
	// ------------------------------------------------------------------
	owner := fields[workContextTenantField]
	if !tenantMayActOnWork(caller, owner) {
		r.logger.WarnContext(ctx, "mission context resolver: work context does not belong to the calling tenant; ignoring component-supplied work id",
			slog.String("caller_tenant", caller),
	...
	// ------------------------------------------------------------------
	// Step 3: look up per-mission slot overrides at the tenant-scoped key.
	// The read is scoped to the verified owner: identical to the caller for
	// every ordinary component, and the mission's own tenant when the
	// _system carve-out lets a shared component act on tenant work.
	// ------------------------------------------------------------------
	tenantCtx := auth.ContextWithTenantString(ctx, owner)
	tenant := owner
	...
	// ------------------------------------------------------------------
	// Step 4: decode the JSON overrides blob.
	// ------------------------------------------------------------------
