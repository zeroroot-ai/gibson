...
func (v *JWTVerifier) VerifyAgentJWT(ctx context.Context, tokenStr, expectedAud string) (*AgentClaims, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("capabilitygrant: VerifyAgentJWT: %w", err)
	}
	...
	if hdr.Typ != "agent+jwt" {
		return nil, fmt.Errorf("capabilitygrant: VerifyAgentJWT: unexpected typ %q, want \"agent+jwt\"", hdr.Typ)
	}
	if hdr.Alg != "EdDSA" {
		return nil, fmt.Errorf("capabilitygrant: VerifyAgentJWT: unsupported alg %q, only EdDSA is accepted", hdr.Alg)
	}
	...
	if payload.Aud != expectedAud {
		return nil, fmt.Errorf("capabilitygrant: VerifyAgentJWT: audience mismatch: got %q, want %q", payload.Aud, expectedAud)
	}

	if payload.ComponentScope == "" {
		return nil, fmt.Errorf("capabilitygrant: VerifyAgentJWT: component_scope is required (spec R2 AC 5)")
	}
	...
	if now.After(expiresAt) {
		return nil, fmt.Errorf("capabilitygrant: VerifyAgentJWT: token expired at %s", expiresAt.Format(time.RFC3339))
	}
	if expiresAt.After(now.Add(maxTokenFuture)) {
		return nil, fmt.Errorf("capabilitygrant: VerifyAgentJWT: token exp is too far in the future (%s)", expiresAt.Format(time.RFC3339))
	}

	// Resolve agent record from the store.
	agent, err := v.store.GetAgent(ctx, payload.Sub)
	if err != nil {
		return nil, fmt.Errorf("capabilitygrant: VerifyAgentJWT: store lookup: %w", err)
	}
	if agent == nil {
		return nil, fmt.Errorf("capabilitygrant: VerifyAgentJWT: unknown agent %q", payload.Sub)
	}
	if agent.Status != "active" {
		return nil, fmt.Errorf("capabilitygrant: VerifyAgentJWT: agent %q has status %q, must be active", payload.Sub, agent.Status)
	}
	if agentCredentialLapsed(agent, now) {
		return nil, fmt.Errorf("capabilitygrant: VerifyAgentJWT: agent %q lapsed at %s",
			payload.Sub, agent.ExpiresAt.Format(time.RFC3339))
	}

	// Parse the agent's registered Ed25519 public key.
	pubKey, err := parseJWKEd25519(agent.PublicKeyJWK)
	if err != nil {
		return nil, fmt.Errorf("capabilitygrant: VerifyAgentJWT: parse agent public key: %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("capabilitygrant: VerifyAgentJWT: decode signature: %w", err)
	}
	if !ed25519.Verify(pubKey, signingInput, sig) {
		return nil, fmt.Errorf("capabilitygrant: VerifyAgentJWT: signature verification failed")
	}
...
// IsCapabilityGrantJWT reports whether token looks like an Agent Auth JWT by
...
// valid.
func IsCapabilityGrantJWT(token string) bool {
