...
func TestResolveCrossComponentRules_DenyOverridesAllow(t *testing.T) {
	...
	auth := &mockAuthorizer{
		listObjectsFunc: func(user, relation, objectType string) ([]string, error) {
			...
			if user == "agent_principal:A" && relation == "can_be_invoked_by" && objectType == "component" {
				return []string{"component:plugin/gitlab", "component:plugin/slack"}, nil
			}
			return nil, nil
		},
	}
	...
	assert.Equal(t, CrossRule{Source: "agent_principal:A", Target: "component:plugin/gitlab", Effect: EffectDeny, Reason: "fga_deny"}, rules[0])
	assert.Equal(t, CrossRule{Source: "agent_principal:A", Target: "component:plugin/slack", Effect: EffectAllow, Reason: "fga_grant"}, rules[1])
}
	...
	auth := &mockAuthorizer{
		listObjectsFunc: func(user, relation, objectType string) ([]string, error) {
			if relation == "cannot_invoke" {
				return []string{"component:tool/unscoped"}, nil
			}
			return nil, nil
		},
	}
...
func TestResolveCrossComponentRules_ComponentSubjectSkipsCannotInvoke(t *testing.T) {
	...
	b := NewFGABridge(auth, &mockRegistry{}, noopLogger())
	rules, err := b.ResolveCrossComponentRules(context.Background(), "component:agent/nmap-agent",
	...
	auth := &mockAuthorizer{
		listObjectsFunc: func(user, relation, objectType string) ([]string, error) {
			...
			case user == "user:owner" && relation == "can_execute":
				return []string{"component:tool/nmap", "component:tool/sslyze"}, nil
			case user == "agent_principal:A" && relation == "can_execute":
				return []string{"component:tool/nmap", "component:plugin/gitlab"}, nil
			}
			return nil, nil
		},
	}
