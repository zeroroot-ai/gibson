...
// workQueueDispatcher implements agentauth.ComponentDispatcher using the
...
func (d *workQueueDispatcher) Dispatch(ctx context.Context, tenant, kind, name string, input []byte) ([]byte, error) {
	if tenant == "" {
		return nil, fmt.Errorf("agentauth dispatcher: tenant cannot be empty")
	}
	if kind == "" {
		return nil, fmt.Errorf("agentauth dispatcher: kind cannot be empty")
	}
	if name == "" {
		return nil, fmt.Errorf("agentauth dispatcher: name cannot be empty")
	}
	...
	if err != nil {
		return nil, fmt.Errorf("agentauth dispatcher: enqueue work for %s/%s/%s: %w", tenant, kind, name, err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("agentauth dispatcher: wait for result from %s/%s/%s: %w", tenant, kind, name, err)
	}

	if result.Error != nil {
		return nil, fmt.Errorf("agentauth dispatcher: component %s/%s/%s returned error: [%s] %s",
