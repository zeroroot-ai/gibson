...
func TestListAlerts_EmptyTenantIDFallsBackToSystemTenant_NilStoreReturnsEmpty(t *testing.T) {
	...
	srv := blankServer()
	resp, err := srv.ListAlerts(context.Background(), &ListAlertsRequest{TenantId: "", UserId: "u1"})
...
func TestListAlerts_MissingUserID_InvalidArgument(t *testing.T) {
	srv := blankServer()
	_, err := srv.ListAlerts(context.Background(), &ListAlertsRequest{TenantId: "acme", UserId: ""})
...
func TestListAlerts_NilStore_ReturnsEmpty(t *testing.T) {
	...
	srv := blankServer()
	resp, err := srv.ListAlerts(context.Background(), &ListAlertsRequest{TenantId: "acme", UserId: "u1"})
...
func TestListAlerts_StoreError_Internal(t *testing.T) {
	...
	_, err := srv.ListAlerts(context.Background(), &ListAlertsRequest{TenantId: "acme", UserId: "u1"})
...
func TestListAlerts_UnreadOnly_FiltersReadAlerts(t *testing.T) {
	...
	resp, err := srv.ListAlerts(context.Background(), &ListAlertsRequest{
		TenantId:   "acme",
		UserId:     "u1",
		UnreadOnly: true,
	})
...
func TestMarkAlertRead_EmptyTenantIDFallsBackToSystemTenant_NilStoreNoError(t *testing.T) {
	...
	srv := blankServer()
	_, err := srv.MarkAlertRead(context.Background(), &MarkAlertReadRequest{TenantId: "", AlertId: "a1"})
	assert.NoError(t, err)
}

func TestMarkAlertRead_MissingAlertID_InvalidArgument(t *testing.T) {
	srv := blankServer()
	_, err := srv.MarkAlertRead(context.Background(), &MarkAlertReadRequest{TenantId: "acme", AlertId: ""})
	assert.Equal(t, codes.InvalidArgument, grpcCode(err))
}
...
func TestMarkAlertRead_NilStore_NoError(t *testing.T) {
	...
	srv := blankServer()
	_, err := srv.MarkAlertRead(context.Background(), &MarkAlertReadRequest{TenantId: "acme", AlertId: "a1"})
	assert.NoError(t, err)
}
...
func TestMarkAllAlertsRead_EmptyTenantIDFallsBackToSystemTenant_NilStoreReturnsZero(t *testing.T) {
	...
	srv := blankServer()
	resp, err := srv.MarkAllAlertsRead(context.Background(), &MarkAllAlertsReadRequest{TenantId: "", UserId: "u1"})
	require.NoError(t, err)
...
func TestMarkAllAlertsRead_MissingUserID_InvalidArgument(t *testing.T) {
	srv := blankServer()
	_, err := srv.MarkAllAlertsRead(context.Background(), &MarkAllAlertsReadRequest{TenantId: "acme", UserId: ""})
	assert.Equal(t, codes.InvalidArgument, grpcCode(err))
}
...
func TestMarkAllAlertsRead_NilStore_ReturnsZero(t *testing.T) {
	...
	srv := blankServer()
	resp, err := srv.MarkAllAlertsRead(context.Background(), &MarkAllAlertsReadRequest{TenantId: "acme", UserId: "u1"})
	require.NoError(t, err)
	assert.Equal(t, int32(0), resp.Count)
}
...
func TestMarkAllAlertsRead_StoreError_Internal(t *testing.T) {
	...
	_, err := srv.MarkAllAlertsRead(context.Background(), &MarkAllAlertsReadRequest{TenantId: "acme", UserId: "u1"})
	assert.Equal(t, codes.Internal, grpcCode(err))
}
