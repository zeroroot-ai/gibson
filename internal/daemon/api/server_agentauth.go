// Package api — server_agentauth.go
//
...
// to agentAuthService (which holds the real logic), map domain results back to
// proto response types.
//
// Error mapping:
//
//	agentAuthService not wired     → codes.Unavailable
...
// RegisterAgentAuth upserts a host record, creates an agent record, resolves
// FGA capability grants, writes grants to the store, and emits an audit event.
func (s *DaemonServer) RegisterAgentAuth(ctx context.Context, req *RegisterAgentAuthRequest) (*RegisterAgentAuthResponse, error) {
	if s.agentAuthService == nil {
	...
	result, err := s.agentAuthService.RegisterAgentAuth(
		ctx,
		tenantID,
		req.GetOwnerUserId(),
		req.GetAgentName(),
	...
	if err != nil {
		s.logger.ErrorContext(ctx, "RegisterAgentAuth: service error",
	...
	return &RegisterAgentAuthResponse{
...
func (s *DaemonServer) ExecuteAgentCapability(ctx context.Context, req *ExecuteAgentCapabilityRequest) (*ExecuteAgentCapabilityResponse, error) {
	...
	result, err := s.agentAuthService.ExecuteAgentCapability(
		ctx,
		req.GetAgentId(),
...
func (s *DaemonServer) ListAgentCapabilities(ctx context.Context, req *ListAgentCapabilitiesRequest) (*ListAgentCapabilitiesResponse, error) {
	...
	caps, err := s.agentAuthService.ListAgentCapabilities(ctx, tenantID, req.GetUserId())
	if err != nil {
...
// GetAgentAuthStatus returns the current agent status and its capability grants.
func (s *DaemonServer) GetAgentAuthStatus(ctx context.Context, req *GetAgentAuthStatusRequest) (*GetAgentAuthStatusResponse, error) {
	if s.agentAuthService == nil {
		return nil, status_grpc.Error(codes.Unavailable, "agent auth service not configured")
	}
	if req.GetAgentId() == "" {
	...
	result, err := s.agentAuthService.GetAgentAuthStatus(ctx, req.GetAgentId(), tenantID)
	if err != nil {
		s.logger.ErrorContext(ctx, "GetAgentAuthStatus: service error",
			slog.String("agent_id", req.GetAgentId()),
			slog.String("error", err.Error()),
		)
	...
	return &GetAgentAuthStatusResponse{
		AgentId:      ag.ID,
		HostId:       ag.HostID,
		Status:       ag.Status,
		Mode:         ag.Mode,
...
// RevokeAgentAuth revokes the agent, all its grants, and emits an audit event.
func (s *DaemonServer) RevokeAgentAuth(ctx context.Context, req *RevokeAgentAuthRequest) (*RevokeAgentAuthResponse, error) {
	if s.agentAuthService == nil {
		return nil, status_grpc.Error(codes.Unavailable, "agent auth service not configured")
	}
	if req.GetAgentId() == "" {
	...
	if err := s.agentAuthService.RevokeAgentAuth(ctx, req.GetAgentId(), tenantID, actorID); err != nil {
		s.logger.ErrorContext(ctx, "RevokeAgentAuth: service error",
			slog.String("agent_id", req.GetAgentId()),
			slog.String("error", err.Error()),
		)
	...
	return &RevokeAgentAuthResponse{}, nil
}
...
// ListAgentAuthAgents returns a paginated list of agents registered in a tenant.
func (s *DaemonServer) ListAgentAuthAgents(ctx context.Context, req *ListAgentAuthAgentsRequest) (*ListAgentAuthAgentsResponse, error) {
	if s.agentAuthService == nil {
		return nil, status_grpc.Error(codes.Unavailable, "agent auth service not configured")
	}
	...
	result, err := s.agentAuthService.ListAgentAuthAgents(
		ctx,
		tenantID,
		int(req.GetLimit()),
		int(req.GetOffset()),
	...
	if err != nil {
		s.logger.ErrorContext(ctx, "ListAgentAuthAgents: service error",
			slog.String("tenant_id", tenantID),
			slog.String("error", err.Error()),
		)
	...
	pbAgents := make([]*GetAgentAuthStatusResponse, 0, len(result.Agents))
	for _, ag := range result.Agents {
		var lastActiveUnix int64
		if ag.LastActiveAt != nil {
			lastActiveUnix = ag.LastActiveAt.Unix()
		}
		pbAgents = append(pbAgents, &GetAgentAuthStatusResponse{
			AgentId:      ag.ID,
			HostId:       ag.HostID,
			Status:       ag.Status,
			Mode:         ag.Mode,
	...
	return &ListAgentAuthAgentsResponse{
		Agents: pbAgents,
...
func (s *DaemonServer) CreateHostRegistrationToken(ctx context.Context, req *CreateHostRegistrationTokenRequest) (*CreateHostRegistrationTokenResponse, error) {
	...
	result, err := s.agentAuthService.CreateHostRegistrationToken(
		ctx,
		tenantID,
		req.GetName(),
		createdBy,
...
func (s *DaemonServer) ListComponentGrants(ctx context.Context, req *ListComponentGrantsRequest) (*ListComponentGrantsResponse, error) {
	...
	grants, err := s.agentAuthService.ListComponentGrants(ctx, tenantID)
	if err != nil {
...
func (s *DaemonServer) BatchGrantComponentAccessV2(ctx context.Context, req *BatchGrantComponentAccessV2Request) (*BatchGrantComponentAccessV2Response, error) {
	...
	changes := make([]agentauth.GrantChangeV2, 0, len(req.GetChanges()))
	for _, c := range req.GetChanges() {
		changes = append(changes, agentauth.GrantChangeV2{
			UserID:        c.GetUserId(),
	...
	applied, err := s.agentAuthService.BatchGrantComponentAccessV2(ctx, tenantID, actorID, changes)
	if err != nil {
...
func (s *DaemonServer) ListAuditLog(ctx context.Context, req *ListAuditLogRequest) (*ListAuditLogResponse, error) {
	...
	entries, total, err := s.agentAuthService.ListAuditLog(
		ctx,
		tenantID,
		req.GetActorId(),
		req.GetAction(),
