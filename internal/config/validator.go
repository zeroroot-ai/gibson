...
// ValidateKeycloakAdminConfig checks that the Keycloak admin credentials are
// sufficiently populated to operate the Organizations provisioner.
//
// This function MUST be called during daemon startup whenever authz.enabled=true.
// Returning a non-nil error means the daemon should refuse to start (fail-closed).
//
// Acceptance criteria:
//   - At least one of the two auth modes is fully configured:
//     a) Client-credentials: Endpoint + Realm + ClientID + ClientSecret all non-empty.
//     b) Password grant: Endpoint + Realm + Username + Password all non-empty.
//   - If neither mode is complete, returns a descriptive error listing the missing fields.
//
// When authz.enabled=false, this validation is skipped entirely (the provisioner
// is not invoked, so incomplete credentials are harmless).
func ValidateKeycloakAdminConfig(cfg KeycloakAdminConfig) error {
	missing := []string{}

	// Both modes require Endpoint and Realm.
	if cfg.Endpoint == "" {
		missing = append(missing, "keycloak.admin.endpoint (GIBSON_KEYCLOAK_ADMIN_ENDPOINT)")
	}
	if cfg.Realm == "" {
		missing = append(missing, "keycloak.admin.realm (GIBSON_KEYCLOAK_ADMIN_REALM)")
	}

	// Check whether at least one complete auth mode is present.
	hasClientCreds := cfg.ClientID != "" && cfg.ClientSecret != ""
	hasPasswordGrant := cfg.Username != "" && cfg.Password != ""

	if !hasClientCreds && !hasPasswordGrant {
		// Report specific missing fields to help operators fix the config.
		if cfg.ClientID == "" && cfg.ClientSecret == "" &&
			cfg.Username == "" && cfg.Password == "" {
			// Neither mode attempted.
			missing = append(missing,
				"one of: [keycloak.admin.client_id + keycloak.admin.client_secret] "+
					"or [keycloak.admin.username + keycloak.admin.password]")
		} else {
			// Partially configured — report the incomplete halves.
			if cfg.ClientID != "" && cfg.ClientSecret == "" {
				missing = append(missing, "keycloak.admin.client_secret (GIBSON_KEYCLOAK_ADMIN_CLIENT_SECRET)")
			}
			if cfg.ClientID == "" && cfg.ClientSecret != "" {
				missing = append(missing, "keycloak.admin.client_id (GIBSON_KEYCLOAK_ADMIN_CLIENT_ID)")
			}
			if cfg.Username != "" && cfg.Password == "" {
				missing = append(missing, "keycloak.admin.password (GIBSON_KEYCLOAK_ADMIN_PASSWORD)")
			}
			if cfg.Username == "" && cfg.Password != "" {
				missing = append(missing, "keycloak.admin.username (GIBSON_KEYCLOAK_ADMIN_USERNAME)")
			}
		}
	}
	...
	if len(missing) > 0 {
		return fmt.Errorf(
			"keycloak admin credentials incomplete — daemon cannot provision Organizations "+
				"(authz.enabled=true requires full credentials); missing: %s",
