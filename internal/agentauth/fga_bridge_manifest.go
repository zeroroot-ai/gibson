...
// but lives in agentauth so that package does not import manifest.
...
func (b *FGABridge) ResolveCrossComponentRules(ctx context.Context, subjectFGA string, components []ComponentRef) ([]CrossRule, error) {
	if subjectFGA == "" {
		return nil, fmt.Errorf("agentauth: ResolveCrossComponentRules: empty subject")
	}
	if len(components) == 0 {
		return nil, nil
	}
	...
	if strings.HasPrefix(subjectFGA, "agent_principal:") {
		...
		if err != nil {
			return nil, fmt.Errorf("agentauth: ListObjects(cannot_invoke): %w", err)
		}
		denyObjects = objs
	}
	...
	if err != nil {
		return nil, fmt.Errorf("agentauth: ListObjects(can_be_invoked_by): %w", err)
	}
...
func (b *FGABridge) ResolveAgentPrincipalIntersection(ctx context.Context, agentPrincipalID, ownerUserID, tenantID string) ([]ComponentRef, error) {
	if agentPrincipalID == "" {
		return nil, fmt.Errorf("agentauth: ResolveAgentPrincipalIntersection: empty agentPrincipalID")
	}
	if ownerUserID == "" {
		return nil, fmt.Errorf("agentauth: ResolveAgentPrincipalIntersection: empty ownerUserID")
	}

	ownerCaps, err := b.ResolveCapabilities(ctx, ownerUserID, tenantID)
	if err != nil {
		return nil, fmt.Errorf("agentauth: owner capabilities: %w", err)
	}
	...
	for _, rel := range relations {
		...
		if err != nil {
			return nil, fmt.Errorf("agentauth: agent ListObjects(%s): %w", rel.fgaRelation, err)
		}
