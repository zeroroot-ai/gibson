...
func TestNew_ActivationsInitialized(t *testing.T) {
	...
	// REGRESSION (GHSA-hmq9-jvvc-73w9): the daemon must NOT carry a mission
	// liveness map keyed by mission ID alone — that is a cross-tenant cancel
	// primitive. Mission liveness belongs to missionManager, which keys by
	// (tenant, missionID). The absence of the field is enforced at compile
	// time; TestDaemonHasNoMissionIDKeyedCancelMap asserts it reflectively so
	// a reintroduction fails a test rather than silently rearming the bug.
}

// TestDaemonHasNoMissionIDKeyedCancelMap is the regression guard for
// GHSA-hmq9-jvvc-73w9. A daemonImpl field of type map[string]context.CancelFunc
// can only be keyed by mission ID, which lets any tenant cancel any mission by
// naming its ID. Mission cancellation must go through missionManager, whose map
// is map[auth.TenantID]map[string]*activeMission.
func TestDaemonHasNoMissionIDKeyedCancelMap(t *testing.T) {
	t.Parallel()
	cancelMapType := reflect.TypeOf(map[string]context.CancelFunc(nil))
	dt := reflect.TypeOf(daemonImpl{})
	for i := range dt.NumField() {
		f := dt.Field(i)
		if f.Type == cancelMapType {
			t.Errorf("daemonImpl.%s is a map[string]context.CancelFunc — a mission-ID-keyed "+
				"cancel map is a cross-tenant cancel primitive (GHSA-hmq9-jvvc-73w9). "+
				"Route cancellation through missionManager, which keys by (tenant, missionID).", f.Name)
		}
	}
}
