...
func TestEventBus_PublishAndSubscribe(t *testing.T) {
	...
	// Publish an event
	event := testMissionEvent("mission.started", "test-mission-1", "")
...
func TestEventBus_FilterByEventType(t *testing.T) {
	...
	event1 := testMissionEvent("mission.started", "mission-1", "")
	err := bus.Publish(ctx, event1)
	require.NoError(t, err)

	// Publish a mission.completed event (should be filtered out)
	event2 := testMissionEvent("mission.completed", "mission-1", "")
...
func TestEventBus_FilterByMissionID(t *testing.T) {
	...
	event2 := testMissionEvent("mission.started", "mission-2", "")
	err = bus.Publish(ctx, event2)
	require.NoError(t, err)
...
func TestEventBus_Close(t *testing.T) {
	...
	event := testMissionEvent("mission.started", "test", "")
	err = bus.Publish(ctx, event)
	assert.Error(t, err)
...
func TestEventBus_CombinedFilters(t *testing.T) {
	...
	event3 := testMissionEvent("mission.started", "mission-2", "")
...
func TestEventBus_HelperFunctions(t *testing.T) {
	// The five mission/node constructors this test used to cover were dead
	// outside tests and are deleted (gibson#1112); what remains covers the
	// constructors production code still calls.
