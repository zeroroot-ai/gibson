...
//	MissionPauseRequested→ mission.paused (status: paused)
//	MissionResumed       → mission.resumed (status: running)
...
func ProjectBrainEvent(ev brain.Event, missionID string) *api.EventData {
	...
	case brain.MissionPauseRequested:
		// Operator pause (gibson#851). The old path emitted this from
		// mission_manager's pause handler onto the per-run channel; the
		// projector derives it from the same brain event that sets
		// status=paused in the World, so the bus and Redis stream keep the
		// wire type without a second emit path (gibson#1112 PR 3).
		return missionEventData(now, "mission.paused", e.ID, "", "paused", "")

	case brain.MissionResumed:
		return missionEventData(now, "mission.resumed", e.ID, "", "running", "")
...
// eng.Work()/Missions()/Findings(), which take the read lock, block the tick
// goroutine against itself, forever. This tap did exactly that on
// WorkCompleted: the tenant's engine stopped ticking the first time any work
// item finished, and from then on every mission in that tenant bootstrapped and
// hung with no status, no failure, no log (gibson#1206).
//
// The World itself is safe to read here — that is what Systems do under the same
// lock — so resolve through p.eng.World, never through the accessor.
func (p *lifecycleProjectorTap) apply(ev brain.Event) {
	...
	// the World the tick has already folded this event into.
	if wc, ok := ev.(brain.WorkCompleted); ok {
		for _, ws := range p.eng.World.WorkSnapshot() {
			if ws.ID == wc.ID {
