...
// HSet hands the field map straight to go-redis, which flattens a
// map[string]string into field/value pairs itself (see appendArgs in
// go-redis/v9 commands.go). Building the interleaved []interface{} by hand
// added nothing but an allocation whose capacity expression, len(fields)*2,
// read as a potential integer overflow to static analysis (gibson#1444).
