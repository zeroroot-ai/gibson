...
type daemonImpl struct {
	...
	// NOTE: there is deliberately no daemon-level `activeMissions` map.
	// Mission liveness is owned solely by missionManager, which keys its
	// map by (tenant, missionID) so Stop/Pause/Resume cannot cross a
	// tenant boundary. A daemon-level map keyed by mission ID alone is a
	// cross-tenant cancel primitive — do not reintroduce one.
	...
	// connectorTokenStatus records connector token refresh outcomes so the
	...
	// on config reload can rebuild providers.
	...
	// graphragQuerier is the single knowledge-graph querier shared by
	// ComponentService and the agent harness. Built lazily by graphRAGQuerier();
	// nil when the daemon cannot serve graph reads.
	graphragQuerier component.GraphRAGQuerier
	...
	// spiffeJWTSource is the SPIFFE Workload API JWT source used to VERIFY plugin
	// JWT-SVIDs at capability-grant enrollment (ADR-0066). It provides the SPIRE
	// JWT trust bundle to jwtsvid.ParseAndValidate. Nil when SPIFFE or the
	// install-tenant binding is not configured, in which case the register
...
func New(cfg *config.Config, opts ...Option) (Daemon, error) {
	...
	// or `git clone` then `go build` cannot see each other.
	//
	// Wired unconditionally. NewSetecSessionClient returns (nil, nil) when the
	// sandbox subsystem is off or gibson was built without the setec tag, and a
	// registry over a nil client answers ErrSessionUnavailable — so DevboxExec
	// reports Unavailable naming the reason instead of the daemon carrying a
	...
	if cfg.Sandbox.Devbox.Image == "" {
		...
		slogLogger.Info("DevboxExec disabled: sandbox.devbox.image is not configured")
	}
	// The callback listener binds every request to the task grant it carries
	// (gibson#1605). The verifier is read per request: the Minter is built
	...
	callbackOpts = append(callbackOpts,
		...
		harness.WithSessionSandboxes(
			sandboxed.NewSessionRegistry(sessionClient, sandboxed.SessionSpec{
				...
				VCPU:          cfg.Sandbox.Devbox.VCPU,
				Memory:        cfg.Sandbox.Devbox.Memory,
				SandboxClass:  cfg.Sandbox.Devbox.SandboxClass,
				WorkspaceSize: cfg.Sandbox.Devbox.WorkspaceSize,
				Idle:          cfg.Sandbox.Devbox.Idle,
			})),
	)
...
// buildPluginSVIDEnroller assembles the SPIFFE-SVID plugin enroller (ADR-0066)
// for the pre-auth register endpoint, or returns a nil interface when SVID
// enrollment is not configured — in which case the register endpoint keeps only
...
func (d *daemonImpl) buildPluginSVIDEnroller(ctx context.Context) pluginEnroller {
	...
	if d.spiffeJWTSource == nil {
		src, srcErr := workloadapi.NewJWTSource(ctx, workloadapi.WithClientOptions(workloadapi.WithAddr(binding.socketAddr)))
		if srcErr != nil {
			d.logger.Warn(ctx, "SPIFFE JWT source unavailable; SVID plugin enrollment disabled", "error", srcErr)
			return nil
		}
		d.spiffeJWTSource = src
	}
...
func (d *daemonImpl) Start(ctx context.Context) error {
	...
	d.graphWriter = newNeo4jGraphWriter(func() datapool.Pool { return d.pool })
	...
	//
	// The adapter carries no default tenant. It used to be constructed with one
	// — "default" when Registry.Namespace was unset — and every discovery query
	// that arrived without a tenant in its context fell into that shared
	// namespace. Discovery now takes its tenant from the caller's context and is
	// refused without one.
	...
	regAdapter := component.NewRegistryAdapter(compRegistry)
	d.registryAdapter = regAdapter
	d.logger.Info(ctx, "initialized Redis-backed component registry adapter")
	...
	if d.brainRegistry != nil {
		...
		d.brainExecutor.jobs = func() (job.Store, error) {
			...
			return job.NewPostgresStore(d.pool), nil
		}
	...
	if d.config.Security.KeyProvider != nil {
		...
		if err != nil {
			...
			if minter, mErr := capabilitygrant.NewMinter(ctx, capabilitygrant.Config{
				...
				// The dedicated CG signing key: its own secret on its own path,
				// rotatable without touching the master KEK (GHSA-3957-8wcf-929q).
				SigningKeyDir: cgSigningKeyDir(),
				// Migration fallback only, for a daemon whose chart has not yet
				...
				d.cgMinter = minter
				if minter.DerivedFromMasterKEK() {
					d.logger.Warn(ctx,
						"CG signing key is derived from the master KEK; it cannot be rotated independently and "+
							"anyone holding the KEK can mint capability grants. Provision a dedicated signing key "+
							"and point GIBSON_CGJWT_SIGNING_KEY_DIR at its mount.",
						"signing_key_dir", cgSigningKeyDir(),
						"key_id", minter.KeyID())
				} else {
	...
	if d.brainRegistry != nil {
		...
		// from the mission record) and can never name a raw brain id.
		minter, mErr := newHandleMinter()
		if mErr != nil {
			// Fail loud: without a handle key the WorldView read cannot mint
			// non-constructible references, and leaving it unwired silently
			// would disable the agent's only World read.
			return fmt.Errorf("initialize world-view handle minter: %w", mErr)
		}
		d.callback.SetWorldViewSource(worldViewSource(d.brainRegistry, minter))
		d.logger.Info(ctx, "wired callback WorldView RPC to the ECS brain")
	...
	// tool that reports discovered entities in proto field 100 has them folded
	// into the tenant's World, and the graph projector — the sole writer —
	// materializes them. There is no GraphLoader any more: the package that used
	// to sit here built Cypher out of the payload's own labels and property
	// names, and was imported by seven files while constructed by none.
	d.callback.SetDiscoveryProcessor(&discoveryProcessorAdapter{inner: d.newDiscoveryProcessor()})
	d.logger.Info(ctx, "wired callback DiscoveryResult ingest to the ECS brain")
	...
	// Wire the session-context store (gibson#1184): components checkpoint
	// session-local context to the per-tenant dataplane Postgres via
	// Put/Get/DeleteSessionContext. The pool getter is lazy — the data-plane
	// pool may finish initializing after this wiring runs, and an
	// unavailable pool is a per-call error rather than a startup failure
	// (same posture as the other pool-backed callback dependencies).
	d.callback.SetSessionContextStore(newPoolSessionContextStore(
		func() datapool.Pool { return d.pool },
		d.logger.WithComponent("session-context-store").Slog(),
	))
	...
	// SPIFFE-SVID plugin enrollment (ADR-0066): built synchronously here so the
	// JWT source opens before the pre-auth listener serves; nil (bootstrap-only)
	// when not configured.
	svidEnroller := d.buildPluginSVIDEnroller(ctx)
	go func() {
		if err := newNativeLoginSubsystem(nativeLoginConfigFromEnv(), d.logger, d.cgMinter, d.capabilityGrantSvc, svidEnroller, d.connectorAuthSrv).Serve(ctx); err != nil {
	...
	//
	// The same listener serves the Capability-Grant per-kid key route
	// (deploy#1187), so ext-authz gets the policy and the verification keys
	...
	if d.authorizer != nil {
		// Seed the connector platform catalog gate (ADR-0067): every embedded
		// catalog entry gets its platform_enabled tuple, so ConnectorService's
		// gate checks pass for listed entries and fail for anything else.
		// Startup converge, add-only, best-effort.
		catalogGate := newComponentCatalogGateState(supplychain.DockerConfigPath(registryAuthDir()))
		if err := seedComponentCatalogGate(ctx, d.authorizer, componentImageVerifier(), d.logger.Slog()); err != nil {
			catalogGate.recordRefusal(err)
			d.logger.Warn(ctx, "connector catalog gate seed failed (non-fatal)", "error", err)
		}
	...
	if d.connectorTokenReconciler != nil {
		go d.connectorTokenReconciler.Run(ctx)
...
func (d *daemonImpl) stopServices(ctx context.Context) {
	// Running missions are cancelled by missionManager, which owns the
	// only (tenant, missionID)-keyed liveness map.
	...
	// Close the SPIFFE JWT source (SVID plugin enrollment, ADR-0066).
	if d.spiffeJWTSource != nil {
		d.logger.Info(ctx, "closing SPIFFE JWTSource")
		if err := d.spiffeJWTSource.Close(); err != nil {
			d.logger.Warn(ctx, "error closing SPIFFE JWTSource", "error", err)
		}
		d.spiffeJWTSource = nil
	}
