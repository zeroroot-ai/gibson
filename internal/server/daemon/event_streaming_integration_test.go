...
import (
	...
	daemonapi "github.com/zeroroot-ai/gibson/internal/server/daemon/api"
)
...
// missionEvent builds the EventData shape these streaming tests publish.
// The exported daemon constructors they used were dead outside tests
// (gibson#1112) and are deleted; the tests exercise PublishEvent routing,
// which cares only about EventType and the MissionEvent payload.
func missionEvent(eventType, missionID, nodeID string) daemonapi.EventData {
	return daemonapi.EventData{
		EventType: eventType,
		Timestamp: time.Now(),
		Source:    "daemon",
		MissionEvent: &daemonapi.MissionEventData{
			EventType: eventType,
			Timestamp: time.Now(),
			MissionID: missionID,
			NodeID:    nodeID,
			// The deleted constructors carried a human message; consumers
			// (and the streaming assertions) treat it as always-present.
...
func TestSubscribeNotImplemented(t *testing.T) {
	...
	event := missionEvent("mission.started", "mission-subscribe-test", "")
...
func TestEventStreamingAllTypes(t *testing.T) {
	...
	want := []struct {
		...
		{"mission.started", func() error {
			return res.PublishEvent(ctx, tenant, missionEvent("mission.started", "m1", ""))
		}},
		{"mission.completed", func() error {
			return res.PublishEvent(ctx, tenant, missionEvent("mission.completed", "m1", ""))
		}},
...
func TestEventStreamingFiltering(t *testing.T) {
	...
	t.Run("event_type_filter", func(t *testing.T) {
		...
		require.NoError(t, res.PublishEvent(ctx, tenant, missionEvent("mission.started", "m-f1", "")))
		require.NoError(t, res.PublishEvent(ctx, tenant, missionEvent("mission.completed", "m-f1", "")))
		require.NoError(t, res.PublishEvent(ctx, tenant, missionEvent("node.started", "m-f1", "n1")))
		require.NoError(t, res.PublishEvent(ctx, tenant, missionEvent("mission.started", "m-f2", "")))
	...
	t.Run("mission_id_filter", func(t *testing.T) {
		...
		require.NoError(t, res.PublishEvent(ctx, tenant, missionEvent("mission.started", "filter-m1", "")))
		require.NoError(t, res.PublishEvent(ctx, tenant, missionEvent("mission.started", "filter-m2", "")))
		require.NoError(t, res.PublishEvent(ctx, tenant, missionEvent("mission.completed", "filter-m1", "")))
...
func TestEventStreamingMissionEvents(t *testing.T) {
	...
	sequence := []struct {
		...
		{func() error { return res.PublishEvent(ctx, tenant, missionEvent("mission.started", missionID, "")) }, "mission.started"},
		{func() error { return res.PublishEvent(ctx, tenant, missionEvent("node.started", missionID, "node-1")) }, "node.started"},
		{func() error {
			return res.PublishEvent(ctx, tenant, missionEvent("node.completed", missionID, "node-1"))
		}, "node.completed"},
		{func() error { return res.PublishEvent(ctx, tenant, missionEvent("mission.completed", missionID, "")) }, "mission.completed"},
	}
...
func TestEventStreamingMultipleSubscribers(t *testing.T) {
	...
	require.NoError(t, res.PublishEvent(ctx, tenant,
		missionEvent("mission.started", "shared-mission", "")))
...
func TestEventStreamingBackpressure(t *testing.T) {
	...
	for i := 0; i < eventCount; i++ {
		require.NoError(t, res.PublishEvent(ctx, tenant,
			missionEvent("mission.started", fmt.Sprintf("bp-mission-%d", i), "")))
	}
...
func TestEventStreamingReconnection(t *testing.T) {
	...
	require.NoError(t, res.PublishEvent(ctx1, tenant,
		missionEvent("mission.started", "pre-disconnect", "")))
	select {
	case e, ok := <-ch1:
		require.True(t, ok)
	...
	require.NoError(t, res.PublishEvent(ctx2, tenant,
		missionEvent("mission.started", "post-reconnect", "")))
	select {
	case e, ok := <-ch2:
		require.True(t, ok)
		assert.Equal(t, "mission.started", e.EventType)
...
// context stops delivery and closes the channel, without leaking the
// subscriber goroutine.
//
// This used to sample runtime.NumGoroutine() before and after and assert on
// the delta. That counter is process-global — anything else alive in the
// same test binary (a go-redis pool reaper, a testcontainers health poller,
// an OTel exporter tick) moves it, so the assertion flaked under a busy
// merge-queue runner even though the subscriber itself behaved correctly
// (gibson#1385). The invariants below are asserted directly instead:
// publish-then-receive proves the subscriber goroutine is actually running
// (not just that SubscribeStream returned without error), and the channel
// close proves it exited — see SubscribeStream in event_stream_redis.go,
// where `defer close(out)` is the goroutine's terminal statement, so
// observing the close is observing the exit.
func TestEventStreamingCancellation(t *testing.T) {
	...
	// Prove the subscriber goroutine is actually running by round-tripping
	// an event through it before cancelling.
	require.NoError(t, res.PublishEvent(ctx, tenant,
		missionEvent("mission.started", "cancel-precheck", "")))
	select {
	case e, ok := <-ch:
		require.True(t, ok, "channel closed before cancellation")
		assert.Equal(t, "mission.started", e.EventType)
	case <-time.After(2 * time.Second):
		t.Fatal("subscriber did not deliver event before cancellation")
	}
...
func TestEventTimestamps(t *testing.T) {
	...
	before := time.Now()
	require.NoError(t, res.PublishEvent(ctx, tenant, missionEvent("mission.started", "ts-mission", "")))
	after := time.Now()
	...
	require.NoError(t, res.PublishEvent(ctx, tenant, missionEvent("mission.completed", "ts-mission", "")))
	select {
	case e, ok := <-ch:
		require.True(t, ok)
