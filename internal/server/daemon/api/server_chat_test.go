...
type mockConversationStore struct {
	...
	// gotCallerUserID records the callerUserID most recently passed to
	// Get/Rename/Delete, so handler tests can assert the caller's own
	// identity (never a body field) reaches the ownership check.
	gotCallerUserID string
}
...
func (m *mockConversationStore) Get(_ context.Context, _, callerUserID, _ string) (*storedConversation, []storedMessage, error) {
	m.gotCallerUserID = callerUserID
	if m.getErr != nil {
...
func (m *mockConversationStore) Rename(_ context.Context, _, callerUserID, _, newTitle string) error {
	m.gotCallerUserID = callerUserID
	if m.renameErr != nil {
		return m.renameErr
	}
...
func (m *mockConversationStore) Delete(_ context.Context, _, callerUserID, conversationID string) error {
	m.gotCallerUserID = callerUserID
	if m.deleteErr != nil {
		return m.deleteErr
	}
...
func TestListConversations_EmptyTenantID_NilStore_Internal(t *testing.T) {
	// Empty TenantId: auth.TenantStringFromContext returns "" (no SystemTenant
	// fallback — see sdk auth.TenantFromContext's doc comment).
	...
	srv := blankServer()
	_, err := srv.ListConversations(context.Background(), &tenantv1.ListConversationsRequest{})
...
func TestListConversations_MissingUserID_InvalidArgument(t *testing.T) {
	srv := blankServer()
	_, err := srv.ListConversations(tenantCtxNoSubject("acme"), &tenantv1.ListConversationsRequest{})
	assert.Equal(t, codes.InvalidArgument, grpcCode(err))
}
...
func TestListConversations_NilStore_Internal(t *testing.T) {
	...
	srv := blankServer()
	_, err := srv.ListConversations(tenantAndSubjectCtx("acme", "u1"), &tenantv1.ListConversationsRequest{})
...
func TestListConversations_Success_ReturnsMapped(t *testing.T) {
	...
	resp, err := srv.ListConversations(tenantAndSubjectCtx("acme", "u1"), &tenantv1.ListConversationsRequest{})
...
func TestGetConversation_MissingConversationID_InvalidArgument(t *testing.T) {
	srv := blankServer()
	_, err := srv.GetConversation(tenantAndSubjectCtx("acme", "u1"), &tenantv1.GetConversationRequest{
		ConversationId: "",
	})
	assert.Equal(t, codes.InvalidArgument, grpcCode(err))
}
...
func TestGetConversation_Success_ReturnsMappedMessages(t *testing.T) {
	...
	resp, err := srv.GetConversation(tenantAndSubjectCtx("acme", "u1"), &tenantv1.GetConversationRequest{
		ConversationId: "c1",
	})
	require.NoError(t, err)
...
func TestSaveConversation_MissingConversationID_InvalidArgument(t *testing.T) {
	srv := blankServer()
	srv.conversationStore = &mockConversationStore{}
	_, err := srv.SaveConversation(tenantAndSubjectCtx("acme", "u1"), &tenantv1.SaveConversationRequest{
		Title: "My Chat",
	})
	assert.Equal(t, codes.InvalidArgument, grpcCode(err))
}

func TestSaveConversation_MissingTenantInContext_InvalidArgument(t *testing.T) {
	// No tenant on the context: the request body cannot supply one.
	srv := blankServer()
	srv.conversationStore = &mockConversationStore{}
	_, err := srv.SaveConversation(context.Background(), &tenantv1.SaveConversationRequest{
		ConversationId: "conv-1",
		TenantId:       "acme", // ignored — must not satisfy the tenant requirement
		Title:          "My Chat",
	})
	assert.Equal(t, codes.InvalidArgument, grpcCode(err))
}

func TestSaveConversation_MissingCallerSubject_InvalidArgument(t *testing.T) {
	// A tenant but no caller subject: the request body cannot supply one.
	srv := blankServer()
	srv.conversationStore = &mockConversationStore{}
	_, err := srv.SaveConversation(tenantCtxNoSubject("acme"), &tenantv1.SaveConversationRequest{
		ConversationId: "conv-1",
		UserId:         "u1", // ignored — must not satisfy the user requirement
		Title:          "My Chat",
	})
	assert.Equal(t, codes.InvalidArgument, grpcCode(err))
}
...
// TestSaveConversation_OwnedByAnotherUser_PermissionDenied pins the handler
// half of the cross-user isolation fix: when the store reports that
// conversation_id already belongs to a different user, the RPC must surface
// PermissionDenied — not silently succeed, and not the generic Internal used
// for other store failures.
func TestSaveConversation_OwnedByAnotherUser_PermissionDenied(t *testing.T) {
	srv := blankServer()
	srv.conversationStore = &mockConversationStore{saveErr: errors.New(errConversationOwnedByAnotherUser)}
	_, err := srv.SaveConversation(tenantAndSubjectCtx("acme", "attacker"), &tenantv1.SaveConversationRequest{
...
func TestSaveConversation_Success_PersistsConversation(t *testing.T) {
	...
	resp, err := srv.SaveConversation(tenantAndSubjectCtx("acme", "u1"), req)
	require.NoError(t, err)
...
func TestSaveConversation_CallerIdentityOverridesRequestUserID(t *testing.T) {
	...
	ctx := tenantAndSubjectCtx("acme", "caller-subject")

	req := &tenantv1.SaveConversationRequest{
		UserId:         "different-user", // must be ignored
		ConversationId: "conv-1",
		Title:          "My Chat",
	}
...
func TestSaveConversation_TenantFromContext_IgnoresRequestTenant(t *testing.T) {
	// The tenant is always resolved from the auth context; a tenant_id on the
	// request must not change the namespace that is written.
	srv := blankServer()
	mock := &mockConversationStore{}
	srv.conversationStore = mock

	ctx := tenantAndSubjectCtx("context-tenant", "u1")

	req := &tenantv1.SaveConversationRequest{
		TenantId:       "other-tenant", // must be ignored
		ConversationId: "conv-1",
		Title:          "My Chat",
	}
...
func TestConversationStore_SaveAndGet(t *testing.T) {
	...
	conv, gotMsgs, err := store.Get(ctx, "tenant-B", "user-2", "conv-2")
	require.NoError(t, err)
...
func TestConversationStore_RoundTrip_SaveListGet(t *testing.T) {
	...
	conv, gotMsgs, err := store.Get(ctx, "tenant-RT", "user-RT", "conv-RT")
	require.NoError(t, err)
...
func TestConversationStore_CrossTenantIsolation(t *testing.T) {
	...
	// Get for tenant-B must return not-found (same caller user-1, different
	// tenant namespace — the hash key itself does not exist there).
	_, _, err = store.Get(ctx, "tenant-B", "user-1", "conv-tenant-a")
...
func (s *inMemConvStore) Save(
	...
	// owned by a different user — mirrors redisConversationStore.Save.
	createdAt := strconv.FormatInt(now, 10)
	if existing, ok := s.hashes[hashKey]; ok {
		if v, exists := existing["created_at"]; exists {
			createdAt = v
		}
		if existingUserID, exists := existing["user_id"]; exists && existingUserID != "" && existingUserID != userID {
...
func (s *inMemConvStore) Rename(_ context.Context, tenantID, callerUserID, conversationID, newTitle string) error {
	if tenantID == "" || callerUserID == "" || conversationID == "" {
		return errors.New("tenant_id, user_id, and conversation_id are required")
	}
	hashKey := convHashKey(tenantID, conversationID)
	h := s.hashes[hashKey]
	if h == nil || h["user_id"] != callerUserID {
...
func (s *inMemConvStore) Delete(_ context.Context, tenantID, callerUserID, conversationID string) error {
	if tenantID == "" || callerUserID == "" || conversationID == "" {
		return errors.New("tenant_id, user_id, and conversation_id are required")
	}
	hashKey := convHashKey(tenantID, conversationID)
	...
	if h, ok := s.hashes[hashKey]; ok && h["user_id"] != callerUserID {
		return errors.New(errConversationNotFound)
	}
...
func TestPartsRoundTrip_OrderingPreserved(t *testing.T) {
	...
	_, gotMsgs, err := store.Get(ctx, "t1", "u1", "c1")
	require.NoError(t, err)
...
func TestPartsRoundTrip_StoreRoundTrip_AllPartTypes(t *testing.T) {
	...
	_, gotMsgs, err := store.Get(ctx, "tenant-X", "user-X", "conv-X")
	require.NoError(t, err)
	require.Len(t, gotMsgs, 1)
...
func TestRenameConversation_MissingTitle_InvalidArgument(t *testing.T) {
	srv := blankServer()
	srv.conversationStore = &mockConversationStore{}
	_, err := srv.RenameConversation(tenantAndSubjectCtx("acme", "u1"), &tenantv1.RenameConversationRequest{
		ConversationId: "c1",
	})
	assert.Equal(t, codes.InvalidArgument, grpcCode(err))
}

func TestRenameConversation_MissingTenantInContext_InvalidArgument(t *testing.T) {
	srv := blankServer()
	srv.conversationStore = &mockConversationStore{}
	_, err := srv.RenameConversation(context.Background(), &tenantv1.RenameConversationRequest{
		TenantId:       "acme", // ignored — must not satisfy the tenant requirement
...
func TestDeleteConversation_MissingConversationID_InvalidArgument(t *testing.T) {
	srv := blankServer()
	srv.conversationStore = &mockConversationStore{}
	ctx := tenantAndSubjectCtx("acme", "u1")
	_, err := srv.DeleteConversation(ctx, &tenantv1.DeleteConversationRequest{})
	assert.Equal(t, codes.InvalidArgument, grpcCode(err))
}

func TestDeleteConversation_MissingTenantInContext_InvalidArgument(t *testing.T) {
	srv := blankServer()
	srv.conversationStore = &mockConversationStore{}
...
func TestDeleteConversation_NoCallerIdentity_InvalidArgument(t *testing.T) {
	srv := blankServer()
	srv.conversationStore = &mockConversationStore{}
	_, err := srv.DeleteConversation(tenantCtxNoSubject("acme"), &tenantv1.DeleteConversationRequest{
		ConversationId: "c1",
	})
	assert.Equal(t, codes.InvalidArgument, grpcCode(err))
}
...
// TestDeleteConversation_OwnedByAnotherUser_NotFound pins the handler half of
// the cross-user isolation fix: when the store reports the conversation
// belongs to a different user (same sentinel message as true absence), the
// RPC must surface NotFound — never silently succeed, and never leak that
// the conversation exists under someone else.
func TestDeleteConversation_OwnedByAnotherUser_NotFound(t *testing.T) {
	srv := blankServer()
	srv.conversationStore = &mockConversationStore{deleteErr: errors.New(errConversationNotFound)}
	ctx := tenantAndSubjectCtx("acme", "attacker")
	_, err := srv.DeleteConversation(ctx, &tenantv1.DeleteConversationRequest{
		ConversationId: "victim-conv",
	})
	assert.Equal(t, codes.NotFound, grpcCode(err))
...
func TestConversationStore_Rename_UpdatesTitleDurably(t *testing.T) {
	...
	err = store.Rename(ctx, "tenant-R", "user-R", "conv-R", "Renamed Title")
	require.NoError(t, err)

	// Get reflects the rename.
	conv, _, err := store.Get(ctx, "tenant-R", "user-R", "conv-R")
	require.NoError(t, err)
...
func TestConversationStore_Rename_NonExistent_Error(t *testing.T) {
	...
	err := store.Rename(ctx, "tenant-R", "user-R", "no-such-conv", "Title")
	require.Error(t, err)
...
func TestConversationStore_Delete_RemovesFromListAndGet(t *testing.T) {
	...
	_, _, err = store.Get(ctx, "tenant-D", "user-D", "conv-D")
	require.Error(t, err)
...
func TestConversationStore_Delete_TenantUserIsolation(t *testing.T) {
	...
	convOther, _, err := store.Get(ctx, "other", "u-A", "conv-other")
	require.NoError(t, err)
...
func TestConversationStore_Rename_TenantIsolation(t *testing.T) {
	...
	err := store.Rename(ctx, "tenant-B", "u", "conv-1", "Hijacked")
	require.Error(t, err)
	assert.Contains(t, err.Error(), "not found")

	// The original title in tenant-A must be unchanged.
	conv, _, err := store.Get(ctx, "tenant-A", "u", "conv-1")
	require.NoError(t, err)
	assert.Equal(t, "Tenant A Chat", conv.Title, "title must not have changed")
}

// TestConversationStore_Save_RefusesCrossUserOverwrite verifies that Save
// will not silently overwrite a conversation that already belongs to a
// different user, even though the hash key is addressable by (tenant,
// conversation_id) alone. This is the store-level half of the
// cross-user-conversation-isolation fix.
func TestConversationStore_Save_RefusesCrossUserOverwrite(t *testing.T) {
	store := newInMemConvStore()
	ctx := context.Background()

	require.NoError(t, store.Save(ctx, "shared", "victim", "conv-shared", "Victim's chat", "", nil))

	err := store.Save(ctx, "shared", "attacker", "conv-shared", "Hijacked title", "", nil)
	require.Error(t, err, "Save must refuse to overwrite another user's conversation")
	assert.Contains(t, err.Error(), "different user")

	// The victim's conversation must be untouched.
	conv, _, err := store.Get(ctx, "shared", "victim", "conv-shared")
	require.NoError(t, err)
	assert.Equal(t, "Victim's chat", conv.Title)
}

// TestConversationStore_Get_RefusesCrossUserRead verifies that Get denies
// access to a conversation that exists (in the caller's own tenant) but
// belongs to a different user, and that the error is indistinguishable from
// true absence.
func TestConversationStore_Get_RefusesCrossUserRead(t *testing.T) {
	store := newInMemConvStore()
	ctx := context.Background()

	require.NoError(t, store.Save(ctx, "shared", "victim", "conv-shared", "Victim's chat", "", nil))

	_, _, err := store.Get(ctx, "shared", "attacker", "conv-shared")
	require.Error(t, err, "Get must refuse to return another user's conversation")
	assert.Contains(t, err.Error(), "not found", "the error must not distinguish absence from wrong ownership")
}

// TestConversationStore_Rename_RefusesCrossUserRename verifies that Rename
// denies mutating a conversation that belongs to a different user.
func TestConversationStore_Rename_RefusesCrossUserRename(t *testing.T) {
	store := newInMemConvStore()
	ctx := context.Background()

	require.NoError(t, store.Save(ctx, "shared", "victim", "conv-shared", "Original", "", nil))

	err := store.Rename(ctx, "shared", "attacker", "conv-shared", "Hijacked")
	require.Error(t, err, "Rename must refuse to mutate another user's conversation")

	conv, _, err := store.Get(ctx, "shared", "victim", "conv-shared")
	require.NoError(t, err)
	assert.Equal(t, "Original", conv.Title, "title must not have changed")
}

// TestConversationStore_Delete_RefusesCrossUserDelete verifies that Delete
// leaves a conversation belonging to a different user intact.
func TestConversationStore_Delete_RefusesCrossUserDelete(t *testing.T) {
	store := newInMemConvStore()
	ctx := context.Background()

	require.NoError(t, store.Save(ctx, "shared", "victim", "conv-shared", "Victim's chat", "", nil))

	err := store.Delete(ctx, "shared", "attacker", "conv-shared")
	require.Error(t, err, "Delete must refuse to destroy another user's conversation")

	conv, _, err := store.Get(ctx, "shared", "victim", "conv-shared")
	require.NoError(t, err, "the victim's conversation must survive the attacker's Delete call")
	assert.Equal(t, "Victim's chat", conv.Title)
}

// ---------------------------------------------------------------------------
// redisConversationStore — absent-owner-field regression tests (miniredis)
//
// Every test above exercises inMemConvStore, a Go-map fake whose zero-value
// semantics (`h["user_id"]` reads "" for a missing map key) never reproduced
// the real bug: go-redis's HGet returns the SAME redis.Nil both when a hash
// key does not exist at all AND when a single field is absent from an
// existing hash. Save/Get/Rename/Delete on the real redisConversationStore
// must treat "hash exists but user_id is absent" as "not mine" (deny),
// never as "no owner recorded, safe to touch" (allow) — exercised here
// against a real (miniredis) Redis client, writing the corrupted hash shape
// directly with HSet so the ambiguity is genuine, not simulated by a fake
// that never had it.
// ---------------------------------------------------------------------------

// newRedisConvStoreTest returns a redisConversationStore backed by an
// in-process miniredis instance, plus the raw client for arranging
// corrupted/partial hash state that Save/Rename/Delete would never write
// themselves.
func newRedisConvStoreTest(t *testing.T) (*redisConversationStore, goredis.UniversalClient) {
	t.Helper()
	mr := miniredis.RunT(t)
	client := goredis.NewClient(&goredis.Options{Addr: mr.Addr()})
	t.Cleanup(func() { _ = client.Close() })
	return &redisConversationStore{client: client, logger: testSlogLogger}, client
}

// TestRedisConversationStore_Save_ReadErrorDoesNotSkipOwnershipCheck pins the
// remaining fail-open in the Save path: the ownership block was guarded by
// `err == nil && len(existing) == 2`, so ANY read failure skipped the check
// and fell through to the write. Get and Delete both return on that error.
// A wrong-type key makes HMGet fail deterministically without a fault
// injector, which is the same class of failure a failover window produces.
func TestRedisConversationStore_Save_ReadErrorDoesNotSkipOwnershipCheck(t *testing.T) {
	store, client := newRedisConvStoreTest(t)
	ctx := context.Background()
	hashKey := convHashKey("tenant-x", "conv-wrongtype")

	// A string at the conversation's key: HMGet returns WRONGTYPE, not nil.
	require.NoError(t, client.Set(ctx, hashKey, "not-a-hash", 0).Err())

	err := store.Save(ctx, "tenant-x", "someone", "conv-wrongtype", "title", "model-x", nil)
	require.Error(t, err, "Save must refuse when the ownership check could not be read")
	assert.Contains(t, err.Error(), "ownership check",
		"the error must name the failed check, not a downstream write failure")

	// The pre-existing value must be untouched — a failed check must never
	// have progressed to the write.
	got, getErr := client.Get(ctx, hashKey).Result()
	require.NoError(t, getErr)
	assert.Equal(t, "not-a-hash", got, "Save must not write when ownership is unverified")
}

// TestRedisConversationStore_Delete_AbsentOwnerField_Denies pins the fix:
// before it, `err == nil && existingUserID != callerUserID` treated the
// goredis.Nil from a missing user_id field as "ownership check does not
// apply" and fell through to an unconditional delete.
func TestRedisConversationStore_Delete_AbsentOwnerField_Denies(t *testing.T) {
	store, client := newRedisConvStoreTest(t)
	ctx := context.Background()
	hashKey := convHashKey("tenant-x", "conv-corrupt")

	// A hash that exists (has content) but is missing user_id — e.g. a
	// partial write. HGet on the missing field returns redis.Nil, identical
	// to a wholly-absent key; Delete must not treat that as "safe to delete."
	require.NoError(t, client.HSet(ctx, hashKey, "title", "orphaned", "created_at", "1").Err())

	err := store.Delete(ctx, "tenant-x", "someone", "conv-corrupt")
	require.Error(t, err, "Delete must refuse a hash that exists but has no recorded owner")
	assert.Contains(t, err.Error(), "not found")

	exists, existsErr := client.Exists(ctx, hashKey).Result()
	require.NoError(t, existsErr)
	assert.Equal(t, int64(1), exists, "the hash must survive when ownership could not be confirmed")
}

// TestRedisConversationStore_Delete_TrulyAbsent_StillIdempotent proves the
// absent-owner-field fix did not regress Delete's documented idempotency
// contract: a conversation that was never created at all must still delete
// as a no-op, not an error.
func TestRedisConversationStore_Delete_TrulyAbsent_StillIdempotent(t *testing.T) {
	store, _ := newRedisConvStoreTest(t)
	ctx := context.Background()

	err := store.Delete(ctx, "tenant-x", "someone", "never-existed")
	require.NoError(t, err, "Delete of a truly non-existent conversation must remain idempotent")
}

// TestRedisConversationStore_Save_AbsentOwnerField_Denies is Save's half of
// the same class of bug: a type assertion on a nil HMGet slot silently
// evaluated ok=false, skipping the ownership check and letting Save adopt or
// overwrite a hash with no recorded owner.
func TestRedisConversationStore_Save_AbsentOwnerField_Denies(t *testing.T) {
	store, client := newRedisConvStoreTest(t)
	ctx := context.Background()
	hashKey := convHashKey("tenant-x", "conv-corrupt")
	...
	err := store.Save(ctx, "tenant-x", "someone", "conv-corrupt", "hijacked", "", nil)
	require.Error(t, err, "Save must refuse to adopt/overwrite a hash that exists but has no recorded owner")
	assert.Contains(t, err.Error(), "different user")

	title, titleErr := client.HGet(ctx, hashKey, "title").Result()
	require.NoError(t, titleErr)
	assert.Equal(t, "orphaned", title, "the existing hash must be untouched")
}

// TestRedisConversationStore_Rename_AbsentOwnerField_Denies is a regression
// guard proving Rename — the one handler that already had the correct
// `err == goredis.Nil || existingUserID != callerUserID` form — keeps
// denying against a real Redis client, not just the in-memory fake.
func TestRedisConversationStore_Rename_AbsentOwnerField_Denies(t *testing.T) {
	store, client := newRedisConvStoreTest(t)
	ctx := context.Background()
	hashKey := convHashKey("tenant-x", "conv-corrupt")
	...
	err := store.Rename(ctx, "tenant-x", "someone", "conv-corrupt", "hijacked")
	require.Error(t, err, "Rename must refuse a hash that exists but has no recorded owner")
...
// TestRedisConversationStore_Get_AbsentOwnerField_Denies is a regression
// guard proving Get — which already compares userID != callerUserID
// unconditionally via HMGet's allNil check, not a bare HGet error — keeps
// denying against a real Redis client.
func TestRedisConversationStore_Get_AbsentOwnerField_Denies(t *testing.T) {
	store, client := newRedisConvStoreTest(t)
	ctx := context.Background()
	hashKey := convHashKey("tenant-x", "conv-corrupt")
	...
	_, _, err := store.Get(ctx, "tenant-x", "someone", "conv-corrupt")
	require.Error(t, err, "Get must refuse to return a hash that exists but has no recorded owner")
}
