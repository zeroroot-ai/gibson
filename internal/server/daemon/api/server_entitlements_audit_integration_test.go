...
// miniredis (same pattern used by internal/platform/audit/logger_test.go), then read
// the tenant's audit Redis stream via AuditLogger.Query and assert the
// event's shape.
//
// Run with:
//   go test -tags integration -run TestAuditEmission ./internal/server/daemon/api/...
