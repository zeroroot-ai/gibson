...
type LLMConfigHandler struct {
	...
	// allowPrivateEndpoints mirrors security.allow_private_llm_endpoints.
	// False (the zero value, and the secure default) keeps the connect-time
	// SSRF egress guard on for providers this handler constructs from a stored
	// base_url. Set via WithAllowPrivateLLMEndpoints.
	allowPrivateEndpoints bool
}

// WithAllowPrivateLLMEndpoints forwards the operator-level
// security.allow_private_llm_endpoints knob to providers this handler builds.
...
func (h *LLMConfigHandler) TestConnection(ctx context.Context, name string) (*TestConnectionResult, error) {
	...
	providerConfig := llm.ProviderConfig{
		...
		// stored.BaseURL is tenant-supplied: keep the connect-time SSRF egress
		// guard on unless the operator opted out.
		AllowPrivateEndpoint: h.allowPrivateEndpoints,
	}
