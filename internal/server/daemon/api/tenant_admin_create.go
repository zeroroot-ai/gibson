...
func (s *DaemonServer) CreateAgentIdentity(ctx context.Context, req *tenantpb.CreateAgentIdentityRequest) (*tenantpb.CreateAgentIdentityResponse, error) {
	...
	// The ceiling is enrollment policy set by the admin (ADR-0045): it names
	// the session capabilities the credential may carry, and only the
	// reserved names exist. An interactive coding agent needs
	// mission:originate to start the live mission that is its session
	// (gibson#1593 decision 9).
	ceiling, err := validateCapabilityCeiling(req.GetCapabilityCeiling())
	if err != nil {
		return nil, status_grpc.Error(codes.InvalidArgument, err.Error())
	}
...
// reserved session capability (capname) and returns the de-duplicated list.
// Any other name is an error: the ceiling is not a place for free-form
// component capabilities, which come from the FGA grants instead.
func validateCapabilityCeiling(requested []string) ([]string, error) {
	if len(requested) == 0 {
		return nil, nil
	}
	...
	for _, name := range requested {
		if !capname.IsReserved(name) {
			return nil, fmt.Errorf("capability_ceiling entry %q is not a session capability; reserved names: %s, %s",
				name, capname.MissionOriginate, capname.MissionDelegate)
		}
...
// componentKindOf maps a principal's FGA type to the component-kind qualifier
// of the object it registers as (authz.ComponentObject).
func componentKindOf(fgaType string) string {
	switch fgaType {
	case "tool_principal":
		return authz.KindTool
	case "plugin_principal":
		return authz.KindPlugin
	default:
		return authz.KindAgent
	}
}
