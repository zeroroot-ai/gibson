...
//
// Fails closed when no authorizer is wired. FGA is a hard startup dependency
// in every shipped configuration (one-code-path epic, deploy#195: the
// noopAuthorizer was deleted, and initAuthorizer aborts daemon startup rather
// than serving with s.authorizer left nil — see
// internal/server/daemon/authz_init.go and the package doc comment on
// internal/platform/authz.Authorizer), so a nil authorizer here means either
// a test harness that hasn't wired one, or a DaemonServer constructed outside
// the normal daemon bootstrap. Every sibling nil-authorizer check in this
// package (isTenantAdmin in server_budget.go, and the ~20 explicit
// `authorizer not configured` returns across internal/server/daemon/api and
// internal/server/admin) already denies in that case; this was the one
// exception that allowed through instead, silently turning every RPC gated
// by requireTenantAdmin — ListAuditEvents, GetTenantQuota, SetBudget,
