...
// integTestSubject is the caller identity used by TestNewRPCsNotUnimplemented.
// requireTenantAdmin fails closed with no authorizer wired (server_audit.go),
// so — same as mission_draft_test.go's draftTestSubject — this smoke test must
// wire a real (fake) authorizer and an identity-bearing context rather than
// relying on the old nil-authorizer bypass.
const integTestSubject = "integ-admin"
...
// integCtx returns a context carrying integTestSubject as the caller
// identity, satisfying requireTenantAdmin's auth.IdentityFromContext lookup.
func integCtx() context.Context {
	return auth.WithIdentity(context.Background(), auth.Identity{Subject: integTestSubject})
}
