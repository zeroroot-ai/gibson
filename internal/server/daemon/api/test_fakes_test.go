...
// exercise the ResumeMission rewind path without needing the production
// Redis + FGA stack. (The checkpoint-browser handlers these fakes also
// served were retired in gibson#1321.)
//
...
func (f *fakeDaemon) RunMission(_ context.Context, _, _ string, _ map[string]string, _ string) (string, error) {
	return "fake-mission", nil
}
...
func (f *fakeDaemon) ResumeMission(_ context.Context, _ string) error        { return nil }
