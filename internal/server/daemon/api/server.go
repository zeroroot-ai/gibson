...
type DaemonServer struct {
	...
	// allowPrivateLLMEndpoints mirrors security.allow_private_llm_endpoints.
	// False (the zero value, and the secure default) keeps the connect-time
	// SSRF egress guard on for every provider this server constructs. Wired via
	// WithAllowPrivateLLMEndpoints.
	allowPrivateLLMEndpoints bool
	...
	// appURL is the PRODUCT-surface origin (app.<domain>) — the host that
	// serves the signup and sign-in pages. Distinct from gibsonPublicURL, which
	// is the API plane. Every user-facing link the daemon emails is built from
	// this one. Wired via WithAppURL from GIBSON_APP_URL; see EnvAppURL.
	appURL string
	...
	// WithSignupVerificationStore.
	//
	// Required for self-serve signup. When nil the SignupService handlers
	// return Unavailable — they do NOT fall back to an unverified path,
	// because without this store there is nowhere to record that an address
	// was ever proven.
	signupVerifications signupVerificationStore

	// signupMailer sends the two signup messages (verification link /
	// account-exists notice). Wired via WithSignupMailer.
	//
	// Required for self-serve signup: with no transport there is no way to
	// obtain proof of mailbox control, so the handlers refuse rather than
	// provision unverified.
	signupMailer signupVerificationSender

	// signupLimiter is the sliding-window limiter guarding the three
	// unauthenticated signup RPCs. Wired via WithSignupLimiter.
	//
	// Required. A nil limiter fails the request closed — see
	// signup_rate_limit.go.
	signupLimiter signupLimiter

	// signupClock is injectable in tests so cooldown and expiry behaviour can
	// be driven without sleeping. Nil means time.Now().
	signupClock func() time.Time
}
...
type DaemonInterface interface {
	...
	// RunMission starts a mission by reference and returns the started
	// mission's ID. The mission definition must already be registered via
	// CreateMissionDefinition and the target must already be registered —
	// inline construction is no longer supported. Callers observe the run
	// via Subscribe filtered on the returned ID; the per-run event channel
	// was retired in gibson#1112 PR 3 (every observable lifecycle event
	// flows brain → lifecycle projector → EventBus/Redis).
	RunMission(ctx context.Context, missionDefinitionID, targetID string, variables map[string]string, memoryContinuity string) (string, error)
...
// missionEventFromBus unwraps the MissionEventData payload for one mission
// from a bus EventData, or nil when the event is not this mission's.
func missionEventFromBus(ev EventData, missionID string) *MissionEventData {
	me := ev.MissionEvent
	if me == nil || me.MissionID != missionID {
		return nil
	}
	return me
}
...
func isTerminalMissionEvent(me *MissionEventData) bool {
	if me == nil || me.EventType != "status" {
		return false
	}
	st, _ := me.Payload["status"].(string)
...
func (s *DaemonServer) RunMission(req *daemonpb.RunMissionRequest, stream grpc.ServerStreamingServer[daemonpb.RunMissionResponse]) error {
	...
	// Subscribe BEFORE starting the mission so no early lifecycle event can
	// be missed; the mission ID is not known until Run returns, so the
	// subscription is unfiltered and the loop below filters by ID
	// (gibson#1112 PR 3 — events flow brain → lifecycle projector → bus,
	// not through a per-run channel).
	eventChan, err := s.daemon.Subscribe(stream.Context(), nil, "")
	if err != nil {
		s.logger.Error("failed to subscribe for mission events", "error", err)
		return preserveStatus(err, "failed to subscribe for mission events")
	}

	missionID, err := s.daemon.RunMission(stream.Context(), req.MissionDefinitionId, req.TargetId, req.Variables, req.MemoryContinuity)
	if err != nil {
	...
	for {
		...
		case <-stream.Context().Done():
			// The caller is a monitor, not the executor: the run continues on its
			// own detached context (see newMissionContext). Losing the client is
			// therefore a normal end to the stream, not a failure — returning the
			// context error made every client disconnect surface as a handler
			// error and read like the mission had died (gibson#1196).
			s.logger.Info("mission stream closed by client; run continues",
				"mission_definition_id", req.MissionDefinitionId,
				"target_id", req.TargetId,
			)
			return nil
		...
		case event, ok := <-eventChan:
			if !ok {
				// The subscription closed under us (daemon shutdown or bus
				// teardown) — not a statement about the mission, which runs
				// on its own detached context.
				s.logger.Info("mission event subscription closed; run continues",
					"mission_id", missionID,
				)
				return nil
			}
			me := missionEventFromBus(event, missionID)
			if me == nil {
				continue
			}
			...
			// A terminal status event ends the stream — the projector is the
			// single producer of these, replacing the channel-close signal.
			if isTerminalMissionEvent(me) {
				s.logger.Info("mission reached terminal state; ending stream",
					"mission_id", missionID,
				)
				return nil
			}
...
func (s *DaemonServer) ResumeMission(req *daemonpb.ResumeMissionRequest, stream grpc.ServerStreamingServer[daemonpb.ResumeMissionResponse]) error {
	...
	// server-side (gibson#1112 PR 3).
	eventChan, err := s.daemon.Subscribe(stream.Context(), nil, req.MissionId)
	if err != nil {
		s.logger.Error("failed to subscribe for mission events", "error", err, "mission_id", req.MissionId)
		return status_grpc.Errorf(codes.Internal, "failed to subscribe for mission events: %v", err)
	}

	// Call daemon implementation to resume the mission
	if err := s.daemon.ResumeMission(stream.Context(), req.MissionId); err != nil {
	...
	for {
		select {
		case <-stream.Context().Done():
			// The caller is a monitor, not the executor: the resumed run
			// continues on its own detached context (newMissionContext), so a
			// client disconnect is a normal end of the stream, not a failure.
			// Returning the context error made every client disconnect surface
			// as a handler error and read like the mission had died — the same
			// defect RunMission fixed (gibson#1196, gibson#1112 PR 3).
			s.logger.Info("mission resume stream closed by client; run continues", "mission_id", req.MissionId)
			return nil

		case event, ok := <-eventChan:
			if !ok {
				// Subscription closed under us (daemon shutdown or bus
				// teardown); the resumed run continues on its own context.
				s.logger.Info("mission event subscription closed; run continues", "mission_id", req.MissionId)
				return nil
			}
			me := missionEventFromBus(event, req.MissionId)
			if me == nil {
				continue
			}
			...
			// Terminal status ends the stream (gibson#1112 PR 3).
			if isTerminalMissionEvent(me) {
				s.logger.Info("resumed mission reached terminal state; ending stream",
					"mission_id", req.MissionId)
				return nil
			}
		}
...
//
// gibsoncheck:allow tenant-from-request — self-scoped: the supplied tenant is only ever the
// OBJECT of an FGA check whose SUBJECT is the caller's own identity, so a foreign tenant id
// truthfully returns role=member / is_admin=false rather than granting anything.
func (s *DaemonServer) GetMyPermissions(ctx context.Context, req *daemonpb.GetMyPermissionsRequest) (*daemonpb.GetMyPermissionsResponse, error) {
	...
	if s.authorizer == nil {
		// Fail closed. Returning role:"member" here is an untrue assertion:
		// with no authorizer wired the daemon cannot verify the caller holds
		// ANY relation on tenantID, and the dashboard renders whatever role
		// this RPC returns as real, live access. Spec:
		// identity-assertion-gaps finding 3.
		s.logger.ErrorContext(ctx, "GetMyPermissions: authorizer not wired; refusing to report a role",
	...
	if err != nil {
		...
		// would assert access the daemon was unable to actually verify.
		s.logger.WarnContext(ctx, "GetMyPermissions: BatchCheck failed",
			slog.String("tenant_id", tenantID),
			slog.String("user_id", userID),
