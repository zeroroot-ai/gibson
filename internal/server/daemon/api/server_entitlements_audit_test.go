...
func TestClassifyActorSource(t *testing.T) {
	cases := []struct {
		...
		{"spiffe platform → platform", &auth.Identity{Subject: "spiffe://zeroroot.ai/platform/tenant-operator", Issuer: "spiffe"}, "platform", ""},
		{"spiffe non-platform → operator", &auth.Identity{Subject: "spiffe://zeroroot.ai/dashboard", Issuer: "spiffe"}, "operator", ""},
		{"spiffe via subject prefix → platform", &auth.Identity{Subject: "spiffe://zeroroot.ai/platform/dashboard", Issuer: "other"}, "platform", ""},
