...
type fakeIDPClient struct {
	createCalls int // counts CreateServiceAccount calls
	...
	// address never reaches a credential write.
	...
	// findUserFn drives FindUserIDByEmail (the read-only directory lookup that
	// selects which signup email to send). Nil means "no such user".
...
func (f *fakeIDPClient) FindUserIDByEmail(ctx context.Context, email string) (string, error) {
	f.findUserCalls = append(f.findUserCalls, email)
	if f.findUserFn != nil {
...
// TestCreateAgentIdentity_CapabilityCeiling: the admin's capability_ceiling
// lands in the bootstrap credential's ceiling (gibson#1593 decision 9), so
// the registered session can start its own mission. Duplicates collapse.
func TestCreateAgentIdentity_CapabilityCeiling(t *testing.T) {
	srv := newTestDaemonServer(t).
		WithIdPAdminClient(&fakeIDPClient{}).
		WithTenantAdminAuditWriter(&fakeAuditWriter{})
	ctx := ctxWithTenantAdmin(context.Background(), "acme", "user-admin")
	resp, err := srv.CreateAgentIdentity(ctx, &tenantpb.CreateAgentIdentityRequest{
		Name:              "claude-session",
		Kind:              tenantpb.PrincipalKind_PRINCIPAL_KIND_AGENT,
		CapabilityCeiling: []string{capname.MissionOriginate, capname.MissionDelegate, capname.MissionOriginate},
	})
	if err != nil {
		t.Fatalf("CreateAgentIdentity: %v", err)
	}
	claims, err := srv.cgMinter.VerifyBootstrapToken(resp.GetBootstrapToken())
	if err != nil {
		t.Fatalf("VerifyBootstrapToken: %v", err)
	}
	want := []string{capname.MissionOriginate, capname.MissionDelegate}
	if !slices.Equal(claims.CapabilityCeiling, want) {
		t.Errorf("CapabilityCeiling = %v, want %v", claims.CapabilityCeiling, want)
	}
}

// TestCreateAgentIdentity_CapabilityCeilingRejectsNonReserved: the ceiling
// accepts only the reserved session capabilities. A free-form name is
// InvalidArgument and nothing is provisioned (no IdP call, no rollback).
func TestCreateAgentIdentity_CapabilityCeilingRejectsNonReserved(t *testing.T) {
	fakeidp := &fakeIDPClient{}
	srv := newTestDaemonServer(t).
		WithIdPAdminClient(fakeidp).
		WithTenantAdminAuditWriter(&fakeAuditWriter{})
	...
	_, err := srv.CreateAgentIdentity(ctx, &tenantpb.CreateAgentIdentityRequest{
		Name:              "claude-session",
		Kind:              tenantpb.PrincipalKind_PRINCIPAL_KIND_AGENT,
		CapabilityCeiling: []string{"agent_execute"},
	})
	if status.Code(err) != codes.InvalidArgument {
		t.Fatalf("err = %v, want InvalidArgument", err)
	}
	if fakeidp.createCalls != 0 {
		t.Errorf("IdP CreateServiceAccount called %d times, want 0", fakeidp.createCalls)
	}
}

// TestCreateAgentIdentity_NoCeilingByDefault: with no capability_ceiling the
// credential carries none, so a plain agent cannot originate missions.
func TestCreateAgentIdentity_NoCeilingByDefault(t *testing.T) {
	srv := newTestDaemonServer(t).
		WithIdPAdminClient(&fakeIDPClient{}).
		WithTenantAdminAuditWriter(&fakeAuditWriter{})
	ctx := ctxWithTenantAdmin(context.Background(), "acme", "user-admin")
	...
	if len(claims.CapabilityCeiling) != 0 {
		t.Errorf("CapabilityCeiling = %v, want empty", claims.CapabilityCeiling)
	}
}
...
func TestCreateAgentIdentity_EnablesComponentForOwningTenant(t *testing.T) {
	cases := []struct {
		...
		{tenantpb.PrincipalKind_PRINCIPAL_KIND_AGENT, "component:agent/zerocool-claude"},
		{tenantpb.PrincipalKind_PRINCIPAL_KIND_TOOL, "component:tool/zerocool-claude"},
		{tenantpb.PrincipalKind_PRINCIPAL_KIND_PLUGIN, "component:plugin/zerocool-claude"},
	}
	for _, tc := range cases {
		t.Run(tc.object, func(t *testing.T) {
			...
			srv := newTestDaemonServer(t).
				...
				WithTenantAdminAuditWriter(&fakeAuditWriter{})
			ctx := ctxWithTenantAdmin(context.Background(), "acme", "user-admin")
			if _, err := srv.CreateAgentIdentity(ctx, &tenantpb.CreateAgentIdentityRequest{Name: "zerocool-claude", Kind: tc.kind}); err != nil {
				t.Fatalf("CreateAgentIdentity: %v", err)
			}
			want := authz.Tuple{User: "tenant:acme", Relation: "tenant_enabled", Object: tc.object}
			for _, tup := range az.writtenTuples() {
				if tup == want {
					return
				}
			}
			t.Errorf("expected tenant_enabled tuple %+v in writes %+v", want, az.writtenTuples())
		})
	}
}
...
// TestClientCapabilityGrants_KindPolicy pins the kind->grant table (ADR-0046,
// ADR-0066): agents and tools are clients (granted direct_execute on the system
// backplane); a plugin is invoked-only for DRIVING the platform but still
// RECEIVES its dispatched work, so it is granted direct_receive_work (which
// gates PollWork/SubmitResult via can_poll_work) and NOT direct_execute.
func TestClientCapabilityGrants_KindPolicy(t *testing.T) {
	...
	sysReceive := authz.Tuple{User: "p:x", Relation: "direct_receive_work", Object: "component:_system"}
	cases := []struct {
		fgaType string
		want    []authz.Tuple
	}{
		...
		{"plugin_principal", []authz.Tuple{sysReceive}},
	}
	for _, tc := range cases {
		got := capabilitygrant.ClientCapabilityGrants("p:x", tc.fgaType)
