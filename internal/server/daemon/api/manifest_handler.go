...
func (s *DaemonServer) resolveManifestSubject(id auth.Identity, req *manifestpb.GetCapabilityManifestRequest, tenantID string, ctx context.Context) (manifest.ManifestSubject, error) {
	...
	if req.GetAgentPrincipalId() != "" {
		...
		// Fail closed. An unwired authorizer is the daemon being unable to
		// evaluate the admin requirement — not evidence that the caller
		// meets it. This check used to be conditional on s.authorizer != nil,
		// which meant a misconfigured or partially-started daemon granted
		// every caller impersonation.
		if s.authorizer == nil {
			s.logger.ErrorContext(ctx, "manifest: impersonation denied — authorizer not wired, cannot verify tenant admin",
				"caller", id.Subject,
				"tenant_id", tenantID,
				"requested_agent_principal_id", req.GetAgentPrincipalId(),
			)
		...
		return manifest.ManifestSubject{
			...
			// fetching its own.
			ImpersonatedAgentPrincipalID: req.GetAgentPrincipalId(),
