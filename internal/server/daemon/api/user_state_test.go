...
// tenantCtxNoSubject returns a context carrying a tenant but no caller subject.
// Used to prove the handlers fail closed rather than falling back to a
// request-body user id.
func tenantCtxNoSubject(tenantID string) context.Context {
	t, _ := auth.NewTenantID(tenantID)
	return auth.WithIdentity(context.Background(), auth.Identity{Tenant: t})
}
...
func TestInvalidateMembershipCache_Idempotent(t *testing.T) {
	...
	_, err := srv.InvalidateMembershipCache(ctx, &tenantv1.InvalidateMembershipCacheRequest{UserId: "user-1"})
	require.NoError(t, err)

	// Set a fake cache entry directly.
	rc := srv.userStateRedis
	require.NoError(t, rc.Set(ctx, membershipCacheKey("user-1"), `[{"role":"admin"}]`, 0).Err())
	...
	// Key must be gone.
	exists, err := rc.Exists(ctx, membershipCacheKey("user-1")).Result()
	require.NoError(t, err)
