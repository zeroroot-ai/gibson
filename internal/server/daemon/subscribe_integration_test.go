...
// testMissionEvent builds the minimal EventData shape the bus tests need.
// The production constructors these tests used to lean on were dead code
// outside the tests (gibson#1112) and are deleted; what the tests exercise
// is Subscribe/Publish routing, which only cares about EventType and the
// presence of a MissionEvent payload.
func testMissionEvent(eventType, missionID, nodeID string) api.EventData {
...
func TestSubscribeIntegration(t *testing.T) {
	...
	event := testMissionEvent("mission.started", "test-mission", "")
...
func TestSubscribeWithMultipleClients(t *testing.T) {
	...
	// Publish an event
	event := testMissionEvent("mission.started", "mission-1", "")
	err := mockDaemon.eventBus.Publish(ctx, event)
	require.NoError(t, err)
...
func TestSubscribeFiltersEventTypes(t *testing.T) {
	...
	events := []api.EventData{
		testMissionEvent("mission.started", "mission-1", ""),
		testMissionEvent("node.started", "mission-1", "node-1"),
		testMissionEvent("mission.completed", "mission-1", ""),
		testMissionEvent("mission.started", "mission-2", ""),
	}
...
func TestSubscribeFiltersByMissionID(t *testing.T) {
	...
	events := []api.EventData{
		...
		testMissionEvent("mission.started", "mission-2", ""),
		testMissionEvent("node.started", "mission-1", "node-1"),
		testMissionEvent("node.started", "mission-2", "node-2"),
		testMissionEvent("mission.completed", "mission-1", ""),
		testMissionEvent("mission.completed", "mission-2", ""),
	}
