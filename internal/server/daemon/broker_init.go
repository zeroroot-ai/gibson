...
func (d *daemonImpl) initBrokerStack(ctx context.Context, compSvc *component.ComponentServiceServer) error {
	...
	if len(systemKEK) == 32 && d.config.PlatformPostgres.Host != "" {
		...
		if err != nil {
			...
			if err != nil {
				...
				// Build the provider factories used to probe a tenant-supplied
				// broker-config candidate. See newBrokerCandidateFactories for
				// why that map is guarded and the registry factory below is not.
				configFactories := newBrokerCandidateFactories(ctx, d.config.Security.AllowPrivateBrokerEndpoints, d.logger)
	...
	// used by the secrets broker. Store the service on the daemon so
