...
func (f *fakeGraphWriter) UpsertEntity(_ context.Context, tenant string, e brain.EntitySnapshot) error {
	f.mu.Lock()
	defer f.mu.Unlock()
...
func (f *fakeGraphWriter) UpsertObservation(_ context.Context, tenant string, o brain.ObservationSnapshot) error {
	f.mu.Lock()
	defer f.mu.Unlock()
	f.observations[tenant] = append(f.observations[tenant], o)
	return nil
}
...
func (f *fakeGraphWriter) UpsertMission(_ context.Context, tenant string, m MissionProjection) error {
	f.mu.Lock()
	defer f.mu.Unlock()
	f.missions[tenant] = append(f.missions[tenant], m)
	return nil
}
