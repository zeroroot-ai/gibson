...
// Per-message ceilings for the daemon gRPC server. Stated explicitly because
// some callers on this surface are components executing inside sandboxes; the
// daemon should not accept an arbitrarily large message from one on the
// strength of a library default.
const (
	maxDaemonRecvMsgBytes = 16 * 1024 * 1024 // 16 MiB
	maxDaemonSendMsgBytes = 16 * 1024 * 1024 // 16 MiB
)
...
// looseModeForEntry reports whether entry is registry-classified as
// tenant-unscoped by design: `unauthenticated: true` (Connect, Ping) or
// `self: true` (sign-in self-bootstrap: ListMyMemberships,
// GetMyPermissions). Both modes bypass strict tenant validation — ext-authz
// has already enforced the per-RPC AllowedIdentities bitfield for self-mode
...
// ONLY to satisfy auth.IdentityFromMetadata's tenant-presence check inside
// looseIdentityFromMD when the real tenant header is absent or empty. It is
// stripped back off before the resulting Identity is placed on the request
// context — it never reaches handler code or auth.TenantFromContext.
const looseModePlaceholderTenant = "loose-mode-placeholder"

// looseIdentityFromMD builds a validated Identity for RPCs whose registry
// entry is Unauthenticated or Self-mode (looseModeForEntry). It applies the
// SAME structural validation and freshness check as auth.IdentityFromMetadata
// — non-empty subject, a known issuer, and an issued-at timestamp inside the
// configured replay-bound skew — but does NOT require the tenant header,
// since self/unauthenticated RPCs are not tenant-scoped by design (Spec:
// self-mode-authz Req 4.6). ext-authz always emits all five identity headers
// on an allow response, including x-gibson-identity-tenant (set to "" rather
// than omitted for self-mode bootstrap calls), so the tenant header is
// distinguished by an empty value, not absence.
//
// On any validation failure — missing/empty subject, unknown issuer, stale
// or future-dated issued-at — this returns a PermissionDenied status error
// so a caller asserting an invalid or replayed identity is rejected exactly
// like every other RPC, instead of the previous behaviour of silently
// degrading to an unvalidated pass-through (identity-assertion-gaps finding
// 1).
func looseIdentityFromMD(ctx context.Context) (context.Context, error) {
	md, ok := grpcmetadata.FromIncomingContext(ctx)
	if !ok {
		return ctx, status.Errorf(codes.PermissionDenied, "auth: no metadata on incoming context")
	}

	// auth.IdentityFromMetadata requires all five headers, including
	// tenant, which self/unauthenticated RPCs legitimately do not carry.
	// Supply a placeholder tenant (a syntactically valid, non-reserved
	// TenantID — auth.SystemTenant is deliberately rejected by
	// NewTenantID's public path, so it cannot be used here) so the shared
	// validator can still run its subject/issuer/freshness checks
	// unchanged, then strip the placeholder back off so the resulting
	// Identity has the documented zero-value tenant (handler must not
	// require it).
	synthetic := md.Copy()
	tenantSupplied := false
	if vals := synthetic.Get(auth.HeaderTenant); len(vals) > 0 && vals[0] != "" {
		tenantSupplied = true
	}
	if !tenantSupplied {
		synthetic.Set(auth.HeaderTenant, looseModePlaceholderTenant)
	}

	id, err := auth.IdentityFromMetadata(synthetic)
	if err != nil {
		return ctx, status.Errorf(codes.PermissionDenied, "auth: %s", err)
	}
	if !tenantSupplied {
		id.Tenant = auth.TenantID{}
	}
...
// recognised direct-dial SPIFFE peer's method policy authorises the call,
// ok=false/err=nil when the peer is not a recognised direct-dial peer at all
// (fall through to the next path), or a non-nil err when a recognised peer's
// method policy denies the call outright.
type spiffeBypassFunc func(ctx context.Context, method string) (context.Context, bool, error)

// resolveLooseOrBypassIdentity is the shared, order-sensitive decision used
// by both the unary and stream registry-aware interceptors.
//
// The SPIFFE direct-dial bypass MUST be evaluated BEFORE the registry
// loose-mode (Unauthenticated/Self) check. Without this ordering, a
// policy-constrained direct-dial peer (e.g. the tenant-operator SVID, which
// is hard-denied outside DaemonOperatorService by operatorMethodPolicy)
// could sidestep its method policy on any RPC that happens to be
// registry-classified as tenant-unscoped — including the sign-in bootstrap
// set (ListMyMemberships, GetMyPermissions) — simply because the loose-mode
// branch used to run first and return unconditionally. bypass itself already
// returns a PermissionDenied error for an allow-listed peer calling a method
// outside its policy (spiffeBypassDecision, operator_method_policy.go); that
// error is now authoritative and is propagated immediately rather than
// falling through to loose mode. (identity-assertion-gaps finding 1.)
//
// Returns:
//   - (ctx, true, nil):  identity resolved via bypass or loose mode; the
//     caller should invoke the handler with ctx.
//   - (ctx, false, nil): neither path applies (not a recognised direct-dial
//     peer, and the method is not registry-classified loose-mode); the
//     caller must fall through to the strict sdk/auth interceptor.
//   - (ctx, false, err): identity resolution failed outright (bypass denied
//     by policy, or loose-mode validation failed); the caller must reject
//     the RPC with err.
func resolveLooseOrBypassIdentity(ctx context.Context, method string, bypass spiffeBypassFunc) (context.Context, bool, error) {
	if bypassCtx, ok, err := bypass(ctx, method); err != nil {
		return ctx, false, err
	} else if ok {
		return bypassCtx, true, nil
	}
	if entry, ok := registry.Registry[method]; ok && looseModeForEntry(entry) {
		looseCtx, err := looseIdentityFromMD(ctx)
		if err != nil {
			return ctx, false, err
		}
		return looseCtx, true, nil
	}
...
func (d *daemonImpl) buildGRPCServer(ctx context.Context) (*grpcSubsystem, error) {
	...
	registryAwareUnary := func(ctx context.Context, req any, info *grpc.UnaryServerInfo, handler grpc.UnaryHandler) (any, error) {
		newCtx, handled, err := resolveLooseOrBypassIdentity(ctx, info.FullMethod, spiffePlatformBypass)
		if err != nil {
			return nil, err
		}
		if handled {
	...
	registryAwareStream := func(srv any, ss grpc.ServerStream, info *grpc.StreamServerInfo, handler grpc.StreamHandler) error {
		newCtx, handled, err := resolveLooseOrBypassIdentity(ss.Context(), info.FullMethod, spiffePlatformBypass)
		if err != nil {
			return err
		}
		if handled {
			wrapped := &serverStreamCtxOverride{ServerStream: ss, ctx: newCtx}
	...
	serverOpts := baseServerOptions(unaryInterceptors, streamInterceptors, otelServerHandler)
	...
	// security.allow_private_llm_endpoints — the same knob the embedder
	// resolver honours below.
	daemonSvc.WithAllowPrivateLLMEndpoints(d.config.Security.AllowPrivateLLMEndpoints)
	if d.authorizer != nil {
	...
	if d.cgMinter != nil {
		...
		// Renewal needs a verifier as well as a minter, or the handler answers
		// FailedPrecondition for every request. It was never wired, so a
		// long-running dispatch could not renew its task grant at all
		// (gibson#1605). It is the same in-process verifier the callback
	...
	// (gibson#1602). The brain executor holds a binding for exactly as long as
	// a mission is executing, so it is the liveness source; without it renewal
	// refuses rather than issuing a grant nothing can end.
	if d.brainExecutor != nil {
		daemonSvc.WithLiveMissionLookup(d.brainExecutor)
		d.logger.Info(ctx, "mission-run liveness wired into DaemonServer; capability-grant renewal is bounded to a live run")
	} else {
		d.logger.Error(ctx, "brain executor NOT wired — capability-grant renewal will refuse every request")
	}
	...
	// Wire the self-serve signup verification stack: the platform-Postgres
	// store that records proof of mailbox control, the mail transport that
	// delivers the proof, and the Redis sliding-window limiter that guards the
	// three unauthenticated RPCs.
	//
	// Each is wired only when its dependency exists, and each handler REFUSES
	// when its piece is missing. That is deliberate: a daemon with no Postgres,
	// no mail transport or no Redis cannot establish that an address belongs to
	// the person signing up, and the correct response to "cannot verify" is to
	// decline the signup, not to provision anyway.
	//
	// When SIGNUP_SELF_SERVE is set, a missing verification store, app origin
	// or limiter is FATAL rather than logged. A deployment that has asked for
	// public signup and cannot record proof or count requests has no correct
	// degraded mode: it would either provision for unproven addresses or
	// advertise a signup form that can never complete. Refusing to boot
	// surfaces the misconfiguration to whoever deployed it, at deploy time,
	// instead of to a user staring at an inbox.
	//
	// The mail transport is the one exception (gibson#1228 / PR #1228, owner
	// decision: email must not block the daemon from booting). A mail
	// misconfiguration never stops the daemon starting — resolveSignupMailer
	// (signup_mailer.go) only warns. The RPC that needs delivery
	// (RequestEmailVerification) refuses per-call with FailedPrecondition
	...
	// signup is unavailable," not "the platform is unavailable."
	{
		...
		// The transport must actually deliver. mailer.RequireDelivering rejects
		// the log provider by name rather than by nil-ness: the previous guard
		// only checked that SOME mailer was wired, and the development stub is
		// not nil.
		//
		...
		// GIBSON_PUBLIC_URL is the api.<domain> origin (it is what the daemon
		// stamps into capability-grant discovery); the signup pages are served
		// from the app.<domain> origin, so the two are different values and
		// reusing the first produced a link to a host that serves no such route.
		if appURL := strings.TrimSpace(os.Getenv(api.EnvAppURL)); appURL != "" {
			daemonSvc.WithAppURL(appURL)
		} else if err := requireSelfServe("no product-surface origin for signup links",
			fmt.Errorf("%s is not set", api.EnvAppURL)); err != nil {
			return nil, err
		}
	...
	// Spec: tenant-secrets-broker-completion (Task 11, design D2); ADR-0039.
	{
		...
		// Invitation mailer (gibson#632). GIBSON_EMAIL_PROVIDER has no default,
		// and the accept link is a capability, so a non-delivering transport
		// (provider=log) is treated as no transport at all rather than as a
		// stub that "sends" to a log. Either way this is deliberately
		// non-fatal: we warn, leave the mailer unwired, and the invitation
		// RPCs refuse with FailedPrecondition. A mail misconfiguration must
		// never take down every other RPC.
		var adminMailer admin.InvitationMailer
		m, mErr := mailer.NewFromEnv(d.logger.Slog())
		if mErr == nil {
	...
	if d.pool != nil && d.authorizer != nil {
		bankSvc, bankErr := NewBankServer(BankServerConfig{
			Store:        bank.NewPostgresStore(d.pool),
			Authorizer:   d.authorizer,
			Entitlements: d.entitlementsProvider,
			Logger:       d.logger.Slog(),
			Control:      d.memberControl,
			Feed:         d.liveAgents,
		})
		if bankErr != nil {
			d.logger.Warn(ctx, "BankService not registered", slog.String("error", bankErr.Error()))
		} else {
			bankpb.RegisterBankServiceServer(srv, bankSvc)
			d.logger.Info(ctx, "registered gibson.bank.v1.BankService gRPC endpoint")
		}
		...
		jobSvc, jobErr := NewJobServer(JobServerConfig{
			...
			Authorizer:   d.authorizer,
			Logger:       d.logger.Slog(),
			MemberEvents: &memberEvents{daemon: d},
			Sessions: newPoolSessionContextStore(
				func() datapool.Pool { return d.pool },
				d.logger.WithComponent("job-result-store").Slog(),
			),
		})
		if jobErr != nil {
		...
		d.logger.Warn(ctx, "BankService and JobService not registered: the data-plane pool or the authorizer is unavailable")
	}
	...
	if d.authorizer != nil && d.compRegistry != nil {
		discoverySrv := discoverysvc.NewServer(d.authorizer, d.compRegistry, d.connectorLister(ctx), d.logger.Slog())
	...
	// by Envoy. Registered unconditionally; it stays empty until the sandboxed
	// launcher (setec_integration build) tees a run into the registry.
	agentconsolepb.RegisterAgentConsoleServiceServer(srv, NewAgentConsoleServer(d.liveAgents, d.logger.WithComponent("agent-console-service").Slog(), WithMemberSource(&lazyMemberSource{daemon: d})))
	d.logger.Info(ctx, "registered AgentConsoleService gRPC endpoint")
	...
	if d.stateClient != nil {
		if redisClient, ok := d.stateClient.Client().(*goredis.Client); ok {
			...
			if d.infrastructure != nil &&
				...
				// Install the per-tenant resolver — the SAME one the mission
				// path uses (harness_init) — or every component completion
				// fails closed with "per-tenant LLM provider resolution is not
				// configured". Without this the adapter was wired but inert:
				// an enrolled agent's completion never reached a provider.
				).WithTenantResolver(d.newSlotManagerForTenant())
				d.logger.Info(ctx, "LLM adapter wired into ComponentService with per-tenant resolver")
			} else {
			...
			// Wire the DiscoveryResult ingest path so a component's SubmitResult
			// carrying proto field 100 folds the discovered entities into the
			// owning tenant's World, which the graph projector materializes
			// (gibson#1266). Shares the daemon's single processor with the
			// callback and sandboxed dispatch paths.
			compSvc.WithDiscoveryProcessor(&discoveryProcessorAdapter{inner: d.newDiscoveryProcessor()})
			d.logger.Info(ctx, "DiscoveryResult ingest wired into ComponentService: SubmitResult field 100 → World → graph projector")
			...
			// Wire the capability checker so DelegateToAgent (and, once slice 2
			// lands, mission origination) can enforce capname.MissionDelegate /
			// capname.MissionOriginate (gibson#1186 slice C). *capabilitygrant.
			// CapabilityGrantService satisfies component.CapabilityChecker
			// structurally via its HasCapability method — no adapter needed.
			// d.capabilityGrantSvc is nil exactly when the FGA authorizer isn't
			// wired (see the block above); DelegateToAgent fails closed
			// (Unavailable) rather than allowing when this stays nil.
			if d.capabilityGrantSvc != nil {
				compSvc.WithCapabilityChecker(d.capabilityGrantSvc)
				d.logger.Info(ctx, "CapabilityGrantService wired into ComponentService as the mission:delegate/mission:originate capability checker")
			} else {
				d.logger.Warn(ctx, "CapabilityGrantService unavailable; ComponentService capability-gated RPCs (DelegateToAgent) will deny every call")
			}
			...
			// Wire the finding reader so an off-cluster component can read back the
			// findings it wrote (gibson#1186 slice B). It is the read counterpart of
			// findingSubmitter above and shares its per-tenant Pool, so isolation is
			// the tenant's own graph database rather than a query predicate.
			// d.pool is nil when security.key_provider is not configured; the seam
			// stays nil in that case and the read RPCs keep answering Unimplemented.
			if d.pool != nil {
				compSvc.WithFindingQuerier(component.NewPoolFindingQuerier(
					d.pool,
					d.logger.WithComponent("finding-querier").Slog(),
				))
				d.logger.Info(ctx, "PoolFindingQuerier wired into ComponentService: finding reads → per-tenant graph")
			} else {
				d.logger.Warn(ctx, "per-tenant pool unavailable; ComponentService finding-read RPCs will return Unimplemented")
			}

			// Wire the GraphRAG reader/writer so an off-cluster component can use
			// the tenant's knowledge graph (gibson#1190). Same per-tenant Pool as
			// the finding seams above — the Conn is the tenant boundary, since
			// per-tenant Neo4j databases and vector collections are physically
			// separate.
			//
			// The embedder resolver is required, not optional: FindSimilarFindings
			// and semantic QueryNodes are vector searches, and serving them without
			// an embedder would mean answering with a different operation under the
			// same name. A tenant with no embedding provider gets a clear error;
			// a daemon with no resolver at all leaves the seam nil, so the RPCs
			// keep answering Unimplemented rather than half-working.
			if d.pool != nil && d.embedderResolver != nil {
				...
				// alike.
				//
				// Decided at WIRING time, not per request: a daemon with no
				// embedder resolver cannot serve vector searches at all, and
				// answering them without one would be a different operation
				// under the same name. Consumers see a nil querier and report
				// unavailable; they do not degrade silently.
				d.graphragQuerier = component.NewPoolGraphRAGQuerier(
					d.pool,
					d.embedderResolver,
					// Query tuning only: the connections come from the pool, so
					// the daemon's Neo4j connection config has no say here.
					graphrag.QueryConfig{},
					d.logger.WithComponent("graphrag-querier").Slog(),
				)
				compSvc.WithGraphRAG(d.graphragQuerier)
				d.logger.Info(ctx, "PoolGraphRAGQuerier wired into ComponentService: knowledge-graph reads/writes → per-tenant graph + vector store")
			} else {
				d.logger.Warn(ctx, "ComponentService GraphRAG RPCs will return Unimplemented",
					"pool_nil", d.pool == nil,
					"embedder_resolver_nil", d.embedderResolver == nil,
				)
			}
			...
			// Wire the mission surface (gibson#1358, slice 2 of gibson#1186
			// slice C). componentMissionManager owns the origination policy —
			// budget clamped and reserved against the parent mission's
			// envelope, target scope checked as a subset of the parent's, and
			// lineage recorded at creation from the verified caller identity.
			//
			// It needs the per-tenant pool: every mission read and write goes
			// through the caller's own tenant Conn, which is the isolation
			// boundary. d.pool is nil when security.key_provider is not
			// configured; the seam stays nil then and every mission RPC
			// answers Unavailable, which is the honest answer — "the daemon
			// cannot evaluate this", never "go ahead".
			if d.pool != nil {
				compSvc.WithMissionManager(newComponentMissionManager(d))
				d.logger.Info(ctx, "componentMissionManager wired into ComponentService: mission origination gated on the mission:originate capability")
			} else {
				d.logger.Warn(ctx, "per-tenant pool unavailable; ComponentService mission RPCs will return Unavailable")
			}
			...
			// can_invoke decision in-handler (gibson#1245): its registry rule
			// derives the FGA object from the PluginName body field, which
			// ext-authz cannot see, so the gateway passes through and the
...
func (d *daemonImpl) RunMission(ctx context.Context, missionDefinitionID, targetID string, variables map[string]string, memoryContinuity string) (string, error) {
...
//
// Cancellation is delegated in full to missionManager, whose liveness map is
// keyed by (tenant, missionID) and which resolves the tenant from the request
// context — so a caller can only ever stop a mission inside its own tenant.
// The daemon deliberately keeps no mission-ID-keyed cancel map of its own: an
// unqualified mission ID is a cross-tenant cancel primitive.
func (d *daemonImpl) StopMission(ctx context.Context, missionID string, force bool) error {
	...
	// Validate mission ID
	if missionID == "" {
		return errors.New("mission ID cannot be empty")
	}

	// Resolve the calling tenant BEFORE touching any mission state. A caller
	// with no tenant on its context must not be able to cancel a mission at
	// all, so this precedes the missionManager delegation below rather than
	// being left to missionManager's own internal check (gibson#1232).
	if _, tenantErr := tenantFromCtx(ctx); tenantErr != nil {
		return tenantErr
	}
	...
	if stopErr := d.missionManager.Stop(ctx, missionID, force); stopErr != nil {
		d.logger.Warn(ctx, "mission not found", "mission_id", missionID, "error", stopErr)
		return fmt.Errorf("mission not found or not running: %s", missionID)
	}
...
// ResumeMission resumes a paused mission from its last checkpoint. Callers
// observe the resumed run by subscribing to the event stream filtered on the
// mission ID (the per-run event channel was retired in gibson#1112 PR 3).
func (d *daemonImpl) ResumeMission(ctx context.Context, missionID string) error {
	...
	if err := d.missionManager.Resume(ctx, missionID); err != nil {
		d.logger.Error(ctx, "failed to resume mission", "error", err, "mission_id", missionID)
		return fmt.Errorf("failed to resume mission: %w", err)
	}
...
func (d *daemonImpl) ListMissionDefinitions(ctx context.Context, limit int, offset int) ([]api.MissionDefinitionData, int, error) {
	...
	tenantForDefs, tenantErr := tenantFromCtx(ctx)
	if tenantErr != nil {
		return nil, 0, tenantErr
	}
	if d.pool == nil {
...
func (d *daemonImpl) GetMissionDefinition(ctx context.Context, name string) (*missionpb.MissionDefinition, error) {
	...
	tenantForDef, tenantErr := tenantFromCtx(ctx)
	if tenantErr != nil {
...
func (d *daemonImpl) CreateMission(ctx context.Context, req api.CreateMissionData) (api.CreateMissionResultData, error) {
	...
	// Resolve the calling tenant BEFORE the target lookup. A caller with no
	// tenant on its context is refused outright rather than being allowed to
	// probe the daemon-wide target store first (gibson#1232).
	tenantForMission, tenantErr := tenantFromCtx(ctx)
	if tenantErr != nil {
		return api.CreateMissionResultData{}, tenantErr
	}
	...
	//
	// The caller tenant here comes from resolveTargetCallerTenant, NOT from the
	// tenantFromCtx result above: it reports an absent context tenant as "",
	// which falls through to resolveTargetUUID's ordinary tenant-mismatch
	// rejection rather than matching the system-tenant exemption. See its doc
	// comment in mission_manager.go. Both resolvers are fail-closed and neither
	// widens an absent tenant to auth.SystemTenant (gibson#1232).
	if d.targetStore == nil {
		return api.CreateMissionResultData{}, fmt.Errorf("target store not initialized")
	}
	resolvedTarget, err := resolveTargetUUID(ctx, d.targetStore, req.TargetID, resolveTargetCallerTenant(ctx))
	if err != nil {
...
func (d *daemonImpl) CreateMissionDefinition(ctx context.Context, req api.CreateMissionDefinitionData) (api.CreateMissionDefinitionResultData, error) {
	...
	tenantForCreate, tenantErr := tenantFromCtx(ctx)
	if tenantErr != nil {
		return api.CreateMissionDefinitionResultData{}, tenantErr
	}
	if d.pool == nil {
...
func (d *daemonImpl) UpdateMissionDefinition(ctx context.Context, req api.UpdateMissionDefinitionData) (api.UpdateMissionDefinitionResultData, error) {
	...
	tenantForUpdate, tenantErr := tenantFromCtx(ctx)
	if tenantErr != nil {
		return api.UpdateMissionDefinitionResultData{}, tenantErr
	}
	if d.pool == nil {
