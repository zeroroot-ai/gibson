...
func TestSPIFFEInitFailClosed(t *testing.T) {
	...
	for _, name := range []string{"grpc.go", "daemon.go"} {
		...
		require.NoError(t, err, "could not read %s; run test from internal/server/daemon/", name)
		srcStr += string(b)
	}
...
func TestBuildGRPCServer_ClientAuthRejectsNoCert(t *testing.T) {
	...
	require.NoError(t, err, "could not read grpc.go; run test from core/gibson/ or internal/server/daemon/")
	srcStr := string(src)
