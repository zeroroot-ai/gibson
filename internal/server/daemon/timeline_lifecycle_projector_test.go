...
func TestProjectBrainEvent(t *testing.T) {
	...
	rows := []row{
		// ---- pause / resume (gibson#1112 PR 3) ----------------------------
		{
			name:          "MissionPauseRequested maps to mission.paused",
			ev:            brain.MissionPauseRequested{ID: "m-pause"},
			wantEventType: "mission.paused",
			wantMissionID: "m-pause",
			wantStatus:    "paused",
		},
		{
			name:          "MissionResumed maps to mission.resumed",
			ev:            brain.MissionResumed{ID: "m-resume"},
			wantEventType: "mission.resumed",
			wantMissionID: "m-resume",
			wantStatus:    "running",
		},
