...
var operatorMethodPolicy = map[string]operatorMethodDecision{
	...
	daemonoperatorv1.DaemonOperatorService_EnqueueTenantProvisioning_FullMethodName: {
		allowed: true,
		reason:  "operator seeds the first tenant on a headless self-hosted install (gibson#1496)",
	},
...
// denied with denyReason.
func denyAllExcept(base map[string]operatorMethodDecision, allowed map[string]string, denyReason string) map[string]operatorMethodDecision {
	out := make(map[string]operatorMethodDecision, len(base))
	for method := range base {
		out[method] = operatorMethodDecision{allowed: false, reason: denyReason}
	}
	for method, reason := range allowed {
		out[method] = operatorMethodDecision{allowed: true, reason: reason}
	}
	return out
}
...
func operatorAllowedMethods() map[string]bool {
	return allowedMethodsOf(operatorMethodPolicy)
}

// connectorOperatorAllowedMethods is the connector-operator's counterpart of
// operatorAllowedMethods, derived from connectorOperatorMethodPolicy.
func connectorOperatorAllowedMethods() map[string]bool {
...
func allowedMethodsOf(policy map[string]operatorMethodDecision) map[string]bool {
	...
	for method, decision := range policy {
		if decision.allowed {
			allowed[method] = true
		}
	}
...
//
// Split out so the merge is exercised in BOTH builds: in a production build
// e2ePeerMethodPolicies returns nil, so the fold in spiffePeerMethodPolicies
// never runs and the behaviour would otherwise be untested exactly where it
// matters most — a bug here silently drops or overwrites a real
// control-plane peer's method policy.
func mergePeerPolicies(base, extra map[string]map[string]bool) map[string]map[string]bool {
	for id, methods := range extra {
		base[id] = methods
	}
	return base
}
