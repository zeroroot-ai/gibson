...
func uriToRef(uri string) string {
	...
	rest := uri[len(prefix):]
	// Try the canonical slash separator first (gibson#1024). The segment
	// after the tenant slash is the "@"-encoded ref (authz.SecretObject folds
	// the ref's category colon to "@" so OpenFGA v1.15.1 accepts the id); undo
	// that to recover the exact broker ref.
	if slash := strings.IndexByte(rest, '/'); slash >= 0 {
		return authz.RefFromObjectSegment(rest[slash+1:])
	}
