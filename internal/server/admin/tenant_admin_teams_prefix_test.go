...
// (tenant:zero-root, parent, team:zero-root/<id>) tuple — never
// "tenant:tenant:zero-root", and never a doubled namespace segment
// ("team:tenant:zero-root/<id>") now that the tenant also appears inside the
// team object id.
//
// The tuple is built from the caller's context tenant, so this now also pins
// that requireCallerTenant accepts the already-prefixed form as a match rather
// than rejecting it as a foreign tenant. Cross-tenant rejection itself is
// covered by TestTeamRPCs_ForeignTenantRejected.
func TestCreateTeam_NormalizesTenantPrefix(t *testing.T) {
	...
	const wantObject = "team:zero-root/test"
