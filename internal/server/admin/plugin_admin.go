...
func (s *PluginsAdminServer) RegisterPlugin(ctx context.Context, req *tenantv1.RegisterPluginRequest) (*tenantv1.RegisterPluginResponse, error) {
	...
	tuples := make([]authz.Tuple, 0, len(req.GetBindings())+1)
	boundRefs := make(map[string]struct{}, len(req.GetBindings()))
	for _, b := range req.GetBindings() {
		ref := bindingRef(b)
		if ref == "" {
			continue
		}
		boundRefs[ref] = struct{}{}
		tuples = append(tuples, authz.Tuple{
			...
			Object:   authz.SecretObject(tenant.String(), ref),
		})
	}
	if len(tuples) > 0 {
