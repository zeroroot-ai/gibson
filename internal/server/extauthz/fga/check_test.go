...
func TestResolveObject(t *testing.T) {
	...
	cases := []struct {
		...
		{
			// A rule that names a request field has no object without that
			// field. Widening to the tenant-wide object "plugin:acme" would
			// ask a question the rule did not intend — one tuple there would
			// cover every plugin in the tenant.
			name:    "tenant_and_field fails closed when the field is absent",
			entry:   Entry{ObjectType: "plugin", ObjectDeriver: "tenant_and_field('plugin_id')"},
			meta:    map[string]string{},
			wantErr: true,
		},
		{
			name:    "from_field fails closed when the field is absent",
			entry:   Entry{ObjectType: "plugin", ObjectDeriver: "from_field('plugin_id')"},
			meta:    map[string]string{},
			wantErr: true,
		},
		{
			// The three field-derived rules in the live registry are secret
			// can_resolve rules. Writers put those tuples at
			// "secret:tenant-<t>/<ref>" (authz.SecretObject); "secret:acme"
			// is a different object, so the fallback could only ever be a
			// deny in disguise or a tenant-wide grant.
			name:    "tenant_and_field for secret fails closed when the ref is absent",
			entry:   Entry{ObjectType: "secret", ObjectDeriver: "tenant_and_field('Name')"},
			meta:    map[string]string{},
			wantErr: true,
		},
		{
			...
			// and the FGA model). The ref's category colon is encoded to "@" so the
			// id is colon-free (OpenFGA v1.15.1 rejects any colon in an object id);
			// the deriver must use SecretObjectFromDeriver, which applies that
			// encoding identically to the daemon-side writers/checkers.
			name:  "tenant_and_field for secret adds tenant- prefix, slash separator, colon-encoded ref",
	...
	for _, tc := range cases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			// The id must contain NO colon at all — OpenFGA v1.15.1 rejects a colon
			// anywhere in an object id, not only at the structural type-id boundary
			// (gibson#1024's colon-count assumption was for v1.8.4). Exact equality
			// against tc.want pins the "@"-encoded, colon-free form.
			if strings.HasPrefix(got, "secret:") && strings.ContainsRune(strings.TrimPrefix(got, "secret:"), ':') {
				t.Fatalf("resolveObject(%q) secret id contains a colon: %q", tc.entry.ObjectDeriver, got)
			}
		})
	}
...
// objectCapturingFGA records every ClientCheckRequest it is asked to execute
// and answers `allowed` to all of them. Recording the body is the point: a
// stub that answered without looking at the object could not tell a
// per-resource check from a tenant-wide one, which is exactly the distinction
// under test.
...
func (m *objectCapturingFGA) captured() []fgaclient.ClientCheckRequest {
	m.mu.Lock()
	defer m.mu.Unlock()
	return append([]fgaclient.ClientCheckRequest(nil), m.requests...)
}

type objectCapturingReq struct {
	m    *objectCapturingFGA
...
func (r *objectCapturingReq) GetAuthorizationModelIdOverride() *string  { return nil } //nolint:revive,staticcheck // method name set by openfga SDK request interface
func (r *objectCapturingReq) GetStoreIdOverride() *string               { return nil } //nolint:revive,staticcheck // method name set by openfga SDK request interface
func (r *objectCapturingReq) GetContext() context.Context               { return context.Background() }
func (r *objectCapturingReq) GetBody() *fgaclient.ClientCheckRequest    { b := r.body; return &b }
func (r *objectCapturingReq) GetOptions() *fgaclient.ClientCheckOptions { return nil }

// fieldDerivedYAML mirrors the live registry's three field-derived rules: a
// per-secret can_resolve rule keyed on the request's Name field.
const fieldDerivedYAML = `entries:
  "/test.v1.S/GetCredential":
...
// TestCheck_FieldDerivedRule_PassesThroughCoarseOnly — a rule whose FGA object
// derives from a request body field the gateway cannot see is handler-enforced:
// ext-authz runs the coarse checks and PASSES THROUGH, leaving the exact-object
// decision to the daemon handler (gibson#1245, owner decision option 1). The
// gateway must never ask FGA a per-object question it cannot form.
//
// The FGA stub says ALLOW to everything and records every question; the test
// asserts the pass-through (allow) AND that FGA was asked NOTHING — the decision
// is deferred, not made at the edge.
func TestCheck_FieldDerivedRule_PassesThroughCoarseOnly(t *testing.T) {
	t.Parallel()
	reg, err := LoadRegistry([]byte(fieldDerivedYAML))
	if err != nil {
		t.Fatal(err)
	}
	stub := &objectCapturingFGA{allowed: true}
	checker := NewChecker(stub, reg)
	id := headers.Identity{
		Subject:        "plugin_principal:7",
		Tenant:         "acme",
		CredentialType: headers.CredentialCapabilityGrant,
	}

	// The metadata the ext-authz server can actually build: the tenant, and
	// nothing from the request body.
	ok, err := checker.Check(context.Background(), "/test.v1.S/GetCredential", id,
		map[string]string{"tenant": "acme"})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if !ok {
		t.Fatal("expected pass-through allow for a handler-enforced field-derived rule, got deny")
	}
	if got := stub.captured(); len(got) != 0 {
		t.Fatalf("FGA was asked %d question(s) %+v; expected none — the gateway must "+
			"defer the per-object decision to the handler, not ask FGA", len(got), got)
	}
}

// TestCheck_FieldDerivedRule_IgnoresAnyFieldValueAtTheGateway — even if a field
// value somehow appears in the gateway's request metadata (a spoofed header, a
// buggy meta builder), ext-authz must STILL pass through and ask FGA nothing.
// The header the gateway would authorize and the body the handler acts on could
// disagree (issue #1245), so the gateway never derives or decides on the object.
func TestCheck_FieldDerivedRule_IgnoresAnyFieldValueAtTheGateway(t *testing.T) {
	t.Parallel()
	reg, err := LoadRegistry([]byte(fieldDerivedYAML))
	if err != nil {
		t.Fatal(err)
	...
	ok, err := checker.Check(context.Background(), "/test.v1.S/GetCredential", id,
		map[string]string{"tenant": "acme", "Name": "openai-prod"})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if !ok {
		t.Fatal("expected pass-through allow, got deny")
	}
	if got := stub.captured(); len(got) != 0 {
		t.Fatalf("FGA was asked %d question(s) %+v; the gateway must NOT derive the "+
			"object from a request field even when one is present in metadata", len(got), got)
	}
}

// TestCheck_FieldDerivedRule_StillEnforcesIdentityClass — the pass-through is
// coarse-ONLY, not a blanket allow: a caller of the wrong identity class is
// still denied at the edge, before the handler is ever reached. The rule allows
// COMPONENT only; a USER caller must be denied and FGA never consulted.
//
// Mutation guard: this is what pins the ordering (identity-class check BEFORE
// the handler-enforced pass-through). Move the pass-through above the class
// check and this test goes red.
func TestCheck_FieldDerivedRule_StillEnforcesIdentityClass(t *testing.T) {
	t.Parallel()
	reg, err := LoadRegistry([]byte(fieldDerivedYAML))
	if err != nil {
		t.Fatal(err)
	...
	if ok {
		t.Fatal("expected deny for USER on a COMPONENT-only rule; the coarse identity-class " +
			"check must run before the handler-enforced pass-through")
	}
	if got := stub.captured(); len(got) != 0 {
		t.Fatalf("FGA was asked %d question(s); expected none for a mis-classed caller", len(got))
	}
}

// TestCachedChecker_FieldDerivedRule_PassesThroughAndDoesNotCache — the cache
// layer mirrors Checker: a handler-enforced rule passes through before object
// resolution, asks FGA nothing, and stores no cache entry (there is no object to
// key one on).
func TestCachedChecker_FieldDerivedRule_PassesThroughAndDoesNotCache(t *testing.T) {
	t.Parallel()
	reg, err := LoadRegistry([]byte(fieldDerivedYAML))
	if err != nil {
		t.Fatal(err)
	...
	stub := &objectCapturingFGA{allowed: true}
	cc := NewCachedChecker(NewChecker(stub, reg), 0, 100)
	id := headers.Identity{
		Subject:        "plugin_principal:7",
		Tenant:         "acme",
		CredentialType: headers.CredentialCapabilityGrant,
	...
	ok, err := cc.Check(context.Background(), "/test.v1.S/GetCredential", id,
		map[string]string{"tenant": "acme"})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if !ok {
		t.Fatal("expected pass-through allow at the CachedChecker level, got deny")
	}
	if got := stub.captured(); len(got) != 0 {
		t.Fatalf("FGA was asked %d question(s); expected none", len(got))
	}
	if n := cc.Len(); n != 0 {
		t.Fatalf("cache holds %d entries; a handler-enforced pass-through must not be cached", n)
	}
}

// TestCheck_TenantScopedRule_StillResolvesAndAsksFGA — the CONTRAST case that
// proves the pass-through is scoped to field-derived rules only. A normal
// tenant_from_identity rule still resolves its object and asks FGA, and an FGA
// deny is a deny. Without this, a bug widening handlerEnforced to all rules
// would turn the whole gateway into a pass-through and this test would catch it.
func TestCheck_TenantScopedRule_StillResolvesAndAsksFGA(t *testing.T) {
	t.Parallel()
	stub := &objectCapturingFGA{allowed: false} // FGA denies
	checker := NewChecker(stub, makeAllowedIdentitiesReg(t))
	id := headers.Identity{Subject: "u-1", Tenant: "acme", CredentialType: "oidc-user"}

	ok, err := checker.Check(context.Background(), "/test.v1.S/UserOnly", id,
		map[string]string{"tenant": "acme"})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if ok {
		t.Fatal("expected deny for a tenant-scoped rule when FGA denies")
	}
	got := stub.captured()
	if len(got) != 1 {
		t.Fatalf("expected exactly 1 FGA question for a tenant-scoped rule, got %d", len(got))
	}
	if want := "tenant:acme"; got[0].Object != want {
		t.Fatalf("FGA object = %q, want %q", got[0].Object, want)
	}
}
