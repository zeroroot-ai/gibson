...
func initOTel() {
	otelOnce.Do(func() {
		meter := otel.GetMeterProvider().Meter("github.com/zeroroot-ai/gibson/internal/server/extauthz/fga")
...
func (r *platformFGAReq) Execute() (*fgaclient.ClientCheckResponse, error) {
	start := nowFn()
	// Condition context MUST be forwarded. CheckActiveSession attaches
	// {token_issued_at: <iat>} here, and the stored active_session tuple
	// carries revoked_at; OpenFGA needs both to evaluate token_not_revoked.
	// Dropping it does not soften the check into a denial — OpenFGA rejects
	// the whole Check as a validation error ("tuple ... is missing context
	// parameters"), which ext-authz surfaces as "FGA unavailable" and turns
	// into a 503 on EVERY session-gated RPC, tenant-wide (gibson#1191).
	...
	if r.body.Context != nil {
		req.Context = *r.body.Context
	}
	resp, err := r.adapter.inner.Check(r.ctx, req)
	dur := nowFn().Sub(start)
