...
func TestIsCapabilityGrantJWT_AgentJWT(t *testing.T) {
	_, priv := genKeyPair(t)
	now := time.Now()
	tp := buildAgentToken(priv, "a", "h", "aud", "jti", now, now.Add(30*time.Second))
	assert.True(t, IsCapabilityGrantJWT(tp.token()), "agent+jwt must be recognised")
}

func TestIsCapabilityGrantJWT_HostJWT(t *testing.T) {
	_, priv := genKeyPair(t)
	now := time.Now()
	tp := buildHostToken(priv, "h", "aud", now, now.Add(30*time.Second))
	assert.True(t, IsCapabilityGrantJWT(tp.token()), "host+jwt must be recognised")
}

func TestIsCapabilityGrantJWT_RegularJWT(t *testing.T) {
	...
	assert.False(t, IsCapabilityGrantJWT(token), "standard JWT must not be recognised as Agent Auth JWT")
}

func TestIsCapabilityGrantJWT_TwoPartsOnly(t *testing.T) {
	// Simulate a Better Auth session token with only one dot.
	assert.False(t, IsCapabilityGrantJWT("payload.signature"))
}

func TestIsCapabilityGrantJWT_EmptyString(t *testing.T) {
	assert.False(t, IsCapabilityGrantJWT(""))
}

func TestIsCapabilityGrantJWT_MalformedBase64Header(t *testing.T) {
	assert.False(t, IsCapabilityGrantJWT("!!!.payload.sig"))
}

func TestIsCapabilityGrantJWT_EdDSARequired(t *testing.T) {
	...
	assert.False(t, IsCapabilityGrantJWT(token), "agent+jwt with RS256 must not be recognised")
}
