...
// convertToolChoice converts llms.ToolChoice to anthropicclient.ToolChoice
func convertToolChoice(choice any) *anthropicclient.ToolChoice {
	if choice == nil {
		return nil
	}

	// Handle llms.ToolChoice struct
	if tc, ok := choice.(llms.ToolChoice); ok {
		if tc.Type == "" {
			return nil
		}
		result := &anthropicclient.ToolChoice{
			Type: tc.Type,
		}
		// If type is "tool" and function is specified, set the name
		if tc.Type == "tool" && tc.Function != nil && tc.Function.Name != "" {
			result.Name = tc.Function.Name
		}
		return result
	}

	// Handle string type (e.g., "auto", "any")
	if s, ok := choice.(string); ok {
		return &anthropicclient.ToolChoice{Type: s}
	}
