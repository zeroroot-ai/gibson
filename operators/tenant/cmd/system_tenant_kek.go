...
// buildWriteTenantBrokerConfigDeps (deploy#971). It wraps loadSystemTenantKEK
// in a closure so the KEK is read at reconcile time, not captured once at
// startup. The function also performs the startup-time probe: if the Secret is
// not yet available it logs an informational message (NOT an error) so the
// operator stays healthy while PlatformBootstrap (a later Argo wave) populates
// gibson-master-key. The WriteTenantBrokerConfig saga step requeues until
// resolveKEK returns a non-empty value.
//
// Extracted from buildWriteTenantBrokerConfigDeps so it is unit-testable
// without a live Postgres pool.
func newSystemTenantKEKProvider(log logr.Logger) func() []byte {
	provider := func() []byte { return loadSystemTenantKEK(log) }
	if len(provider()) == 0 {
