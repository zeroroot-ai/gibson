...
func main() {
	...
	// Zitadel IAM client — authenticates as the operator's own Zitadel
	// machine user (the gibson-zitadel-tenant-operator OIDCClient) via an
	// OAuth2 client_credentials grant, reusing the ZITADEL_TENANT_OPERATOR_
	// CLIENT_ID/SECRET already required above for the dashboard-audience
	// token. hosted#200: the operator no longer holds the shared IAM_OWNER
	// PAT — its machine user declares exactly the roles its calls need
	// (charts sets them on the OIDCClient CR), and this client reports a
	// permission failure the way Zitadel returns it (ErrUnauthorized)
	...
	// dashboard-audience token above — because Zitadel's Management/v2 API
	// rejects admin writes that cross the public edge (gibson#1560).
	//
	...
	zitadelClient, err = buildTenantOperatorZitadelClient(operatorClientID, operatorClientSecret, zitadelURL, zitadelExternalDomain)
	if err != nil {
		setupLog.Error(err, "failed to build the operator's Zitadel client (hosted#200)")
		os.Exit(1)
	}
	...
	if grpcAddr := os.Getenv("GIBSON_DAEMON_GRPC_ADDRESS"); grpcAddr != "" {
		...
		// operator enqueues the first tenant itself over the workload-authed
		// EnqueueTenantProvisioning RPC. The PendingProvisioningRunnable above
	...
	// ConnectorInstance authz — converges the connector's FGA component
	// tuples (plugin-parity posture: owner + tenant_enabled) and removes
	// them behind a finalizer on disable. The connector-operator owns the
	// runtime; the tenant-operator stays the single FGA writer in the
	// operator plane. ADR-0067, gibson#1548.
	if err := (&controller.ConnectorInstanceAuthzReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
		FGA:    fgaClient,
	}).SetupWithManager(mgr); err != nil {
...
func buildDataPlaneProvisioner(
	...
	n4j, err := dataplane.NewNeo4jProvisioner(dataplane.Neo4jConfig{
		...
		VaultClient: vaultClient,
		// PlatformNamespace is the namespaceSelector half of the
		// per-tenant Neo4j bolt NetworkPolicy's single ingress peer
...
func buildWriteTenantBrokerConfigDeps(log logr.Logger) flows.WriteTenantBrokerConfigDeps {
	...
	//
	// deploy#971: the KEK is provided LAZILY via newSystemTenantKEKProvider —
	// read at reconcile time, not captured once here, so the operator does NOT
	// crash-loop when gibson-master-key is absent at startup.
	systemTenantKEK := newSystemTenantKEKProvider(log)
