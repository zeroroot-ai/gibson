...
// aggregateChildStatus lifts scalar status a child owns up onto the parent
// Tenant status, so a consumer that reads the Tenant (bootstrap-tenant-owner,
// the daemon, the dashboard) sees one authoritative record without having to
// know the child CRDs. Today that is only the Zitadel org id: the
// EnsureZitadelOrg saga step became the TenantIdentity child (gibson#805), which
// writes the org id to its OWN status — leaving Tenant.Status.ZitadelOrgID (a
// field that still exists and is still read, e.g. by the first-admin bootstrap)
// permanently empty. This closes that gap. Runs every reconcile once the child
// is Ready; the reconcile's closing Status().Patch persists it.
func aggregateChildStatus(child client.Object, tenant *gibsonv1alpha1.Tenant) {
	if ti, ok := child.(*gibsonv1alpha1.TenantIdentity); ok && ti.Status.ZitadelOrgID != "" {
		tenant.Status.ZitadelOrgID = ti.Status.ZitadelOrgID
	}
}
