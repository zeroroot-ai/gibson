...
func TestAggregateChildStatus_LiftsZitadelOrgID(t *testing.T) {
	tenant := &gibsonv1alpha1.Tenant{}
	// A ready TenantIdentity with an org id must lift onto the parent.
	ti := &gibsonv1alpha1.TenantIdentity{}
	ti.Status.ZitadelOrgID = "387205818321207333"
	aggregateChildStatus(ti, tenant)
	if tenant.Status.ZitadelOrgID != "387205818321207333" {
		t.Fatalf("want org id lifted onto Tenant.Status, got %q", tenant.Status.ZitadelOrgID)
	}
	// An empty child org id must not clobber an already-set parent value.
	empty := &gibsonv1alpha1.TenantIdentity{}
	aggregateChildStatus(empty, tenant)
	if tenant.Status.ZitadelOrgID != "387205818321207333" {
		t.Errorf("empty child org id must not clobber parent, got %q", tenant.Status.ZitadelOrgID)
	}
	// A non-identity child must be a no-op.
	other := &gibsonv1alpha1.TenantSecretsBackend{}
