...
// TestReconcile_NetworkPolicy_NoBoltGrantOnDefaultDeny is the successor to
// TestReconcile_NetworkPolicy_DaemonNeo4jIngress (gibson#1255).
//
// The daemon → Neo4j bolt:7687 grant used to live on the namespace-wide
// gibson-tenant-default-deny policy. It has moved to the dedicated
// per-tenant policy the Neo4j provisioner emits, because the default-deny
// policy no longer selects the Neo4j pod at all — its intra-namespace
// allow-all rule would otherwise union away the restriction.
//
// A bolt rule left behind here would be dead config that reads as though
// it grants something, so this test asserts its ABSENCE. The grant itself
// is covered by TestNeo4jNetworkPolicyAdmitsOnlyTheDaemonOnBolt in
// internal/dataplane, and the exclusion by TestDefaultDenyExcludesTenantNeo4j.
func TestReconcile_NetworkPolicy_NoBoltGrantOnDefaultDeny(t *testing.T) {
	tenant := &gibsonv1alpha1.Tenant{
		ObjectMeta: metav1.ObjectMeta{
			Name:       "acme",
			Finalizers: []string{gibsonv1alpha1.TenantFinalizer},
	...
	for _, rule := range np.Spec.Ingress {
		for _, port := range rule.Ports {
			if port.Port != nil && port.Port.IntValue() == 7687 {
				t.Error("gibson-tenant-default-deny still carries a bolt:7687 ingress rule — " +
					"it no longer selects the Neo4j pod, so the rule grants nothing and misleads")
			}
		}
	}
}

// TestReconcile_NetworkPolicy_DaemonEgressIsPodScoped verifies that the egress
// rule opening the daemon ports selects the daemon pod, not merely the platform
// namespace. A namespaceSelector on its own permits a tenant workload to reach
// every pod in the platform namespace on those ports — Postgres, Redis,
// OpenFGA, Zitadel, OpenBao and Envoy all live there.
func TestReconcile_NetworkPolicy_DaemonEgressIsPodScoped(t *testing.T) {
	tenant := &gibsonv1alpha1.Tenant{
		ObjectMeta: metav1.ObjectMeta{
			Name:       "acme",
			Finalizers: []string{gibsonv1alpha1.TenantFinalizer},
	...
	foundDaemonEgress := false
	for _, rule := range np.Spec.Egress {
		for _, port := range rule.Ports {
			if port.Port == nil || port.Port.IntValue() != 50002 {
				continue
			}
			foundDaemonEgress = true
			if len(rule.To) == 0 {
				t.Fatal("daemon egress rule has no To peers")
			}
			for i, peer := range rule.To {
				if peer.NamespaceSelector == nil {
					t.Errorf("daemon egress peer %d missing NamespaceSelector", i)
				}
				if peer.PodSelector == nil {
					t.Errorf("daemon egress peer %d missing PodSelector — rule permits egress to the whole platform namespace", i)
					continue
				}
				if got := peer.PodSelector.MatchLabels["app.kubernetes.io/component"]; got != "daemon" {
					t.Errorf("daemon egress peer %d PodSelector component = %q, want daemon", i, got)
				}
			}
		}
	}
	if !foundDaemonEgress {
		t.Error("NetworkPolicy has no egress rule for the daemon gRPC port")
	}
}
