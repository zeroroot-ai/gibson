...
func (c *httpClient) mountJWTAuth(ctx context.Context, namespace string) error {
	...
	// Skip the enable if the auth method is already mounted (see mountKVv2).
	if c.mountedAt(ctx, namespace, "/v1/sys/auth", jwtPath) {
		return nil
	}
...
func (c *httpClient) mountKVv2(ctx context.Context, namespace, mountPath string) error {
	// Skip the enable if the engine is already mounted. Reconciles run every
	// few minutes; re-POSTing the enable makes OpenBao log a server-side ERROR
	// ("path is already in use") in the tenant namespace each cycle. mountedAt
	// returns false on ANY uncertainty, so we fall through to the idempotent
	// POST below and never suppress a real mount.
	if c.mountedAt(ctx, namespace, "/v1/sys/mounts", mountPath) {
		return nil
	}
	body := map[string]any{
		"type": "kv",
...
// mountedAt reports whether a secrets engine or auth method is already present
// at mountPath ("secret", "jwt", …) under listPath ("/v1/sys/mounts" or
// "/v1/sys/auth") in the given namespace. The list response carries the table
// under "data", keyed by path-with-trailing-slash. On ANY error or unexpected
// shape it returns false, so callers fall through to the idempotent enable — a
// wrong check must never suppress a genuine mount.
func (c *httpClient) mountedAt(ctx context.Context, namespace, listPath, mountPath string) bool {
	var resp struct {
		Data map[string]json.RawMessage `json:"data"`
	}
	if err := c.do(ctx, http.MethodGet, listPath, namespace, nil, &resp); err != nil {
		return false
	}
	_, ok := resp.Data[strings.TrimSuffix(mountPath, "/")+"/"]
	return ok
}
...
path "secret/data/cred:*" {
  capabilities = ["create", "read", "update", "delete"]
}

path "secret/metadata/cred:*" {
  capabilities = ["read", "list", "delete"]
}

path "secret/data/provider_config:*" {
  capabilities = ["create", "read", "update", "delete"]
}

path "secret/metadata/provider_config:*" {
  capabilities = ["read", "list", "delete"]
}
`
}
