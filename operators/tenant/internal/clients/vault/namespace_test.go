...
func TestPolicyHCLContent(t *testing.T) {
	...
	// cred:* secrets (SecretsService credentials like cred:github_token and
	// connector OAuth grants cred:connector/<vendor>/grant) get full CRUD.
	// Distinct from provider_cred:* (LLM broker fields); without this grant
	// SetSecret and the connector-token refresher get "vault denied".
	if !strings.Contains(hcl, `path "secret/data/cred:*"`) {
		t.Fatal("tenant policy missing cred: data path (blocks plugin creds + connector OAuth grants)")
	}
	if !strings.Contains(hcl, `path "secret/metadata/cred:*"`) {
		t.Fatal("tenant policy missing cred: metadata path")
	}

	// provider_config:* secrets (SecretsService PROVIDER_CONFIG category; the
	// LLM credential-resolution broker-first read at provider_config:<p>:<field>)
	// get full CRUD. Same class as cred:; without it SetSecret and provider
	// credential resolution get "vault denied".
	if !strings.Contains(hcl, `path "secret/data/provider_config:*"`) {
		t.Fatal("tenant policy missing provider_config: data path")
	}
	if !strings.Contains(hcl, `path "secret/metadata/provider_config:*"`) {
		t.Fatal("tenant policy missing provider_config: metadata path")
	}
...
// TestMountKVv2_SkipsEnableWhenAlreadyMounted proves the check-before-enable:
// when GET sys/mounts already lists the engine, mountKVv2 must NOT re-POST the
// enable (which makes OpenBao log a server-side ERROR every reconcile). When it
// is absent, the enable is issued. A GET failure falls through to the enable.
func TestMountKVv2_SkipsEnableWhenAlreadyMounted(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			c := ac.(*httpClient)
			if err := c.mountKVv2(context.Background(), "", "secret"); err != nil {
				t.Fatalf("mountKVv2: %v", err)
			}
...
// TestMountJWTAuth_SkipsEnableWhenAlreadyMounted mirrors the KV test for the
// JWT auth method: no enable POST when sys/auth already lists jwt/.
func TestMountJWTAuth_SkipsEnableWhenAlreadyMounted(t *testing.T) {
	for _, tc := range []struct {
		name         string
		mountPresent bool
		wantPosts    int
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				...
				case r.Method == http.MethodGet && r.URL.Path == "/v1/sys/auth":
					w.Header().Set("Content-Type", "application/json")
					if tc.mountPresent {
						_, _ = w.Write([]byte(`{"data":{"jwt/":{"type":"jwt"}}}`))
					} else {
						_, _ = w.Write([]byte(`{"data":{}}`))
					}
				case r.Method == http.MethodPost && r.URL.Path == "/v1/sys/auth/jwt":
					posts++
					w.WriteHeader(http.StatusNoContent)
				default:
					w.WriteHeader(http.StatusNotFound)
			...
			c := ac.(*httpClient)
			if err := c.mountJWTAuth(context.Background(), ""); err != nil {
				t.Fatalf("mountJWTAuth: %v", err)
			}
			if posts != tc.wantPosts {
				t.Errorf("enable POSTs = %d, want %d", posts, tc.wantPosts)
			}
