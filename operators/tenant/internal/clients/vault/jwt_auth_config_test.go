...
func TestConfigureTenantJWTAuth_WritesMirroredConfig(t *testing.T) {
	...
	c, err := New(Config{
		...
		JWKSURL:          jwksURL,
		JWKSCAPEM:        caPEM,
		// srv.Client() gives this subtest a transport dedicated to its own
		// httptest.Server instead of the process-wide http.DefaultTransport.
		// The five t.Parallel() subtests here otherwise share DefaultTransport,
		// and httptest.Server.Close() unconditionally calls
		// http.DefaultTransport.CloseIdleConnections() — which can tear a
		// keep-alive connection out from under a sibling subtest mid-request,
		// surfacing as "http: CloseIdleConnections called" (gibson#1338).
...
func TestConfigureTenantJWTAuth_Idempotent(t *testing.T) {
	...
	c, err := New(Config{
		...
		// Isolated per-server transport — see WritesMirroredConfig (gibson#1338).
		HTTPClient: srv.Client(),
	})
	if err != nil {
		t.Fatalf("New: %v", err)
