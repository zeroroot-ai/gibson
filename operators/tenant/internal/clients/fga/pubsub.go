...
// WriteConditional delegates to the inner client. Condition-bearing tuples are
// not published to the membership-cache channel: the active_session relation is
// not a role-hierarchy change and carries no dashboard membership implication.
func (p *publishingClient) WriteConditional(ctx context.Context, t ConditionalTuple) error {
	if err := p.Client.WriteConditional(ctx, t); err != nil {
		return fmt.Errorf("fga: WriteConditional: %w", err)
	}
	return nil
}
