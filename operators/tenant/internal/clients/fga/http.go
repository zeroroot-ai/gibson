...
// WriteConditional implements Client. It writes a single condition-bearing FGA
// tuple by including the condition name and context in the tuple_keys entry.
// The OpenFGA write API accepts a condition object on each tuple key; no
// special endpoint is required — the standard /stores/:id/write is used.
//
// Idempotent: if an identical (user, relation, object, condition, context)
// tuple already exists, ErrAlreadyExists is mapped to nil by doJSON.
//
// Spec: instant-session-revocation (gibson#627 Slice 2).
func (c *HTTPClient) WriteConditional(ctx context.Context, t ConditionalTuple) error {
	start := time.Now()

	tupleKey := map[string]any{
		"user":     t.User,
		"relation": t.Relation,
		"object":   t.Object,
	}
	if t.ConditionName != "" {
		cond := map[string]any{"name": t.ConditionName}
		if len(t.ConditionContext) > 0 {
			cond["context"] = t.ConditionContext
		}
		tupleKey["condition"] = cond
	}

	body := map[string]any{
		"authorization_model_id": c.modelID,
		"writes": map[string]any{
			"tuple_keys": []any{tupleKey},
		},
	}
	err := c.doJSON(ctx, fmt.Sprintf("/stores/%s/write", c.storeID), body, nil)
	if err != nil && errors.Is(err, clients.ErrAlreadyExists) {
		metrics.ObserveSubsystemCall("fga", "WriteConditional", start, nil)
		return nil
	}
	metrics.ObserveSubsystemCall("fga", "WriteConditional", start, err)
	return err
}
