...
// TestBrokerConfigOps_ResolveKEK_ErrorPaths verifies that both WriteBrokerConfig
// and DeleteBrokerConfig propagate resolveKEK retryable errors correctly
// (deploy#971): a nil KEK provider or an empty KEK must surface as an error so
// the saga step requeues rather than panicking or silently writing a bad row.
// The nil-pool subcase checks that a NewStore construction failure (valid KEK,
// nil pool) is also wrapped and returned by each operation.
func TestBrokerConfigOps_ResolveKEK_ErrorPaths(t *testing.T) {
	t.Parallel()
	...
	kek32 := func() []byte { return bytes.Repeat([]byte{0x42}, 32) }

	type opFunc func(d WriteTenantBrokerConfigDeps) error
	write := func(d WriteTenantBrokerConfigDeps) error { return d.WriteBrokerConfig(ctx, "acme-kek-test") }
	del := func(d WriteTenantBrokerConfigDeps) error { return d.DeleteBrokerConfig(ctx, "acme-kek-test") }

	for _, op := range []struct {
		name string
		fn   opFunc
	}{
		{"WriteBrokerConfig", write},
		{"DeleteBrokerConfig", del},
	} {
		t.Run(op.name, func(t *testing.T) {
			t.Parallel()

			// nil KEK provider → resolveKEK returns error
			if err := op.fn(WriteTenantBrokerConfigDeps{VaultConfig: vc}); err == nil {
				t.Errorf("%s with nil KEK provider: expected error, got nil", op.name)
			}

			// empty KEK provider → resolveKEK retryable error
			emptyKEK := WriteTenantBrokerConfigDeps{
				SystemTenantKEK: func() []byte { return nil },
				VaultConfig:     vc,
			}
			if err := op.fn(emptyKEK); err == nil {
				t.Errorf("%s with empty KEK: expected retryable error, got nil", op.name)
			}

			// valid KEK but nil pool → resolveKEK succeeds, NewStore fails
			nilPool := WriteTenantBrokerConfigDeps{
				PlatformPG:      nil,
				SystemTenantKEK: kek32,
				VaultConfig:     vc,
			}
			if err := op.fn(nilPool); err == nil {
				t.Errorf("%s with nil pool: expected NewStore error, got nil", op.name)
			}
		})
	}
}
