...
func TestComponentGrantTuplesCanonicalForm(t *testing.T) {
	...
	if tu.Object != "component:tool/noop" {
		t.Errorf("object = %q, want canonical component:tool/noop", tu.Object)
	}
}
...
func TestWriteComponentGrantsStepWritesCanonicalTuples(t *testing.T) {
	...
	for _, tu := range fakeFGA.snapshot() {
		if tu.Relation == "direct_execute" && tu.Object == "component:tool/noop" &&
