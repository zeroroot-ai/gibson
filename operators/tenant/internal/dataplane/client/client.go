...
// perTenantKind reports whether o is one of the kinds in PerTenantKinds,
// and returns that kind's name for use in diagnostics.
...
//
// The name is returned as a compile-time constant rather than derived
// from o at the call site on purpose: the guard below reports on objects
// that routinely hold live tenant credentials (*corev1.Secret carries the
// per-tenant Neo4j password), and passing such an object into a
// formatting call — even under %T, which reads only the dynamic type —
// puts the credential one verb change away from the operator's logs.
// Keeping o out of the error's argument list entirely removes that
// footgun instead of relying on the verb staying %T (gibson#1444).
func perTenantKind(o ctrlclient.Object) (string, bool) {
	switch o.(type) {
...
func (c *Client) guard(obj ctrlclient.Object, namespace, op string) error {
	...
	if namespace == c.operatorNamespace {
		// obj is deliberately NOT an argument here — see perTenantKind.
		return fmt.Errorf(
			"dataplane/client: refusing %s of %s in operator namespace %q — "+
