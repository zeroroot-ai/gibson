...
func (c *httpClient) EnsureTransitMounted(ctx context.Context) error {
	// Check before enabling. A reconcile runs every few minutes; POSTing an
	// enable for an already-mounted path makes OpenBao log a server-side ERROR
	// ("path is already in use") every cycle, which buries real errors and adds
	// needless load. A GET is a clean read. On ANY uncertainty mountExists
	// returns false and we fall through to the idempotent POST below, so the
	// check can never skip a genuine mount.
	if c.mountExists(ctx, "transit/") {
		return nil
	}
	body := map[string]any{
		"type":        "transit",
...
// mountExists reports whether a secrets engine is already mounted at mountPath
// (e.g. "transit/") in the root namespace. It reads sys/mounts, whose response
// carries the mount table under "data" keyed by path-with-trailing-slash. On
// ANY error or unexpected shape it returns false, so the caller falls through
// to the idempotent enable — a wrong check must never suppress a real mount.
func (c *httpClient) mountExists(ctx context.Context, mountPath string) bool {
	var resp struct {
		Data map[string]json.RawMessage `json:"data"`
	}
	if err := c.doJSON(ctx, http.MethodGet, "/v1/sys/mounts", nil, &resp); err != nil {
		return false
	}
	_, ok := resp.Data[mountPath]
	return ok
}
