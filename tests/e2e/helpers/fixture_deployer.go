...
// testRedisClient dials the cluster's Redis for the fixture helpers below.
//
...
// and every fixture that registers a target fails before the test it belongs to
...
func RegisterTestTarget(ctx context.Context, name, targetURL string) (string, error) {
	...
	if err := rdb.Ping(ctx).Err(); err != nil {
		return "", fmt.Errorf("fixture_deployer: RegisterTestTarget: ping Redis %s: %w", rdb.Options().Addr, err)
	}
