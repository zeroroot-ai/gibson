...
//
// An *empty* allowlist is not the same as a nil filter: warn/info/log/debug are
// unconditionally benign and stay filtered even with zero entries, while every
// error survives. Only a nil *ConsoleFilter (no allowlist configured at all)
// passes messages through untouched. Short-circuiting on len(cf.entries) == 0
// conflated the two and leaked development noise into the residue (gibson#1293).
...
func (cf *ConsoleFilter) isAllowed(msg ConsoleMessage) bool {
	...
	if msgLevel == "warn" || msgLevel == "info" || msgLevel == "log" || msgLevel == "debug" {
		...
		// as they are always benign. (This branch is only reachable for those
		// four levels, so the old `return msgLevel != "error"` was a condition
