...
func TestTenantContext(t *testing.T) {
	...
	t.Run("nil_context_does_not_fall_back_to_the_system_tenant", func(t *testing.T) {
		// The legacy internal/identity.TenantFromContext returned "_system" on
		// absence. auth.TenantStringFromContext deliberately does NOT: it
		// returns "" so a missing identity can never be silently promoted to
		// the privileged system tenant, and call sites that genuinely want the
		// fallback have to opt in explicitly. This assertion tracked the legacy
		// behaviour and had never run (gibson#1293); it now pins the current,
		// fail-closed contract.
		tenant := auth.TenantStringFromContext(nil)
		assert.Empty(t, tenant, "Nil context must not yield a tenant")
		assert.NotEqual(t, auth.SystemTenantString, tenant,
			"Nil context must never be promoted to the system tenant")
	})
