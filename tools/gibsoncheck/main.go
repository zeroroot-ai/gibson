...
//
//   - failopenauthorizer: a function whose job is to make an
//     authorization decision may not treat an ABSENT authorization
//     dependency as permission granted. Two shapes: a nil-guard whose
//     branch is permissive for the function's own result signature, and
//     the omission form where the only check sits inside
//     `if <dep> != nil` so a nil dependency skips it. Gated on the TYPE
//     of the dependency and on RETURN POLARITY, which is what keeps
//     fail-CLOSED helpers (`return false, nil`) and plain value
//     nil-guards out.
//
//   - privilegedfallback: a privileged sentinel (auth.SystemTenant and
//     friends, declared in checks/privileged_sentinels.yaml) may not be
//     substituted for an absent tenant or identity on the failure branch
//     of a fallible security accessor. Callers of a tainted helper are
//     reported too, via an analysis.Fact, so a reintroduction under a new
//     name is caught by shape rather than by name.
//
//   - fgalistusers: Authorizer.ListUsers hardcodes a user-type filter, so
//     calling it for a relation that cannot yield a user-typed subject
//     returns a successful EMPTY result — an authorization query that is
//     structurally incapable of answering. Checked against
//     internal/platform/authz/model.fga, parsed with the official OpenFGA
//     transformer, so a later model narrowing that invalidates a
//     previously-correct call is caught.
//
//   - constantverdictdouble: a test double standing in for an
//     authorization decision must read the arguments that carry the
//     decision. Reports only PERMISSIVE doubles: one that always denies
//     cannot hide a defect, and that asymmetry is what keeps the check
//     usable.
//
// The four guards above record their pre-existing findings in
// checks/guard_baseline.txt, a shrink-only ratchet whose stale half is
// enforced by scripts/check-guard-baselines.sh.
//
...
//     NO baseline and should never acquire one.
//
//   - cypheridentifier: in internal/engine/graphrag, a caller-supplied node
//     type, relationship type or property name may reach Cypher read-path
//     query text only through the cypherFrag type and its designated
//     constructors (sanitizeIdentifier and friends). Flags a raw
//     fmt.Sprintf/string-concatenation building Cypher-shaped text, and a
//     conversion to cypherFrag that bypasses every constructor. Ships with
//     NO baseline and should never acquire one (gibson#1440).
//
...
func main() {
	...
	multichecker.Main(
		[]*analysis.Analyzer{
			...
			checks.FailOpenAuthorizerAnalyzer,
			checks.PrivilegedFallbackAnalyzer,
			checks.FGAListUsersAnalyzer,
