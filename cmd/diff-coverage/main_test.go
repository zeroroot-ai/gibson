...
func TestParseAddedLines_WhitespaceOnlyRealignmentExcluded(t *testing.T) {
	// gibson#1157: a gofmt struct-literal realignment rewrites lines without
	// changing their content. Those must not count as changed lines.
	diff := `+++ b/internal/server/daemon/mission_manager.go
@@ -150,3 +150,3 @@
-	store:   store,
-	logger:  logger,
-	tracker: tracker,
...
`
	got := parseAddedLines([]byte(diff))
	if lines := got["internal/server/daemon/mission_manager.go"]; len(lines) != 0 {
		t.Fatalf("whitespace-only realignment must yield no added lines, got %v", lines)
	}
}

func TestParseAddedLines_MixedHunkKeepsRealChanges(t *testing.T) {
	// One line is realigned (whitespace-only), one is genuinely new, one is a
	// real content change. Only the latter two count.
	diff := `+++ b/a.go
@@ -10,2 +10,3 @@
-	alpha :=  1
-	beta := compute(x)
...
`
	...
	if len(lines) != 2 || lines[0] != 11 || lines[1] != 12 {
		t.Fatalf("want [11 12] (alpha realignment skipped), got %v", lines)
	}
}

func TestParseAddedLines_DuplicateContentConsumedOnce(t *testing.T) {
	// Two identical added lines but only one matching removed line: the match
	// is consumed once; the second copy still counts as added.
	diff := `+++ b/a.go
@@ -10,1 +10,2 @@
-	x++
+	x++
...
`
	...
	if len(lines) != 1 || lines[0] != 11 {
		t.Fatalf("want [11] (first copy matched to removal), got %v", lines)
	}
}

func TestParseAddedLines_MatchingScopedToHunk(t *testing.T) {
	// A removal in one hunk must not absolve an identical addition in a later
	// hunk of a DIFFERENT file; matching is per-hunk.
	diff := `+++ b/a.go
@@ -10,1 +9,0 @@
-	shared := 1
+++ b/b.go
...
`
	got := parseAddedLines([]byte(diff))
	if lines := got["b.go"]; len(lines) != 1 || lines[0] != 21 {
		t.Fatalf("cross-file/hunk match must not be absolved; want [21], got %v", lines)
	}
}
