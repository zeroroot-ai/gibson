...
func TestTenantIDSource(t *testing.T) {
	...
	allowlist := map[string]string{
		...
		"internal/daemon/api/tenant_admin_langfuse_delete.go:32":   "platform-operator action; req.TenantId is the object being acted upon, not the authz subject",
		...
		"internal/daemon/api/tenant_admin_langfuse_delete.go:45":   "platform-operator action; req.TenantId is the object being acted upon, not the authz subject",
		"internal/daemon/api/tenant_admin_langfuse_delete.go:49":   "platform-operator action; req.TenantId is the object being acted upon, not the authz subject",
		"internal/daemon/api/tenant_admin_langfuse_delete.go:53":   "platform-operator action; req.TenantId is the object being acted upon, not the authz subject",
		"internal/daemon/api/tenant_admin_langfuse_get.go:32":      "platform-operator action; req.TenantId is the object being acted upon, not the authz subject",
		"internal/daemon/api/tenant_admin_langfuse_get.go:37":      "platform-operator action; req.TenantId is the object being acted upon, not the authz subject",
		"internal/daemon/api/tenant_admin_langfuse_get.go:45":      "platform-operator action; req.TenantId is the object being acted upon, not the authz subject",
		"internal/daemon/api/tenant_admin_langfuse_get.go:54":      "platform-operator action; req.TenantId is the object being acted upon, not the authz subject",
		"internal/daemon/api/tenant_admin_langfuse_get.go:55":      "platform-operator action; req.TenantId is the object being acted upon, not the authz subject",
		"internal/daemon/api/tenant_admin_langfuse_get.go:57":      "platform-operator action; req.TenantId is the object being acted upon, not the authz subject",
		"internal/daemon/api/tenant_admin_langfuse_get.go:59":      "platform-operator action; req.TenantId is the object being acted upon, not the authz subject",
		"internal/daemon/api/tenant_admin_langfuse_get.go:64":      "platform-operator action; req.TenantId is the object being acted upon, not the authz subject",
		...
		"internal/daemon/api/tenant_admin_langfuse_set.go:35":      "platform-operator action; req.TenantId is the object being acted upon, not the authz subject",
		"internal/daemon/api/tenant_admin_langfuse_set.go:52":      "platform-operator action; req.TenantId is the object being acted upon, not the authz subject",
		"internal/daemon/api/tenant_admin_langfuse_set.go:56":      "platform-operator action; req.TenantId is the object being acted upon, not the authz subject",
		"internal/daemon/api/tenant_admin_langfuse_set.go:67":      "platform-operator action; req.TenantId is the object being acted upon, not the authz subject",
		"internal/daemon/api/tenant_admin_langfuse_set.go:76":      "platform-operator action; req.TenantId is the object being acted upon, not the authz subject",
		...
		"internal/daemon/api/tenant_admin_langfuse_set.go:84":      "platform-operator action; req.TenantId is the object being acted upon, not the authz subject",
