...
func TestNoGracefulNilInRequestPaths(t *testing.T) {
	...
	requestPathAllowlist := astchecks.Allowlist{
		...
		"internal/platform/identity/service.go :: if s.authorizer == nil || tenantID == \"\" { ... }":                         astchecks.Entry{Category: astchecks.CategoryLegacyOptional, Reason: "s.authorizer nil-check in canRevokeSomeSessions; fail-closed (no revoke capability) when FGA unwired — mirrors server_revoke_sessions.go, predates noopAuthorizer deletion"},
		...
		"internal/server/daemon/grpc.go :: if d.graphWriter == nil { ... }":                                                   astchecks.Entry{Category: astchecks.CategoryLegacyOptional, Reason: "graph writer nil-guard in CreateMission's best-effort, non-fatal background Neo4j mirror (ADR-0012 single writer); the RPC's own success never depends on it"},
