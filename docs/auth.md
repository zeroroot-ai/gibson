...
The public counterpart is served **per kid**, at
`/capabilitygrant/v1/keys/{kid}`
([`internal/server/daemon/capabilitygrant_keys.go`](../internal/server/daemon/capabilitygrant_keys.go)).
That one endpoint answers for both kid classes (ADR-0045): the daemon's own
signing kid returns a bare key document (dispatch CG-JWTs carry their own
claims), a registered component's kid returns a key descriptor that also carries
the daemon-asserted FGA principal and tenant. ext-authz fetches the single kid it
needs and caches it for 1 hour.

**ext-authz reads it over mTLS, and only over mTLS** (GHSA-8m76-9r77-xvh3). The
route is mounted on the daemon's `:8086` SPIFFE-mTLS listener beside the authz
registry ([`authz_registry_subsystem.go`](../internal/server/daemon/authz_registry_subsystem.go)),
and `cmd/ext-authz` fetches it with a client pinned to `EXT_AUTHZ_DAEMON_SVID`
via `tlsconfig.AuthorizeID` — the same transport, the same pin and the same env
var as the registry fetch. `EXT_AUTHZ_CGJWT_KEYS_URL` must be `https`; a
plaintext URL fails startup rather than downgrading the fetch.

The reason is integrity, not confidentiality. The document is public-key
material, so nothing about it is secret — but it is also the assertion that
binds a kid to an FGA principal, and ext-authz turns that binding directly into
the caller's identity. A key you cannot attribute to the daemon is not a trust
anchor, so the peer that serves it is authenticated before any of it is read.
Pinning is `AuthorizeID`, not trust-domain membership: every workload in
`zeroroot.ai` holds a valid SVID, and only one of them is the daemon.
...
ext-authz's job
([`internal/server/extauthz/cgjwt/verifier.go`](../internal/server/extauthz/cgjwt/verifier.go)).
